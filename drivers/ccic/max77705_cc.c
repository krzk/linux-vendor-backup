...
static irqreturn_t max77705_detabrt_irq(int irq, void *data)
{
	struct max77705_usbc_platform_data *usbc_data = data;
	struct max77705_cc_data *cc_data = usbc_data->cc_data;
	...
	pr_debug("%s: IRQ(%d)_IN\n", __func__, irq);
	max77705_read_reg(usbc_data->muic, REG_CC_STATUS1, &cc_data->cc_status1);
	cc_data->detabrt = (cc_data->cc_status1 & BIT_AttachSrcErrI)
			>> FFS(BIT_AttachSrcErrI);
	msg_maxim("New Detabrt Status Interrupt (%d)",
		cc_data->detabrt);
	pr_debug("%s: IRQ(%d)_OUT\n", __func__, irq);
...
int max77705_cc_init(struct max77705_usbc_platform_data *usbc_data)
{
	...
	cc_data->irq_detabrt = usbc_data->irq_base + MAX77705_CC_IRQ_DETABRT_INT;
	if (cc_data->irq_detabrt) {
		ret = request_threaded_irq(cc_data->irq_detabrt,
			   NULL, max77705_detabrt_irq,
			   0,
			   "cc-detabrt-irq", usbc_data);
		if (ret) {
			pr_err("%s: Failed to Request IRQ (%d)\n", __func__, ret);
			goto err_irq;
		}
