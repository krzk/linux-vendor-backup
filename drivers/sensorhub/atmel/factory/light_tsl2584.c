...
uint32_t light_get_lux(uint16_t ch0, uint16_t ch1, int32_t gain)
{
	...
	ch0lux = (int32_t)((taos_device_lux_tsl2584tsv[0].coeffA * ch0) -
		(taos_device_lux_tsl2584tsv[0].coeffB * ch1)) / (gain << MPOW);
	ch1lux = (int32_t)((taos_device_lux_tsl2584tsv[0].coeffC * ch0) -
		(taos_device_lux_tsl2584tsv[0].coeffD * ch1)) / (gain << MPOW);
