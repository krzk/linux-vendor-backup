...
static int max14577_reg_get_current_limit(struct regulator_dev *rdev)
{
	u8 reg_data;
	struct regmap *rmap = rdev->regmap;
	struct maxim_core *max14577 = rdev_get_drvdata(rdev);
 ...
 */
static struct regmap *max14577_get_regmap(struct maxim_core *max14577,
		int reg_id)
{
...
{
	struct maxim_core *max14577 = dev_get_drvdata(pdev->dev.parent);
	...
	for (i = 0; i < supported_regulators_size; i++) {
		...
		regulator = regulator_register(&supported_regulators[i],
						&config);
		if (IS_ERR(regulator)) {
