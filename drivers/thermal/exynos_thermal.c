...
static int exynos_tmu_read(struct exynos_tmu_data *data)
{
	...
	for (i = 0; i < EXYNOS_TMU_COUNT; i++) {
		...
		if (temp < 0)
			temp = 0;
#ifdef CONFIG_THERMAL_TEST
		if (data->temp_test != INT_MAX)
			temp = data->temp_test;
#endif
		alltemp[i] = temp;
...
#ifdef CONFIG_THERMAL_TEST
static ssize_t exynos_thermal_show_temp_test(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct exynos_tmu_data *data = th_zone->sensor_conf->private_data;
...
static ssize_t exynos_thermal_store_temp_test(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("%s: Force set temperature to %d\n", __func__, temp);
	return count;
}
static DEVICE_ATTR(temp_test, S_IRUGO| S_IWUGO,
	exynos_thermal_show_temp_test, exynos_thermal_store_temp_test);
#endif /* CONFIG_THERMAL_TEST */
