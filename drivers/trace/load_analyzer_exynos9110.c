...
#if defined(CONFIG_SLP_BUSY_LEVEL)
	if (cpu_busy_level_enable) {
		cpu_busy_level = check_load_level(cnt);

		if (time_before(jiffies, high_cpu_load_boot_completed)) {
			if (cpu_busy_level == 9) {
		...
		} else if (cpu_busy_level != cpu_busy_level_before) {
			pr_info("cpu_busy_level=%d->%d", cpu_busy_level_before, cpu_busy_level);

			cpu_busy_level_before = cpu_busy_level;
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_LOAD] = busy_state_get_time_occur();

			pr_info("%s: [BUSY_LEVEL] Trigger reason: High cpu load\n", __FUNCTION__);
