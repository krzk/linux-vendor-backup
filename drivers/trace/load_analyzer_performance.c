...
//static unsigned long performance_boot_completed;
...
int check_load_level(unsigned int performance_cnt)
{
	...
	//if (time_before(jiffies, performance_boot_completed))
	//	return 0;
...
static ssize_t performance_monitor_read(struct file *file,
	char __user *buffer, size_t count, loff_t *ppos)
{
	...
	ret = show_performance_monitor_read_sub(cnt, show_line_num, buf, count, ret);
...
static ssize_t performance_monitor_write(struct file *file,
...
int performance_monitor_en = 1;

static int performance_monitor_en_read_sub(char *buf, int buf_size)
{
	int ret = 0;

	ret += snprintf(buf + ret, buf_size - ret, "%d\n", performance_monitor_en);

	return ret;
}

static ssize_t performance_monitor_en_read(struct file *file,
		char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned int size_for_copy;

	size_for_copy = wrapper_for_debug_fs(buffer, count, ppos
						,performance_monitor_en_read_sub);

	return size_for_copy;
}

static ssize_t performance_monitor_en_write(struct file *file,
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	char buf[32];
	size_t buf_size = min(count, (sizeof(buf)-1));
	...
	performance_monitor_en = atoi(buf);

	return count;
}

static const struct file_operations performance_monitor_fops = {
	.owner = THIS_MODULE,
	.read  = performance_monitor_read,
	.write = performance_monitor_write,
};

static const struct file_operations performance_monitor_en_fops = {
	.owner = THIS_MODULE,
	.read  = performance_monitor_en_read,
	.write = performance_monitor_en_write,
};

void debugfs_performance(struct dentry *d)
{
	if (!debugfs_create_file("performance_monitor", 0600, d, NULL, &performance_monitor_fops))
		pr_err("%s : debugfs_create_file, error\n", "performance_monitor");

	if (!debugfs_create_file("performance_monitor_en", 0600, d, NULL, &performance_monitor_en_fops))
		pr_err("%s : debugfs_create_file, error\n", "performance_monitor_en");
}
/*
void load_analyzer_performance_init(void)
{
	#define EXPECTED_BOOT_TIME	(90 * HZ)
	performance_boot_completed = jiffies + EXPECTED_BOOT_TIME;
}
*/
