...
int input_rec_store_input_dev_info(void)
{
	...
	list_for_each_safe(p, n, &input_dev_head) {
		...
		input_dev_info_performance[i].dev = dev;
		strncpy(input_dev_info_performance[i].name, dev->name, MAX_INPUT_DEV_NAME_LEN-1);
		i++;
	}

	input_dev_info_performance[i].dev = &touch_booster;
	strncpy(input_dev_info_performance[i].name, touch_booster.name, MAX_INPUT_DEV_NAME_LEN-1);
	i++;

	input_dev_info_performance[i].dev = &rotary_booster;
	strncpy(input_dev_info_performance[i].name, rotary_booster.name, MAX_INPUT_DEV_NAME_LEN-1);
	i++;

	input_dev_info_performance_num = i;

	cnt = i;

	for (i = 0; i < cnt; i++) {
		pr_info("Current [%d]dev=%p, name=%s\n",
				i, input_dev_info_performance[i].dev, input_dev_info_performance[i].name);
	}
...
int input_rec_save_data(void)
{
	...
	data = input_dev_info_performance;
	size = sizeof(struct input_dev_info_tag) * input_dev_info_performance_num;
	save_data_to_file(file_path, data, size);
...
struct input_dev *input_rec_change_dev(struct input_dev *old_dev)
{
	...
	for (i = 0 ; i < input_dev_info_performance_num; i++) {
		if (strcmp(input_dev_info_performance[i].name, input_dev_info_saved[old_dev_index].name) == 0) {
			break;
		}
	}

	if (i >= input_dev_info_performance_num)
		goto fail;
	...
	return input_dev_info_performance[new_dev_index].dev;
		...
		loff_t *ppos)
{
	...
	input_rec_str_to_index(&start_index, &end_index, (char *)buf);
