...
#define EXT_CLOCK_RATE			14745600
...
static u8 xrm117x_port_read(struct uart_port *port, u8 reg)
{
	...
	if(status < 0)
		dev_err(s->dev, "Failed to xrm117x_port_read error code %d\n", status);
...
static void xrm117x_port_write(struct uart_port *port, u8 reg, u8 val)
{
	...
	if(status < 0)
		dev_err(s->dev, "Failed to xrm117x_port_write Err_code %d\n", status);
	mutex_unlock(&s->mutex_bus_access);
}
...
void xrm117x_raw_write(struct uart_port *port, const void *reg,
			unsigned char *buf, int len)
{
	...
	if(status < 0)
		dev_err(s->dev, "Failed to xrm117x_raw_write Err_code %d\n", status);
	mutex_unlock(&s->mutex_bus_access);
}
...
void xrm117x_raw_read(struct uart_port *port, unsigned char *buf, int len)
{
	...
	mutex_unlock(&s->mutex_bus_access);
	if(status < 0)
		dev_err(s->dev, "Failed to xrm117x_raw_read Err_code %d\n", status);
}
#else
...
static u8 xrm117x_port_read(struct uart_port *port, u8 reg)
{
	...
	if (ret == 2) {
		...
		dev_err(s->dev, "Failed to xrm117x_port_read <%d>\n", ret);
		return ret;
	} else {
		return -EIO;
	}
...
static int xrm117x_port_write(struct uart_port *port, u8 reg, u8 val)
{
	...
	if (ret == 2) {
		...
		dev_err(s->dev, "Failed to xrm117x_port_write <%d>\n", ret);
		return ret;
	} else	{
		return -EIO;
	}
...
static int xrm117x_port_fifo_write(struct uart_port *port,
					const void *val, size_t val_len)
{
	...
	if (ret == val_len + 1) {
		...
		dev_err(s->dev, "Failed to xrm117x_port_fifo_write <%d>\n", ret);
		return ret;
	} else {
		return -EIO;
	}
...
static int xrm117x_raw_write(struct uart_port *port, const void *reg,
		unsigned char *buf, int len)
{
	struct xrm117x_port *s = dev_get_drvdata(port->dev);
	struct i2c_msg xfer[2];
	 ...
	 */
	if (!i2c_check_functionality(xrm117x_i2c_client->adapter,
				I2C_FUNC_NOSTART)) {
	...
	if (ret == 2) {
		...
		dev_err(s->dev, "Failed to xrm117x_raw_write <%d>\n", ret);
		return ret;
	} else {
		return -EIO;
	}
...
static int xrm117x_raw_read(struct uart_port *port,unsigned char *buf,int len)
{
	...
	if (ret == 2) {
		...
		dev_err(s->dev, "Failed to xrm117x_raw_read <%d>\n", ret);
		return ret;
	} else {
		return -EIO;
	}
...
static void xrm117x_handle_tx(struct uart_port *port)
{
	...
	if (uart_circ_empty(xmit) || uart_tx_stopped(port)) {
		dev_dbg(s->dev, "xrm117x_handle_tx stopped\n");
		return;
	}
	...
	if (likely(to_send)) {
		...
		dev_dbg(s->dev, "xrm117x_handle_tx %d bytes\n", to_send);
...
static void xrm117x_port_irq(struct xrm117x_port *s, int portno)
{
	...
	do {
		...
		case XRM117XX_IIR_CTSRTS_SRC:
			...
			dev_dbg(s->dev, "uart_handle_cts_change =0x%02x\n", msr);
			break;
...
static void xrm117x_md_proc(struct work_struct *ws)
{
	...
	/* xrm117x_port_update(&one->port, XRM117XX_MCR_REG,
		XRM117XX_MCR_LOOP_BIT,
		(one->port.mctrl & TIOCM_LOOP) ? XRM117XX_MCR_LOOP_BIT : 0);
	*/
}
				  ...
				  struct ktermios *old)
{
	...
	dev_dbg(port->dev, "xrm117x_set_termios write EFR = 0x%02x\n", flow);

	/* Update LCR register */
	xrm117x_port_write(port, XRM117XX_LCR_REG, lcr);

	if (termios->c_cflag & CRTSCTS) {
		dev_dbg(port->dev, "xrm117x_set_termios enable rts/cts\n");
		xrm117x_port_update(port, XRM117XX_MCR_REG,
				XRM117XX_MCR_RTS_BIT, XRM117XX_MCR_RTS_BIT);
	} else {
		dev_dbg(port->dev, "xrm117x_set_termios disable rts/cts\n");
		xrm117x_port_update(port, XRM117XX_MCR_REG,
				XRM117XX_MCR_RTS_BIT, 0);
	}
	/* xrm117x_port_write(port, XRM117XX_IOCONTROL_REG, io_control); */
	/* configure GPIO  to GPIO or Modem control mode */
...
static int xrm117x_startup(struct uart_port *port)
{
	...
	udelay(5);
	xrm117x_port_write(port, XRM117XX_FCR_REG, XRM117XX_FCR_FIFO_BIT);

	/* Enable EFR */
	xrm117x_port_write(port, XRM117XX_LCR_REG, XRM117XX_LCR_CONF_MODE_B);

	/* Enable write access to enhanced features and internal clock div */
	xrm117x_port_write(port, XRM117XX_EFR_REG, XRM117XX_EFR_ENABLE_BIT);

	/* Enable TCR/TLR */
	xrm117x_port_update(port, XRM117XX_MCR_REG,
			      XRM117XX_MCR_TCRTLR_BIT, XRM117XX_MCR_TCRTLR_BIT);
	...
	val =	XRM117XX_IER_RDI_BIT |
		XRM117XX_IER_THRI_BIT |
		XRM117XX_IER_CTSI_BIT;
...
{
	dev_dbg(port->dev, "xrm117x_shutdown...\n");
			   ...
			   int irq, unsigned long flags)
{
	...
	ret = devm_request_threaded_irq(dev, irq, NULL, xrm117x_ist,
					...
					IRQF_SHARED | flags, dev_name(dev), s);

	dev_info(s->dev, "xrm117x_probe~ irq = %d result= %d\n", irq, ret);
	if (!ret)
		return 0;
