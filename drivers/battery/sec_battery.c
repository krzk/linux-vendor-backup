				...
				bool enable)
{
	...
	pr_info("%s:bat_status[%d] enable[%d]", __func__, battery->status, enable);
...
#ifdef CONFIG_WPC_BCM5935X
	if ((battery->cable_type == POWER_SUPPLY_TYPE_WPC) &&
			(battery->charging_passed_time < 600)) {
			charging_heat = battery->temperature - battery->charging_start_temp;
			pr_info("heat[%d],start_temp[%d]\n", charging_heat, battery->charging_start_temp);
			if (charging_heat > 120) {
				psy_do_property("sec-wpc2", set,
					POWER_SUPPLY_PROP_CURRENT_AVG, val);
				val.intval = 475;
				psy_do_property(battery->pdata->charger_name, set,
