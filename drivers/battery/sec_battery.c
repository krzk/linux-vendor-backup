				...
				if (battery->status == POWER_SUPPLY_STATUS_CHARGING) {
					if (battery->charging_mode == SEC_BATTERY_CHARGING_NONE) {
						...
						if (battery->voltage_now < 4200) {
							...
							psy_do_property("wpc", set,
								POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
						} else if ((battery->voltage_now >= 4200) && (battery->voltage_now < 4250)) {
							value.intval = SEC_MID_BATT_MODE;
							psy_do_property("wpc", set,
								POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
						} else if (battery->voltage_now >= 4250) {
...
static void sec_booting_check_work(struct work_struct *work)
{
	...
	if (battery->swelling_mode) {
		pr_info("%s: Skip normal check routine for swelling mode\n",
			__func__);
		return;
	}
...
static void sec_bat_wpc_mode_check(struct sec_battery_info *battery)
{
	...
	if (battery->status == POWER_SUPPLY_STATUS_DISCHARGING) {
		dev_err(battery->dev, "%s: Charging Disabled [%d]\n",
			__func__, battery->cable_type);
		value.intval = battery->voltage_now;
		psy_do_property("wpc", set,
			POWER_SUPPLY_PROP_PRESENT, value);
		return;
	}
	...
	if (battery->cable_type == POWER_SUPPLY_TYPE_WPC) {
		...
		if (battery->retail_mode == true) {
			...
			if ((battery->capacity < 50) && (value.intval != SEC_FULL_NONE_LDU_MODE)) {
				value.intval = SEC_LOW_BATT_LDU_MODE;
				psy_do_property("wpc", set,
					POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
			} else if ((battery->capacity >= 50) && (value.intval != SEC_FULL_NONE_LDU_MODE)) {
...
#if defined(CONFIG_BATTERY_SWELLING)// && !defined(CONFIG_IFA_DEMO)
static void sec_bat_swelling_check(struct sec_battery_info *battery, int temperature)
{
	...
	pr_info("%s: 1st status(%d), swell_mode(%d:%d:%d), cv(0x%02x), temp(%d)\n",
		__func__, battery->status, battery->swelling_mode, battery->swelling_partial,
	...
	if (!battery->swelling_mode) {
		if (((temperature >= battery->pdata->swelling_high_temp_block) ||
			...
			/* Add swelling condition when temperature 5 ~ 10 C under 4.2V */
			if ((temperature <= battery->pdata->swelling_low_temp_block) &&
				(temperature > (battery->pdata->swelling_low_temp_block - 50))) {
				pr_info("%s: match swelling under 4.2V between 5 ~ 10 C down (%d:%d)\n",
						__func__, temperature, battery->voltage_now);
				battery->swelling_partial = true;
			}
		...
		pr_info("%s: now swelling mode (%d:%d)\n",
				__func__, temperature, battery->voltage_now);
		if (!battery->swelling_partial) {
			if ((temperature < battery->pdata->swelling_low_temp_recov) &&
				(temperature > (battery->pdata->swelling_low_temp_block - 50))) {
				pr_info("%s: match swelling parital (%d:%d)\n",
						__func__, temperature, battery->voltage_now);
				battery->swelling_partial = true;
				sec_bat_set_charge(battery, false);
			}
			...
			if ((temperature >= battery->pdata->swelling_low_temp_recov) ||
				(temperature <= (battery->pdata->swelling_low_temp_block - 50))) {
				pr_info("%s: match swelling not partial(%d:%d)\n",
						__func__, temperature, battery->voltage_now);
	...
	pr_info("%s: 2nd status(%d), swell_mode(%d:%d:%d), cv(0x%02x), temp(%d)\n",
		__func__, battery->status, battery->swelling_mode, battery->swelling_partial,
		battery->charging_block, val.intval, temperature);
	...
	if (battery->swelling_mode) {
		...
		if ((temperature <= battery->pdata->swelling_high_temp_recov) &&
			...
			pr_info("%s: swelling mode end. restart charging (t:%d, v:%d)\n",
				__func__, temperature, battery->voltage_now);
			battery->swelling_mode = false;
			battery->swelling_partial = false;
			...
			pr_info("%s: swelling mode vnow < rechg_vol (t:%d, v:%d)\n",
				__func__, temperature, battery->voltage_now);
			pr_info("%s: swelling mode reduce charging (temp:%d)\n",
			...
			if ((temperature > battery->pdata->swelling_high_temp_recov) ||
				...
				if ((temperature < battery->pdata->swelling_low_temp_recov) &&
					(temperature > (battery->pdata->swelling_low_temp_block - 50))) {
					pr_info("%s: swelling 5 ~ 15 (t:%d, v:%d)\n",
							__func__, temperature, battery->voltage_now);
					battery->swelling_partial = true;
					...
					pr_info("%s: swelling the others (t:%d, v:%d)\n",
							__func__, temperature, battery->voltage_now);
					battery->swelling_partial = false;
				...
				pr_info("%s: swelling mode reduce charging current (%d mA)\n", __func__,
					battery->pdata->swelling_low_temp_current);
				val.intval = battery->pdata->swelling_low_temp_current;
				psy_do_property(battery->pdata->charger_name, set,
			   ...
			   battery->charging_block) {
			pr_info("%s: swelling mode vnow >= rechg_vol I(t:%d, v:%d)\n", __func__,
				temperature, battery->voltage_now);
			if ((temperature > battery->pdata->swelling_high_temp_recov) ||
				(temperature < battery->pdata->swelling_low_temp_recov)) {
				if ((temperature < battery->pdata->swelling_low_temp_recov) &&
					(temperature > (battery->pdata->swelling_low_temp_block - 50))) {
					pr_info("%s: swelling partial mode reduce charging current(t:%d, v:%d) ",
						__func__, temperature, battery->voltage_now);
					pr_info(" (vnow:%d)\n", battery->voltage_now);
					battery->swelling_partial = true;
					battery->charging_mode = SEC_BATTERY_CHARGING_1ST;
					...
					pr_info(" 0.2C (current:%d)\n", val.intval);
					psy_do_property(battery->pdata->charger_name, set,
							POWER_SUPPLY_PROP_CURRENT_AVG, val);
				} else {
					pr_info("%s: swelling mode (vnow:%d)(temp:%d)\n"
						, __func__, battery->voltage_now, temperature);
					battery->swelling_partial = false;
					/* change 4.20V float voltage */
					val.intval = battery->pdata->swelling_drop_float_voltage;
					psy_do_property(battery->pdata->charger_name, set,
		...
		} else if (!battery->charging_block) {
			pr_info("%s: swelling mode !battery->charging_block (t:%d, v:%d)\n", __func__,
				temperature, battery->voltage_now);
			if ((temperature < battery->pdata->swelling_low_temp_recov) &&
				(temperature > (battery->pdata->swelling_low_temp_block - 50))) {
				pr_info("%s: swelling partial mode ", __func__);
				battery->swelling_partial = true;
				/* change 4.4V float voltage */
				val.intval = battery->pdata->swelling_normal_float_voltage;
				psy_do_property(battery->pdata->charger_name, set,
	...
	if ((battery->health == POWER_SUPPLY_HEALTH_OVERHEAT) ||
		...
		if ((battery->health == POWER_SUPPLY_HEALTH_GOOD) &&
			...
			if ((temp_value >= battery->pdata->swelling_high_temp_block) ||
				...
				/* Add for factory mode because no need to turn off charger */
				if ((battery->is_jig_on == false) || (battery->retail_mode == false)) {
					battery->swelling_mode = true;
					battery->swelling_full_check_cnt = 0;
					sec_bat_set_charge(battery, false);
				} else {
					pr_info("%s: skip swelling mode start. is_jig_on(%d) retail_mode(%d)\n"
							, __func__, battery->is_jig_on, battery->retail_mode);
				}
			} else {
				...
				struct sec_battery_info *battery)
{
	...
	if (battery->status == POWER_SUPPLY_STATUS_CHARGING) {
		pr_debug("%s : (battery->status == POWER_SUPPLY_STATUS_CHARGING)\n"
			,__func__);
		if ((battery->ldu_charger_on == true) && (battery->capacity >= 50)){
			...
			psy_do_property(battery->pdata->fuelgauge_name, set,
				POWER_SUPPLY_PROP_ONLINE, val);
		} else if ((battery->ldu_charger_on == false) && (battery->capacity < 40)) {
...
static void sec_bat_wpc_chg_mode(
				struct sec_battery_info *battery)
{
	union power_supply_propval val;
	...
	if ((battery->status == POWER_SUPPLY_STATUS_CHARGING) ||
		(battery->status == POWER_SUPPLY_STATUS_FULL)) {
		if ((battery->limit_current == 0) && (battery->capacity >= 96)) {
			val.intval = 75;
			pr_info("%s : Change Charging Current (%d)\n",__func__, val.intval);
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_AVG, val);
			battery->limit_current = 1;
		} else if ((battery->limit_current == 1) && (battery->capacity < 96)) {
			val.intval = 200;
			pr_info("%s : Change Charging Current (%d)\n",__func__, val.intval);
			psy_do_property(battery->pdata->charger_name, set,
					POWER_SUPPLY_PROP_CURRENT_AVG, val);
			battery->limit_current = 0;
		}
	}
}
				...
				struct work_struct *work)
{
	...
	dev_info(battery->dev, "%s: cable_type : %d\n", __func__, battery->cable_type);
	/* -2. usb chg on/off*/
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = battery->cable_type;
		if (battery->cable_type == POWER_SUPPLY_TYPE_WPC) {
			if (battery->charging_passed_time > 60) {
				psy_do_property(battery->pdata->charger_name, get,
		...
		struct sec_battery_info *battery)
{
	...
	pr_info("%s : SWELLING_HIGH_TEMP(%d) SWELLING_HIGH_TEMP_RECOVERY(%d)\n"
		...
		pdata->swelling_high_temp_current, pdata->swelling_low_temp_current);
