	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_ONLINE:
			...
			if (val->intval == POWER_SUPPLY_TYPE_WPC) {
				p9220_reg_write(charger->client, 0x3F, 0x7E);
				p9220_reg_write(charger->client, 0x3E, 0x0E);
...
#endif
				{
					...
					switch (val->intval) {
						...
						case SEC_SWELLING_LDU_MODE:
							...
							p9220_reg_write(charger->client, 0x3E, 0x0B);
							msleep(200);
							p9220_reg_write(charger->client, 0x3F, 0x3E);
							break;
...
static void p9220_wpc_mode_change(struct work_struct *work)
{
	...
	switch (charger->pdata->charging_mode) {
		case SEC_INITIAL_MODE:
			break;
		case SEC_SWELLING_MODE:
			p9220_reg_write(charger->client, 0x3E, 0x0E);
			msleep(200);
			p9220_reg_write(charger->client, 0x3F, 0x6E);
			break;
		case SEC_LOW_BATT_MODE:
			p9220_reg_write(charger->client, 0x3E, 0x0B);
			msleep(200);
			p9220_reg_write(charger->client, 0x3F, 0x0F);
			break;
		case SEC_MID_BATT_MODE:
			p9220_reg_write(charger->client, 0x3E, 0x0E);
			msleep(200);
		...
		case SEC_HIGH_BATT_MODE:
			p9220_reg_write(charger->client, 0x3E, 0x0E);
			msleep(200);
			p9220_reg_write(charger->client, 0x3F, 0x46);
			break;
		case SEC_VERY_HIGH_BATT_MODE:
			p9220_reg_write(charger->client, 0x3E, 0x0E);
			msleep(200);
			p9220_reg_write(charger->client, 0x3F, 0x5E);
			break;
		case SEC_FULL_NONE_MODE:
			p9220_reg_write(charger->client, 0x3E, 0x0E);
			msleep(200);
						...
						const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&charger->wpc_init_work, p9220_wpc_init_detect);
	schedule_delayed_work(&charger->wpc_init_work, msecs_to_jiffies(2000));
...
static int p9220_charger_resume(struct i2c_client *client)
{
	...
	if (charger->pdata->charging_mode != SEC_INITIAL_MODE) {
		p9220_reg_write(charger->client, 0x3F, 0x7E);
		msleep(200);
		p9220_reg_write(charger->client, 0x3E, 0x0E);
		schedule_delayed_work(&charger->wpc_mode_work, msecs_to_jiffies(5000));
