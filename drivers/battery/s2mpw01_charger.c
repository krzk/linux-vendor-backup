...
#if 0
		ret = s2mpw01_read_reg(charger->client, S2MPW01_CHG_REG_CTRL4, &temp);
		if (ret < 0) {
			pr_err("[DEBUG]%s: i2c read error ! %d\n", __func__, ret);
		}
		pr_info("[DEBUG]%s chg_ctrl4: %02x\n " , __func__, temp);
		ret = s2mpw01_write_reg(charger->client, S2MPW01_CHG_REG_CTRL4, (temp & 0x0F));
		if (ret < 0) {
			pr_err("[DEBUG]%s: i2c write error ! %d\n", __func__, ret);
		}
		ret = s2mpw01_write_reg(charger->client, S2MPW01_CHG_REG_CTRL4, temp);
		if (ret < 0) {
			pr_err("[DEBUG]%s: i2c write error !! %d\n", __func__, ret);
		}
		ret = s2mpw01_read_reg(charger->client, S2MPW01_CHG_REG_STATUS1, &temp1);
		if (ret < 0) {
			pr_err("[DEBUG]%s: i2c read error !! %d\n", __func__, ret);
		}
		pr_info("[DEBUG]%s chg_status1: %02x\n " , __func__, temp1);
		if(temp1 & TOP_OFFSTATUS_MASK) {
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		if (charger->onoff == 1) {
			...
			wake_lock_timeout(&charger->ac_ok_lock, HZ);
