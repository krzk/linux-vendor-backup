...
int decon_enable(struct decon_device *decon)
{
	...
	if (ret)
		decon_err("%s:pm_runtime_get_sync[%d]\n", __func__, ret);
#else
...
int decon_disable(struct decon_device *decon)
{
	...
	if (ret)
		decon_err("%s:pm_runtime_put_sync[%d]\n", __func__, ret);
#else
...
static int decon_blank(int blank_mode, struct fb_info *info)
{
	...
	decon_info("%s:blank[%d]state[%d]aod[%d]pm[%d]r[%d %d]\n", __func__,
...
bypass:
	decon_info("%s:blank[%d]state[%d]aod[%d]pm[%d]r[%d %d]ret[%d]\n", __func__,
		blank_mode, decon->state, decon->aod_state, pm_runtime_active(decon->dev),
...
static int decon_resume(struct device *dev)
{
	...
	decon_info("%s:state[%d]dpms[%d]aod[%d]pm[%d]r[%d %d]\n", __func__, decon->state,
...
static int decon_suspend(struct device *dev)
{
	...
	if (pm_runtime_active(decon->dev)) {
		if (decon->dpms != FB_BLANK_POWERDOWN && decon->aod_state == AOD_OFF) {
			...
			decon_info("%s:blank:state[%d]pm[%d]r[%d %d]\n", __func__, decon->state,
				pm_runtime_active(decon->dev), atomic_read(&decon->lpd_block_cnt),
		...
		case DECON_STATE_ON:
			...
			decon_info("%s:tried:state[%d]pm[%d]r[%d %d]\n", __func__, decon->state,
				pm_runtime_active(decon->dev), atomic_read(&decon->lpd_block_cnt),
				atomic_read(&decon->lpd_trig_cnt));
