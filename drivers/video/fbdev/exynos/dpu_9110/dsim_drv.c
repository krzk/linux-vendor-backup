...
static int dsim_get_gpios(struct dsim_device *dsim)
{
	...
	if (of_get_property(dev->of_node, "gpios", NULL) != NULL)  {
		...
		if (ret < 0) {
			...
			dsim_info("dsim_ub_con_det_irq_handler register\n");
...
static ssize_t dsim_show_ub_con_intr_cnt(struct device *dev, struct device_attribute *devattr, char *buf)
{
        ...
        return snprintf(buf, PAGE_SIZE, "%d\n", dsim->ub_con_intr_cnt > 0 ? dsim->ub_con_intr_cnt - 1 : 0);
}
...
static void dsim_register_panel(struct dsim_device *dsim)
{
	...
	if (!strcmp(lcd_info.name, "s6e36w2x01")) {
		dsim->panel_ops = &s6e36w2x01_mipi_lcd_driver;
		dsim_info("%s:%s\n", __func__, lcd_info.name);
	} else if (!strcmp(lcd_info.name, "s6e36w3x01")) {
		dsim->panel_ops = &s6e36w3x01_mipi_lcd_driver;
		dsim_info("%s:%s\n", __func__, lcd_info.name);
	} else {
		dsim_err("%s:Can't find panel name.\n", __func__);
		dsim->panel_ops = NULL;
	}
#else
