...
int __set_panel_power(struct panel_device *panel, int power)
{
	...
	pr_info("%s, power(%s)\n", __func__, power == PANEL_POWER_ON ? "on" : "off");
...
static int __panel_seq_exit(struct panel_device *panel)
{
	...
	ret = panel_set_gpio_irq(&panel->gpio[PANEL_GPIO_ERR_FG], false);
	if (ret < 0)
		panel_warn("PANEL:WARN:%s:do not support irq\n", __func__);
...
static int panel_sleep_in(struct panel_device *panel)
{
	...
	state->prev_state = state->cur_state;
	state->cur_state = PANEL_STATE_ON;
	panel_info("PANEL:INFO:%s panel_state:%s -> %s\n", __func__,
...
int __set_panel_reset(struct panel_device *panel)
{

	run_list(panel->dev, "panel_reset");
	panel_info("PANEL:INFO:%s: panel_reset\n", __func__);
...
static int panel_ioctl_set_reset(struct panel_device *panel)
{
	...
	state->connected = panel_conn_det_state(panel);
	if (state->connected < 0) {
		pr_debug("PANEL:INFO:%s:conn unsupported\n", __func__);
	} else if (state->connected == PANEL_STATE_NOK) {
		panel_warn("PANEL:WARN:%s:ub disconnected\n", __func__);
		return -ENODEV;
	} else {
		pr_debug("PANEL:INFO:%s:ub connected\n", __func__);
	...
	if (state->prev_state == PANEL_STATE_OFF) {
	...
	pr_debug("PANEL:INFO:%s : panel reset called\n", __func__);
	panel_info("PANEL:INFO:%s panel_state:%s -> %s\n", __func__,
			panel_state_names[state->prev_state], panel_state_names[state->cur_state]);
...
static int panel_set_indisplay_post(struct panel_device *panel, void *arg)
{
	...
	if (indisplay_request) {
		...
		panel_bl->props.indisplay_actual_br = 0;
...
static long panel_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	switch(cmd) {
		...
		case PANEL_IOC_REG_RESET_CB:
			panel_info("PANEL:INFO:%s:PANEL_IOC_REG_RESET_CB\n", __func__);
		...
		case PANEL_IOC_EVT_FRAME_DONE:
			...
			if (panel->state.disp_on == PANEL_DISPLAY_OFF) {
				...
				ret = panel_set_gpio_irq(&panel->gpio[PANEL_GPIO_ERR_FG], true);
				if (ret < 0)
					panel_warn("PANEL:WARN:%s:do not support irq\n", __func__);
...
static int panel_drv_set_gpios(struct panel_device *panel)
{
	...
	/*
	 * panel state is decided by boot_panel_id, conn_det and disp_det pin
	 *
	 * @boot_panel_id
	...
	if ((panel->state.init_at == PANEL_INIT_BOOT) &&
		...
		/*
			...
			disp_det is nomal(1)
			init panel in bootloader(boot_panel_id >= 0)
		*/
	...
	panel_info("PANEL:INFO:%s: id:0x%06x, disp_det:%d (init_at:%s, ub_con:%d(%s) panel:%d(%s))\n",
		__func__, boot_panel_id, det_val,
...
static int panel_parse_dt(struct panel_device *panel)
{
	...
	if (ret < 0) {
		panel_warn("PANEL:WARN:%s:panel-%d:failed to parse regulator\n",
			__func__, panel->id);
	}
...
void pcd_handler(struct work_struct * data)
{
	...
	if (decon == NULL) {
		panel_err("PANEL:ERR:%s: decon is NULL\n", __func__);
		return;
	}
	...
	if (pcd_state == PANEL_STATE_NOK) {
		...
		panel_info("PANEL:INFO:%s: PCD detect bypass on\n", __func__);
	}
	panel_info("PANEL:INFO:%s: pcd_state:%s\n",
			__func__, pcd_state == PANEL_STATE_OK ? "OK" : "NOK");
	...
	switch (*blank) {
		...
		case FB_BLANK_UNBLANK:
			if (event == FB_EARLY_EVENT_BLANK) {
				...
				pcd_state = panel_pcd_state(panel);
				panel_info("PANEL:INFO:%s: pcd_state:%s\n", __func__, 
					pcd_state == PANEL_STATE_OK ? "OK" : "NOK");
				if (pcd_state == PANEL_STATE_NOK) {
					decon_bypass_on_global(0);
					panel_info("PANEL:INFO:%s: PCD detect bypass on\n", __func__);
				}
...
static int compare_name(struct device *dev, void *data)
{
...
static struct platform_device *find_panel_device(void)
{
    struct platform_device *pdev = NULL;
    struct device *dev = NULL;

    dev = driver_find_device(&panel_driver.driver, NULL, "panel", compare_name);
    if (!dev) {
		panel_err("PANEL:ERR:%s: find device fail\n", __func__);
        return NULL;
    }

    pdev = to_platform_device(dev);
    if (!pdev) {
        panel_err("PANEL:ERR:%s: to_platform_device fail\n", __func__);
        return NULL;
    }
...
static int __init panel_late_init(void)
{
	...
	pdev = find_panel_device();
	if (!pdev) {
		panel_err("PANEL:ERR:%s: find_panel_device fail\n", __func__);
		return 0;
    }

	panel = platform_get_drvdata(pdev);
    if (!panel) {
		panel_err("PANEL:ERR:%s: platform_get_drvdata fail\n", __func__);
		return 0;
    }
