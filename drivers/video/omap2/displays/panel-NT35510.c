...
void nt35510_lcd_poweroff(void)
{
	...
	// fix backlight on issue by floating VLCD_1.8V
	// DISPLAY_MCLK GPIO_66
	omap34xx_pad_set_config_lcd(0x00D4,  OMAP34XX_MUX_MODE7 | OMAP34XX_PIN_INPUT_PULLDOWN);
	// DISPLAY_HSYNC GPIO_67
	omap34xx_pad_set_config_lcd(0x00D6,  OMAP34XX_MUX_MODE7 | OMAP34XX_PIN_INPUT_PULLDOWN);
	// DISPLAY_VSYNC GPIO_68
	omap34xx_pad_set_config_lcd(0x00D8,  OMAP34XX_MUX_MODE7 | OMAP34XX_PIN_INPUT_PULLDOWN);
	// DISPLAY_CE GPIO_69
	omap34xx_pad_set_config_lcd(0x00DA,  OMAP34XX_MUX_MODE7 | OMAP34XX_PIN_INPUT_PULLDOWN);
...
void nt35510_lcd_poweron(void)
{
	...
	// fix backlight on issue by floating VLCD_1.8V
	// DISPLAY_MCLK GPIO_66
	omap34xx_pad_set_config_lcd(0x00D4,  OMAP34XX_MUX_MODE0 | OMAP34XX_PIN_OFF_OUTPUT_LOW);
	// DISPLAY_HSYNC GPIO_67
	omap34xx_pad_set_config_lcd(0x00D6,  OMAP34XX_MUX_MODE0 | OMAP34XX_PIN_OFF_OUTPUT_LOW);
	// DISPLAY_VSYNC GPIO_68
	omap34xx_pad_set_config_lcd(0x00D8,  OMAP34XX_MUX_MODE0 | OMAP34XX_PIN_OFF_OUTPUT_LOW);
	// DISPLAY_CE GPIO_69
	omap34xx_pad_set_config_lcd(0x00DA,  OMAP34XX_MUX_MODE0 | OMAP34XX_PIN_OFF_OUTPUT_LOW);
...
static ssize_t lcd_version_read_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk(KERN_DEBUG "[LCD] lcd_version_read_show \n");
	return sprintf(buf, "SON_L4F00423\n");
}

static struct kobj_attribute lcd_hw_version = __ATTR(lcdversion, 0444, lcd_version_read_show, NULL);
...
static int nt35510_spi_probe(struct spi_device *spi)
{
	...
	error = sysfs_create_file(lcd_kobj, &lcd_hw_version.attr);
	if (error) {
