...
struct exynos_powermode_info {
	...
	int		sicd_block_jig;		/* blocking SICD by jig */
...
static int exynos_check_idle_ip_stat(int mode, int reg_index)
{
	...
	if (ret) {
		...
		cpuidle_profile_collect_idle_ip(mode, reg_index,
				((val & ~mask) ^ ~mask));
#ifdef CONFIG_ARM64_EXYNOS_CPUIDLE_STAT
		cpuidle_stat_collect_idle_ip(mode, reg_index,
				((val & ~mask) ^ ~mask));
#endif
	}
...
#ifdef CONFIG_ARM64_EXYNOS_CPUIDLE_STAT
			cpuidle_stat_collect_cpus_busy();
#endif
			return -EBUSY;
		}
...
int exynos_get_sicd_block_jig(void)
{
	return pm_info->sicd_block_jig;
}

void exynos_set_sicd_block_jig(bool block)
{
	pm_info->sicd_block_jig = block;
}

extern int muic_get_jig_state(void);

static int is_jig_connected(void)
{
	if (pm_info->sicd_block_jig && muic_get_jig_state()) {
		cpuidle_stat_collect_jig_block();
		return true;
	}
...
static int __init dt_init(void)
{
	...
	if (of_property_read_u32(np, "sicd_block_jig", &pm_info->sicd_block_jig))
		pr_warn("No matching property: sicd_block_jig\n");
