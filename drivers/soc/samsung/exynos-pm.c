...
#if defined(CONFIG_SLEEP_HISOTRY) || defined(CONFIG_SLEEP_MONITOR) || defined(CONFIG_ENERGY_MONITOR)
	...
	for (i = 0, size = 8; i < pm_info->num_eint; i += size) {
		...
		for_each_set_bit(bit, &ext_int_pend, size) {
			...
			if (strncmp(desc->action->name, "s2mpw01-irq", 11) &&
				strncmp(desc->action->name, "bcmsdh_sdmmc", 12))
...
#if defined(CONFIG_SLEEP_HISOTRY) || defined(CONFIG_SLEEP_MONITOR) || defined(CONFIG_ENERGY_MONITOR)
{
			if (desc && desc->action && desc->action->name) {
				if (!strncmp(desc->action->name, "s2mpw01-irq", 11)) {
					g_check_pmic_int = 1;
				} else if(strncmp(desc->action->name, "bcmsdh_sdmmc", 12)) {
					g_check_pmic_int = 0;
#ifdef CONFIG_PM_SLEEP_HISTORY
...
#endif
#ifdef CONFIG_ENERGY_MONITOR
					energy_monitor_record_wakeup_reason(irq, NULL);
#endif
				}
			} else {
#ifdef CONFIG_SLEEP_MONITOR
...
#endif
#ifdef CONFIG_SLEEP_MONITOR
		add_slp_mon_irq_list(0, "CP");
#endif
#ifdef CONFIG_ENERGY_MONITOR
		energy_monitor_record_wakeup_reason(0, "CP");
#endif
	}else
...
static void exynos_pm_syscore_resume(void)
{
	...
	if (exynos_ss_get_enable("base") == true) {
