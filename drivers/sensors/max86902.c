...
static int max86900_regulator_onoff(struct max86900_device_data *data, int onoff)
{
	...
	if (onoff == HRM_LDO_ON) {
		rc = regulator_set_voltage(data->vdd_1p8, 1800000, 1800000);
		if (rc < 0) {
		...
		rc = regulator_set_voltage(data->led_3p3, 3300000, 3300000);
		if (rc < 0) {
		...
		rc = regulator_enable(data->led_3p3);
		if (rc) {
		...
		rc = regulator_disable(data->led_3p3);
		if (rc) {
	...
	struct device *dev)
{
	...
	data->vdd_1p8 = devm_regulator_get(data->dev, "vdd_1p8");
	if (IS_ERR(data->vdd_1p8)) {
		pr_err("%s - get vdd_1p8 error\n", __func__);
		return PTR_ERR(data->vdd_1p8);
	}

	data->led_3p3 = devm_regulator_get(data->dev, "led_3p3");
	if (IS_ERR(data->led_3p3)) {
		pr_err("%s - get led_3p3 error\n", __func__);
		return PTR_ERR(data->led_3p3);
	}
...
static int max86900_setup_irq(struct max86900_device_data *data)
{
	...
	errorno = request_threaded_irq(data->irq, NULL,
		max86900_irq_handler, IRQF_TRIGGER_LOW|IRQF_ONESHOT,
		"hrm_sensor_irq", data);
...
static int __init max86902_init(void)
{
	return i2c_add_driver(&max86900_i2c_driver);
}

static void __exit max86902_exit(void)
{
	i2c_del_driver(&max86900_i2c_driver);
}

module_init(max86902_init);
module_exit(max86902_exit);
