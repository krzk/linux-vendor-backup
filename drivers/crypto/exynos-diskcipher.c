...
static int fmp_setkey(struct crypto_tfm *tfm, u8 *in_key, u32 keylen,
		      bool persistent)
{
	struct fmp_crypto_info *ci = crypto_tfm_ctx(tfm);

	return exynos_fmp_setkey(ci, in_key, keylen, persistent);
}
