...
#if defined(CONFIG_SOC_EXYNOS5420) && defined(CONFIG_PM_RUNTIME)
#define ACE_USE_RUNTIME_PM
#endif
...
#if defined(ACE_USE_RUNTIME_PM)
		exynos_pm_interrupt_runtime_put_sync(s5p_ace_dev.dev);
#endif
...
static void s5p_ace_clock_gating(int status)
{
	...
	if (status == ACE_CLOCK_ON) {
		...
		if (tmp == 0) {
#if defined(ACE_USE_RUNTIME_PM)
			exynos_pm_interrupt_runtime_get_sync(s5p_ace_dev.dev);
#endif
