...
#include <linux/onedram_svn/onedram.h>
...
struct wake_lock onedram_wake_lock;
mm_segment_t old_fs;
struct file *filp_for_dump;
static int cp_upload_cmd_rcvd = 0;
...
//static void onedram_cp_dump( struct work_struct *unused );

//static DECLARE_WORK( onedram_cp_dump_work, onedram_cp_dump );
...
static irqreturn_t onedram_irq_handler(int irq, void *data)
{
	...
	if ( dump_on ){
		if( ( mailbox == 0x0ADD0ADD ) || ( mailbox == 0xFADEFADE ) ) { // CP uploaded Data into Dpram.
...
static int onedram_phone_ramdump_on( struct onedram *od )
{
	u16 irq_mb = 0;
	...
	od->reg->sem = 0;
	irq_mb = MB_CMD(MB_CMD_RAMDUMP_MODE_ON);
	onedram_write_mailbox(irq_mb);

	dprintk( "Send interrupt Cmd : 0x%X\n", irq_mb );

	dprintk( "Uart switch to Modem.\n" );
	gpio_set_value( 126, 0); //OMAP_GPIO_UART_SEL

	return 0;
}

static int onedram_phone_ramdump_off( struct onedram *od )
{
	u16 irq_mb = 0;
	...
	od->reg->sem = 0;
	irq_mb = MB_CMD(MB_CMD_RAMDUMP_MODE_OFF);
	onedram_write_mailbox(irq_mb);
...
//static void onedram_cp_dump( struct work_struct *unused )
static void onedram_cp_dump( struct onedram *od )
{	
	void * onedram_vbase;
	u32 send_mail;
	u32 read_mail;
	...
	char buf_for_dump[ 2048 ];
	unsigned long flags;
	int r;
	
	int loop_count = 1;
	
	//wake_lock(&onedram_wake_lock);
	...
	gpio_set_value(OMAP_GPIO_CON_CP_SEL, 0);

	filp_for_dump = filp_open( "/data/dump_for_cp", O_CREAT | O_TRUNC | O_WRONLY, 0666 );
	if( IS_ERR( filp_for_dump ) )
		dprintk( "Can't create /data/dump_for_cp file\n" );

	od->reg->sem = 0;
	send_mail = 0xDEADDEAD;
	//od->reg->mailbox_BA = send_mail;
	onedram_write_mailbox(send_mail);
	dprintk("Send 0x%X, semaphore = %d\n", send_mail, onedram_read_sem());
	
	// DUMP MODE
	while( 1 ) {
		while( !cp_upload_cmd_rcvd ){
			msleep(10);
		}
		onedram_read_mailbox(&read_mail);
		
		if( ( read_mail == 0x0ADD0ADD ) || ( read_mail == 0xFADEFADE ) ) { // CP uploaded Data into Dpram.
			cp_upload_cmd_rcvd = 1;
		...
		onedram_read_mailbox(&read_mail);
		if( read_mail == 0xFADEFADE ) // Copy All ram Data.
			break;
	
		od->reg->sem = 0;
		send_mail = 0xEDEDEDED; // Copy Data from Dpram.
		onedram_write_mailbox(send_mail);
	...
	//wake_unlock( &onedram_wake_lock );
	dump_done = 1;
	
	// Send uevnet for Ril reset
	//kobject_uevent_env(&si->svndev->dev.kobj, KOBJ_OFFLINE, NULL);
}

static int go_dump( struct onedram *od )
{
	dump_done = 0;
	dump_on = 1;

	onedram_cp_dump(od);
	//schedule_work(&onedram_cp_dump_work);
		...
		unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case ONEDRAM_PHONE_RAMDUMP_ON:
		//dprintk("Start cp_dump schedule_work 1 \n");
		go_dump(od);
		return 0;
	case ONEDRAM_PHONE_RAMDUMP_MODE_ON:
		onedram_phone_ramdump_on(od);
		return 0;
	case ONEDRAM_PHONE_RAMDUMP_MODE_OFF:
		onedram_phone_ramdump_off(od);
		return 0;
