...
static void _read_err(struct sipc *si);
...
//	dev_dbg(&si->svndev->dev, "Command: %x\n", cmd);
	...
	case MBC_PHONE_START:
		onedram_write_mailbox(MB_CMD(MBC_INIT_START));
...
void sipc_handler(u32 mailbox, void *data)
{
	...
	if( ( mailbox == 0x0ADD0ADD ) || ( mailbox == 0xFADEFADE ) ) {
		return;
	}
#if defined(CONFIG_KERNEL_DEBUG_SEC)
...
static void _read_err(struct sipc *si)
{
	...
	remove_invalid_string(cp_errmsg);
	
	printk("CP Dump Cause - %s\n", cp_errmsg);
	
	skb = netdev_alloc_skb(ndev, sizeof(cp_errmsg)
				+ sizeof(struct phonethdr));	
	if (unlikely(!skb))
		return -ENOMEM;	
	skb_reserve(skb, sizeof(struct phonethdr));
	
	p_skb = skb_put(skb, errmsg_len);
	memcpy(p_skb, cp_errmsg, errmsg_len);

	_phonet_rx(ndev, skb, PN_ERR);	
}
