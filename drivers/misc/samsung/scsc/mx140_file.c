...
#define MX140_HW_UNKNOWN (0xffffffff)
...
static u32 rf_hw_ver = MX140_HW_UNKNOWN;
module_param(rf_hw_ver, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(rf_hw_ver, "mx140 RF hw variant, 0xffffffff for unknown");
...
int mx140_file_download_fw(struct scsc_mx *mx, void *dest, size_t dest_size, u32 *fw_image_size)
{
	...
	if (fw_suffix_found != -1 && !manual)
		memcpy(firmware_hw_ver, fw_suffixes[fw_suffix_found].suffix, sizeof(fw_suffixes[0].suffix));
...
int mx140_file_select_fw(struct scsc_mx *mx, u32 hw_ver)
{
	...
	rf_hw_ver = hw_ver; /* Save RF CHIP_VERSION in module param for info */
