...
static u32 platform_mif_irq_bit_mask_status_get(struct scsc_mif_abs *interface)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Getting INTMSR0: 0x%x\n", val);
	return val;
}
...
static u32 platform_mif_irq_get(struct scsc_mif_abs *interface)
{
	...
	val = platform_mif_reg_read(platform, MAILBOX_WLBT_REG(INTSR0)) >> 16;
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Getting INT-INTSR0: 0x%x\n", val);
