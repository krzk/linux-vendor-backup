...
static void slsi_sm_bt_service_cleanup_interrupts(void)
{
	...
	scsc_service_mifintrbit_bit_clear(bt_service.service, int_src);
	scsc_service_mifintrbit_bit_unmask(bt_service.service, int_src);
...
static int slsi_sm_bt_service_cleanup_stop_service(void)
{
	...
	/* Stop service first, then it's safe to release shared memory
	   resources */
	SCSC_TAG_DEBUG(BT_COMMON, "stopping Bluetooth service\n");
...
static int slsi_bt_service_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "  read_wake_lock_count          = %zu\n",
		bt_service.read_wake_lock_count);
	seq_printf(m, "  read_wake_unlock_count        = %zu\n",
		bt_service.read_wake_unlock_count);
