...
static int ice4_irda_parse_dt(struct device *dev)
{
	...
	data->fpga_regulator = devm_regulator_get(dev, "fpga");
...
static void work_function_firmware_download(struct work_struct *work)
{
	...
	ice4_irda_firmware_download(&data->client->dev,
			fpga_irda_fw, ARRAY_SIZE(fpga_irda_fw));
		...
		const struct i2c_device_id *id)
{
	...
	data->sys_dev = sec_device_create(data, "sec_ir");
...
err_sysfs_create:
	sec_device_destroy(data->sys_dev->devt);
