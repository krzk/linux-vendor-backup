...
static int sgl_init_lock(struct sgl_session_data *session_data, void __user *arg)
{
	struct sgl_attribute attr;
	struct sgl_lock *lock;
	int err = 0;
...
static int sgl_set_data(struct sgl_session_data *session_data, void __user *arg)
{
	struct sgl_user_data usr_data;
	struct sgl_lock *lock;
	int ret = 0;
	...
	SGL_LOG("key: %d", usr_data.key);

	mutex_lock(&sgl_global.mutex);

	lock = sgl_get_lock(sgl_global.locks, usr_data.key);
	if (lock == NULL) {
		mutex_unlock(&sgl_global.mutex);
		SGL_WARN("lock is not in the inited locks");
		return -ENOENT;
	...
	mutex_lock(&lock->data_mutex);
	lock->user_data1 = usr_data.data1;
	lock->user_data2 = usr_data.data2;
	usr_data.locked = lock->locked;
	mutex_unlock(&lock->data_mutex);
	mutex_unlock(&sgl_global.mutex);
...
static int sgl_get_data(struct sgl_session_data *session_data, void __user *arg)
{
	struct sgl_user_data usr_data;
	struct sgl_lock *lock;
	int ret = 0;
	...
	mutex_lock(&lock->data_mutex);
	usr_data.data1 = lock->user_data1;
	usr_data.data2 = lock->user_data2;
	usr_data.locked = lock->locked;
	mutex_unlock(&lock->data_mutex);
	mutex_unlock(&sgl_global.mutex);
...
#ifdef CONFIG_SGL_SUPPORT_FOTA
static int sgl_query_magic(struct sgl_session_data *session_data, void __user *arg)
{
	struct sgl_magic_cookie magic_cookie;

	magic_cookie.key = TGL_MAGIC_KEY;

	if (copy_to_user(arg, &magic_cookie, sizeof(magic_cookie)))
		return -EFAULT;
...
#endif
{
	...
	case SGL_IOC_INIT_LOCK:
		...
		err = sgl_init_lock(session_data, (void __user *)arg);
		break;
	...
	case SGL_IOC_SET_DATA:
		err = sgl_set_data(session_data, (void __user *)arg);
		break;
	case SGL_IOC_GET_DATA:
		err = sgl_get_data(session_data, (void __user *)arg);
		break;
	case SGL_IOC_DUMP_LOCKS:
		sgl_dump_locks();
		break;
#ifdef CONFIG_SGL_SUPPORT_FOTA
	case SGL_IOC_QUERY_MAGIC:
		err = sgl_query_magic(session_data, (void __user *)arg);
		break;
#endif
	default:
