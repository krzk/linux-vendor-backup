...
static int ss222_get_cp_status(struct modem_ctl *mc)
{
	int cp_on = gpio_get_value(mc->gpio_cp_on);
	int cp_off = gpio_get_value(mc->gpio_cp_off);
	int cp_reset  = gpio_get_value(mc->gpio_cp_reset);
...
static int ss222_set_ap_status(struct modem_ctl *mc)
{
	int cp_on = gpio_get_value(mc->gpio_cp_on);
	int cp_off = gpio_get_value(mc->gpio_cp_off);
	int cp_reset  = gpio_get_value(mc->gpio_cp_reset);
...
static int ss222_clear_ap_status(struct modem_ctl *mc)
{
	int cp_on = gpio_get_value(mc->gpio_cp_on);
	int cp_off = gpio_get_value(mc->gpio_cp_off);
	int cp_reset  = gpio_get_value(mc->gpio_cp_reset);
...
static void ss222_get_ops(struct modem_ctl *mc)
{
	...
	mc->ops.set_ap_status = ss222_set_ap_status;
	mc->ops.clear_ap_status = ss222_clear_ap_status;
	mc->ops.get_cp_status = ss222_get_cp_status;
}
...
int ss222_init_modemctl_device(struct modem_ctl *mc, struct modem_data *pdata)
{
	...
	mc->irq_cp_status = pdata->irq_cp_status;
	if (!mc->irq_cp_status) {
	...
	irq = mc->irq_cp_status;
	flag = IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND;
	ret = request_irq(irq, cp2ap_status_handler, flag, "umts_status", mc);
	if (ret) {
		mif_err("ERR! request_irq(#%d) fail (err %d)\n", irq, ret);
		mif_err("xxx\n");
		return ret;
