...
#ifdef SAMSUNG_RTC_DURATION	//TI Patch start: Lewis Byun --> if you can't understand the procedure, send a mail at lewis.byun@ti.com
#define SAMSUNG_RTC_DEFAULT_YEAR_CUSTOMIZATION	70

struct rtc_time init_rtc_setting_val = //Please change this structure for setting your initial time.
{
	.tm_sec = 0,
	.tm_min = 0,
	.tm_hour = 0,
	.tm_mday = 1,
	.tm_mon = 0,
	.tm_year = 100,		//+1900 is the real year
};
#endif //SAMSUNG_RTC_DURATION
...
#else
	tm->tm_year = bcd2bin(rtc_data[5]) + SAMSUNG_RTC_DEFAULT_YEAR_CUSTOMIZATION;
#endif
	//TI Patch End
#ifdef DEBUG_MESSAGE
	printk("setting system clock to "
		"%d-%02d-%02d %02d:%02d:%02d twl_rtc_read_time--\n",
...
#ifdef DEBUG_MESSAGE
	printk("setting system clock to "
		"%d-%02d-%02d %02d:%02d:%02d twl_rtc_set_time++\n",
		tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday,
		tm->tm_hour, tm->tm_min, tm->tm_sec);
#endif

#ifdef SAMSUNG_RTC_DURATION
	err = rtc_valid_tm(tm);

	if (err != 0)
	{
		printk("twl_rtc_set_time: ERROR --> not valid time to set");
		ret = EINVAL;
		goto out;
	}
...
#else
	rtc_data[6] = bin2bcd(tm->tm_year - SAMSUNG_RTC_DEFAULT_YEAR_CUSTOMIZATION);
#endif
	//TI Patch End
...
#else
	alm->time.tm_year = bcd2bin(rtc_data[5]) + SAMSUNG_RTC_DEFAULT_YEAR_CUSTOMIZATION;
#endif
		//TI Patch End
...
#else
	alarm_data[6] = bin2bcd(alm->time.tm_year - SAMSUNG_RTC_DEFAULT_YEAR_CUSTOMIZATION);
#endif
	//TI Patch End
...
#ifdef SAMSUNG_RTC_DURATION
		twl_rtc_set_time(NULL, &init_rtc_setting_val);
#endif
	}
...
#if ( defined( CONFIG_MACH_SAMSUNG_LATONA ) && defined(CONFIG_CHN_KERNEL_STE_LATONA) && \
      ( CONFIG_SAMSUNG_REL_HW_REV == 1 ) )
	/* backup batery charge - configuration 3v, 25uA */
	//twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, 0x14, 0x12 /*BB_CFG*/);
#endif
...
#if ( defined( CONFIG_MACH_SAMSUNG_LATONA ) && defined(CONFIG_CHN_KERNEL_STE_LATONA) && \
      ( CONFIG_SAMSUNG_REL_HW_REV == 1 ) )
	/* backup batery charge - configuration 3v, 150uA */
	//twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, 0x15, 0x12 /*BB_CFG*/);
#endif
