...
struct workqueue_struct *virtual_rtc_wq;
struct delayed_work virtual_rtc_work;
bool boot_completed = false;

static void virtual_rtc_work_handler( struct work_struct *work )
{
	printk("%s, boot completed.\n", __func__);
	boot_completed = true;
}
#endif
...
//	kobject_uevent(&dev->kobj, KOBJ_CHANGE);
	...
	if(boot_completed)
		...
		printk("%s, don't send time to cp (ignored)\n", __func__);
#else
	kobject_uevent_env(&dev->kobj, KOBJ_CHANGE, envp);
#endif
...
#if defined(BOOT_TIME)
	virtual_rtc_wq = create_singlethread_workqueue("virtual_rtc_workq");
	INIT_DELAYED_WORK( &virtual_rtc_work, virtual_rtc_work_handler );	
	queue_delayed_work( virtual_rtc_wq, &virtual_rtc_work, BOOT_TIME * HZ );
#endif
