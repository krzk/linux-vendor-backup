...
static void check_all_raw_cap(struct synaptics_drv_data *data)
{
	int i, j, k=0;
	u16 temp = 0;
	u16 length = data->x_line * (data->y_line+2) * 2;
...
static void synaptics_fw_phone(struct synaptics_drv_data *data,
	u8 *buf)
{
	sprintf(buf, "%c%c%.2d%.4d", data->firm_version[0], data->firm_version[1], data->firm_version[2], data->firm_version[3]);
	...
	u8 *buf)
{
	...
	sprintf(buf, "%c%c%.2d%.4d", buf[0], buf[1], buf[2], buf[3]);
	...
	u32 pos_x, u32 pos_y)
{
	...
	case REPORT_TYPE_RAW_CAP:
	{
		u16 position = (u16)((data->y_line+2) * pos_x) + pos_y;
		position *= 2;
