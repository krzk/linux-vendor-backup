...
void enable_autocal_timer(unsigned int value)
{
	...
	{
		set_autocal(value);
		mod_timer(&autocal_timer, jiffies + (10 * HZ));
	}
}
...
/*
    ...
    */
	...
	if(is_boot_state) {
		...
		set_autocal(15); // holding 3 seconds on touch point makes calibration automatically
    ...
    Atmel_model_type model_type )
{
    switch(model_type)
    {
        case ARCHER_KOR_SK:
        {
            ...
            #ifdef CONFIG_MACH_ARCHER
            #if (CONFIG_ARCHER_HW_REV == ARCHER_REV12) || (CONFIG_ARCHER_HW_REV == ARCHER_REV13)
        	power_config->idleacqint = 48;
    ...
    Atmel_model_type model_type )
{
    switch(model_type)
    {
		...
		case ARIES:
		{
			acquisition_config->chrgtime = 7;//8;
			acquisition_config->tchdrift = 5;
			acquisition_config->driftst = 0;
			acquisition_config->atchcalst = 9;
			acquisition_config->atchcalsthr = 35;//15;
		}
		break;
		case LATONA:
		{
...
//            touchscreen_config->blen = 49;
            ...
            #endif
            #endif
            touchscreen_config->numtouch= 5;// 2->5 kmj_DB16
        }
        break;
        case OSCAR:
        {
			...
			touchscreen_config->movfilter = 31;
        ...
        case ARIES:
        {
			...
			touchscreen_config->orientate = 1;
			touchscreen_config->tchthr = 40;//35;
			touchscreen_config->movhysti = 3;
			...
			touchscreen_config->xedgectrl = 143;
			touchscreen_config->xedgedist = 40;//50;
			touchscreen_config->yedgectrl = 143;
			touchscreen_config->yedgedist = 80;
			touchscreen_config->jumplimit = 18;
        }
        ...
        case LATONA:
        {
			...
			touchscreen_config->mrgthr = 40;//10;
			touchscreen_config->jumplimit = 18;
#if defined(CONFIG_MACH_SAMSUNG_LATONA) && defined(CONFIG_SAMSUNG_HW_EMU_BOARD)
			touchscreen_config->xsize = 17; 
			touchscreen_config->ysize = 12; 
			touchscreen_config->xrange = 479;
			touchscreen_config->yrange = 319;
#endif
        }
        break;
    }    
    ...
    Atmel_model_type model_type )
{
    switch(model_type)
    {
		...
		gripfacesuppression_config->ylogrip = 8;
		...
		gripfacesuppression_config->szthr2 = 40;
		gripfacesuppression_config->shpthr1 = 4;
		gripfacesuppression_config->shpthr2 = 35;//15;
		gripfacesuppression_config->supextto = 10;	
        }
        break;
		case LATONA:
    ...
    Atmel_model_type model_type )
{
    switch(model_type)
    {
        case ARCHER_KOR_SK:
        {
            ...
            #ifdef CONFIG_MACH_ARCHER
            #if (CONFIG_ARCHER_HW_REV >= ARCHER_REV13)       
            cte_config->mode = 2;
            cte_config->idlegcafdepth = 4;
