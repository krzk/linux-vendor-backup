...
static irqreturn_t cyttsp5_irq(int irq, void *handle)
{
	...
	if (cd->trustedUI_mode && cd->device_enabled) {
		dev_dbg(cd->dev,
			"%s: trustedUI_mode is enabed.\n", __func__);
		goto out;
	}
#endif
...
static int cyttsp5_reset_and_wait(struct cyttsp5_core_data *cd)
{
	...
	if (cd->hw_power_state == true) {
		dev_info(cd->dev, "%s: cd->sysinfo.ready=[%d]\n", __func__, cd->sysinfo.ready);
		rc = cyttsp5_reset(cd);
		if (rc < 0)
			goto error;
	} else {
...
#define SDF_PANEL_ID		0x13
#define WC1_S_MAIN_REV04	0x06
#define WC1_S_MAIN_REV10	0x07
#define WC1_S_MAIN_REV11	0x08
#define WC1_S_MAIN_REV13	0x0a
...
static int cyttsp5_check_silicon_id(struct cyttsp5_core_data *cd)
{
	...
	if ((cd->silicon_id) == CSP_SILICON_ID) {
		...
		if (system_rev >= WC1_S_MAIN_REV13) {
			cd->fw_path = CY_CSP31F_FW_FILE_NAME;
		} else if (system_rev >= WC1_S_MAIN_REV10) {
			cd->fw_path = CY_CSP30_FW_FILE_NAME;
		} else if ((system_rev >= WC1_S_MAIN_REV04) && \
			((get_panel_id() & 0xff) == SDF_PANEL_ID)) {
			cd->fw_path = CY_CSP22_FW_FILE_NAME;
		} else {
			cd->fw_path = CY_CSP37_FW_FILE_NAME;
		}
#else
	...
	} else if ((cd->silicon_id) == BGA_SILICON_ID) {
		cd->fw_path = CY_BGA_FW_FILE_NAME;
	} else {
...
int cyttsp5_core_ambient_on(struct device *dev)
{
	...
	if (device_may_wakeup(dev)) {
		...
		dev_err(dev, "%s Device can't wakeup\n", __func__);
...
#ifdef CONFIG_TRUSTONIC_TRUSTED_UI_NOTIFY
extern void cyttsp5_mt_run_close(struct device *dev);
extern int cyttsp5_irq_stat(struct cyttsp5_core_platform_data *pdata,
		struct device *dev);

int cyttsp5_secure_clear_event(struct cyttsp5_core_data *cd)
{
	int irq_state, rc, retry = 5;

	do {
		irq_state = cyttsp5_irq_stat(cd->cpdata, cd->dev);
		retry--;
		if (!irq_state) {
			dev_dbg(cd->dev, "%s: irq gpio is low.\n", __func__);

			rc = cyttsp5_adap_read_default_nosize(cd, cd->input_buf, CY_MAX_INPUT);
			if (rc) {
				dev_err(cd->dev, "%s: Error getting report, r=%d\n",
						__func__, rc);
				return rc;
			}
			msleep(10);
		}
	} while(!irq_state && retry);

	return 0;
}

int cyttsp5_secure_enable(struct cyttsp5_core_data *cd)
{
	cd->trustedUI_mode = true;
	disable_irq(cd->irq);

	return 0;
}

int cyttsp5_secure_disable(struct cyttsp5_core_data *cd)
{

	if (cyttsp5_secure_clear_event(cd))
		dev_err(cd->dev, "%s: failed to clear event.\n", __func__);

	cd->trustedUI_mode = false;
	enable_irq(cd->irq);

	return 0;
}

int cyttsp5_secure_notify(struct notifier_block *this, unsigned long cmd, void *_data)
{
	struct cyttsp5_core_data *cd = container_of(this,
		struct cyttsp5_core_data, nb_ctrl);
	int ret;

	dev_info(cd->dev, "%s:cmd[0x%x]\n", __func__, (int)cmd);

	if (!cd) {
		dev_err(cd->dev, "%s:cd is NULL.\n", __func__);
		return 0;
	}
	...
	case TRUSTEDUI_MODE_CHANGE: {
		...
		dev_info(cd->dev, "%s:mode[0x%x]\n", __func__, mode);
		switch (mode) {
		case TRUSTEDUI_MODE_OFF:
			ret = cyttsp5_secure_disable(cd);
			if (ret)
				dev_err(cd->dev, "%s: failed to disable secure mode.\n", __func__);
			else
				dev_info(cd->dev, "%s:disabled trustedUI_mode\n", __func__);
			break;
		case TRUSTEDUI_MODE_INPUT_SECURED:
		case TRUSTEDUI_MODE_ALL:
			ret = cyttsp5_secure_enable(cd);
			if (ret)
				dev_err(cd->dev, "%s: failed to enable secure mode.\n", __func__);
			else
				dev_info(cd->dev, "%s:enabled trustedUI_mode\n", __func__);
			break;
		default:
			dev_err(cd->dev, "%s: unsupported mode[0x%x]\n", __func__, mode);
			break;
		}
	}
		break;
	default:
		dev_err(cd->dev, "%s: unsupported cmd[0x%x]\n", __func__, (int)cmd);
		break;
	}
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	rc = devm_request_threaded_irq(dev, irq, NULL, cyttsp5_irq, irq_flags,
		SEC_TSP_NAME, cd);
	if (rc < 0) {
	...
	if (!cd->fw_path) {
		rc = cyttsp5_check_silicon_id(cd);
		if (rc < 0)
			dev_err(cd->dev, "%s: Failed to get silicon_id r=%d\n",
				__func__, rc);
	}
...
#ifdef CONFIG_TRUSTONIC_TRUSTED_UI_NOTIFY
	if (trustedui_nb_register(&cd->nb_ctrl)) {
		dev_err(dev, "%s: could not register trustedui notify callback\n", __func__);
		goto error_trustedui_nb_register;
	} else
		cd->nb_ctrl.notifier_call = cyttsp5_secure_notify;
#endif
#ifdef CONFIG_SLEEP_MONITOR
...
#ifdef CONFIG_TRUSTONIC_TRUSTED_UI_NOTIFY
	trustedui_nb_unregister(&cd->nb_ctrl);
#endif
#ifdef CONFIG_SLEEP_MONITOR
	sleep_monitor_unregister_ops(SLEEP_MONITOR_TSP);
#endif
#ifdef CONFIG_TRUSTONIC_TRUSTED_UI_NOTIFY
error_trustedui_nb_register:
#endif
...
error_request_irq:
	if (cd->cpdata->power)
		cd->cpdata->power(cd->cpdata, 0, dev, 0);
