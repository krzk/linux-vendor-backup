...
static int cyttsp5_hw_power(struct device *dev, int onoff)
{
	...
	dev_dbg(dev, "%s called with [%s]\n", __func__, onoff ? "ON":"OFF");

	if (!vddo_vreg) {
		vddo_vreg = regulator_get(NULL, "tsp_vdd_1.8v");
		if (IS_ERR(vddo_vreg)) {
			vddo_vreg = NULL;
			dev_err(dev, "%s: failed to get tsp_vdd_1.8v regulator\n", __func__);
			return -ENODEV;
		}
	}

	if (!avdd_vreg) {
		avdd_vreg = regulator_get(NULL, "tsp_vdd_3.3v");
		if (IS_ERR(avdd_vreg)) {
			avdd_vreg = NULL;
			dev_err(dev, "%s: failed to get tsp_vdd_3.3v regulator\n", __func__);
			return -ENODEV;
		}
		ret = regulator_set_voltage(avdd_vreg, 3300000, 3300000);
		if (ret) {
			dev_err(dev, "%s: unable to set voltage for tsp_vdd_3.3v, %d\n",
				__func__, ret);
			return ret;
		}
	}
	...
	if (onoff) {
		if (regulator_is_enabled(avdd_vreg))
			dev_err(dev, "%s: avdd_vreg already enabled\n", __func__);
		else {
			ret = regulator_enable(avdd_vreg);
			if (ret) {
				dev_err(dev, "%s: unable to enable avdd_vreg, %d\n",
					__func__, ret);
				return ret;
			}
		}
		...
		if (regulator_is_enabled(vddo_vreg))
			dev_err(dev, "%s: vddo_vreg already enabled\n", __func__);
		else {
			ret = regulator_enable(vddo_vreg);
			if (ret) {
				dev_err(dev, "%s: unable to enable vddo_vreg, %d\n",
					__func__, ret);
				return ret;
			}
		}
		...
		if (!regulator_is_enabled(vddo_vreg))
			dev_err(dev, "%s: vddo_vreg already disabled\n", __func__);
		else {
			ret = regulator_disable(vddo_vreg);
			if (ret) {
				dev_err(dev, "%s: unable to disable vddo_vreg, %d\n",
					__func__, ret);
				return ret;
			}
		}
		...
		if (!regulator_is_enabled(avdd_vreg))
			dev_err(dev, "%s: avdd_vreg already disabled\n", __func__);
		else {
			ret = regulator_disable(avdd_vreg);
			if (ret) {
				dev_err(dev, "%s: unable to disable avdd_vreg, %d\n",
					__func__, ret);
				return ret;
			}
		}
