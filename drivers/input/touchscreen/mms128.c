				  ...
				  const struct i2c_device_id *id)
{
	...
	info = devm_kzalloc(&client->dev, sizeof(struct mms_ts_info),
			GFP_KERNEL);
	if (!info) {
	...
	info->regulator_pwr = devm_regulator_get(&client->dev, "avdd");
	if (IS_ERR(info->regulator_pwr)) {
		ret = PTR_ERR(info->regulator_pwr);
		dev_err(&client->dev, "Unable to get the power regulator\n");
		return ret;
	}

	info->regulator_vdd = regulator_get(&client->dev, "vdd");
	...
	ret = devm_request_threaded_irq(&client->dev, client->irq, NULL,
			mms_ts_interrupt, IRQF_TRIGGER_FALLING  | IRQF_ONESHOT,
			MELFAS_TS_NAME, info);
	if (ret < 0) {
