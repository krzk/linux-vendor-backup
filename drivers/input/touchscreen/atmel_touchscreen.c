 ...
 *
 */
#ifdef CONFIG_ARCHER_TARGET_SK // kmj_da17
 #define FEATURE_MOVE_LIMIT //KMJ_db06
// #define TOUCH_DEVICE // KMJ_db06
#define TOUCH_PROC // KMJ_db08
#endif
...
#define MAX_TOUCH_X_RESOLUTION	320
#define MAX_TOUCH_Y_RESOLUTION	480
int atmel_ts_tk_keycode[] = {}; // dummy for build
#else
...
void (*atmel_handler_functions[MODEL_TYPE_MAX])(struct work_struct *work) =
{
    ...
    read_func_for_multi_touch, // OSCAR
    read_func_for_multi_touch,        // HALO	
    read_func_for_multi_touch, // ARIES
...
void clear_touch_history(void)
{
	...
	for(i=0;i<MAX_TOUCH_NUM;i++)
	{
		...
		if(touch_info[i].press > 0)
		{
			...
			REPORT( touch_info[i].press, touch_info[i].size, touch_info[i].x, touch_info[i].y );
		}
...
void handle_keyarray(uint8_t * atmel_msg)
{
    ...
    {
        case ARCHER_KOR_SK:
#if defined(CONFIG_MACH_SAMSUNG_LATONA) && defined(CONFIG_SAMSUNG_HW_REL_BOARD)
        case LATONA:
#endif
        {
...
#if defined(CONFIG_ARCHER_TARGET_SK) || defined(CONFIG_MACH_OSCAR) || defined(CONFIG_MACH_HALO) || defined(CONFIG_MACH_ARIES) || defined(CONFIG_MACH_SAMSUNG_LATONA)
        // kmj_cl23.enable_tsp
#if 0 //removed for Aries
        if (gpio_is_valid(OMAP_GPIO_TOUCH_EN)) {
            if (gpio_request(OMAP_GPIO_TOUCH_EN, "touch")<0) // ryun 20091218 
                printk(KERN_ERR "Filed to request OMAP_GPIO_TOUCH_EN!\n");
            gpio_direction_output(OMAP_GPIO_TOUCH_EN, 0);
        }
#endif   
