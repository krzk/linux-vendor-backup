...
#if defined(CONFIG_DEBUG_FS) && defined(CONFIG_MACH_BLINK)
#include <linux/debugfs.h>
struct dentry *svcled_debugfs_dir;
#define SVCLED_BUF_SIZE PAGE_SIZE
#endif

#if defined(CONFIG_MACH_BLINK)
extern  unsigned int system_rev;
#define CROSS_RED_GREEN_REV 0x1

static bool is_cross_rev(void){
	if(CROSS_RED_GREEN_REV == system_rev)
		return true;
	else
		return false;
}

static void change_red_green_value(u8 *red_value, u8 *green_value){
	u8 temp=0;
	temp = *red_value;
...
#if defined(CONFIG_MACH_BLINK)
	/*It's work around for stranger device rev*/
	if(true == is_cross_rev())
		change_red_green_value(&led_r_brightness,&led_g_brightness);
#endif
...
#if defined(CONFIG_DEBUG_FS)&& defined(CONFIG_MACH_BLINK)
static int svcled_debug_open(struct inode *inode, struct file *filp)
{
	filp->private_data = inode->i_private;
	return 0;
}

static ssize_t svcled_debug_read(struct file *filp,
	char __user *buffer, size_t count, loff_t *ppos)
{
	...
	buf = kzalloc(SVCLED_BUF_SIZE, GFP_KERNEL);
	if (!buf)
		return -ENOMEM;

	for(sub_address = 0x01; sub_address < 0x15; sub_address+=0x01){
		ledon_buffer = i2c_smbus_read_byte_data(b_client,sub_address);
		len+= snprintf(buf+len,SVCLED_BUF_SIZE-len,
			"[svc_led :%x ] = %x\n",sub_address,ledon_buffer);
	}
	ret = simple_read_from_buffer(buffer, len, ppos, buf, SVCLED_BUF_SIZE);
	kfree(buf);

	return ret;
}

static const struct file_operations svcled_debugfs_fops = {
	.owner = THIS_MODULE,
	.open =  svcled_debug_open,
	.read = svcled_debug_read,
};
#endif
...
#if defined(CONFIG_DEBUG_FS) && defined(CONFIG_MACH_BLINK)
	{
		svcled_debugfs_dir = kzalloc(sizeof(struct dentry), GFP_KERNEL);
		if (!svcled_debugfs_dir){
			ret = -ENOMEM;
			goto exit;
		}

		svcled_debugfs_dir =
			debugfs_create_dir("svcled_debug", NULL);
		if (svcled_debugfs_dir) {
			if (!debugfs_create_file("svcled_reg", 0644,
				svcled_debugfs_dir,
				NULL, &svcled_debugfs_fops))
