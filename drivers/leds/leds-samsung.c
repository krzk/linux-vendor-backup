...
struct sec_led_data {
	...
	unsigned gpio1;
	unsigned gpio2;
	unsigned gpio3; // temp for hw rev 0.4
//	struct work_struct work;
	u8 new_level;
	u8 can_sleep1;
	u8 can_sleep2;
	u8 can_sleep3; // temp for hw rev 0.4
//	u8 active_low;
	...
	enum led_brightness value)
{
	...
	{
		gpio_set_value(led_dat->gpio3, 0); // temp for hw rev 0.4
	...
	{
		gpio_set_value(led_dat->gpio3, 1); // temp for hw rev 0.4
...
{	
	...
	for (i = 0; i < pdata->num_leds; i++) {
		...
		ret = gpio_request(OMAP_GPIO_LED_EN1_REV04, cur_led->name); // temp for hw rev 0.4
		ret = gpio_request(OMAP_GPIO_LED_EN1, cur_led->name);
		ret = gpio_request(OMAP_GPIO_LED_EN2, cur_led->name);
		if (ret < 0)
			goto err;

		led_dat->gpio3 = OMAP_GPIO_LED_EN1_REV04; // temp for hw rev 0.4
		led_dat->gpio1 = OMAP_GPIO_LED_EN1;
		led_dat->gpio2 = OMAP_GPIO_LED_EN2;

		led_dat->can_sleep3 = gpio_cansleep(OMAP_GPIO_LED_EN1_REV04); // temp for hw rev 0.4
		led_dat->can_sleep1 = gpio_cansleep(OMAP_GPIO_LED_EN1);
		led_dat->can_sleep2 = gpio_cansleep(OMAP_GPIO_LED_EN2);

		gpio_direction_output(OMAP_GPIO_LED_EN1_REV04, 0); // temp for hw rev 0.4
		...
		if (ret < 0) {
			gpio_free(led_dat->gpio3);
			gpio_free(led_dat->gpio1);
			gpio_free(led_dat->gpio2);
			goto err;
		}
	}
...
//			cancel_work_sync(&leds_data[i].work);
			gpio_free(leds_data[i].gpio3); // temp for hw rev 0.4
			gpio_free(leds_data[i].gpio1);
			gpio_free(leds_data[i].gpio2);
		}
	}
