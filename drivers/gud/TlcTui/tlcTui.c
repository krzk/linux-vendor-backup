...
static bool tlc_open_driver(void)
{
	...
	mc_ret = mc_malloc_wsm(DEVICE_ID, 0, sizeof(struct tui_dci_msg_t),
			(uint8_t **)&dci, 0);
	if (MC_DRV_OK != mc_ret) {
		pr_err("ERROR %s: Allocation of DCI WSM failed: %d\n",
			 __func__, mc_ret);
		return false;
	}
	...
	if (MC_DRV_OK != mc_ret) {
		pr_err("ERROR %s: Open driver session failed: %d\n",
			 __func__, mc_ret);
		ret = false;
	} else {
		ret = true;
...
static bool tlc_open(void)
{
	...
	if (MC_DRV_OK != mc_ret && MC_DRV_ERR_INVALID_OPERATION != mc_ret) {
		pr_err("ERROR %s: Error %d opening device\n", __func__,
			 mc_ret);
		return false;
	}
...
static void tlc_wait_cmd_from_driver(void)
{
	...
	if (MC_DRV_OK == ret)
		...
		pr_err("ERROR %s: mc_wait_notification() failed: %d\n",
			 __func__, ret);
}
...
static uint32_t send_cmd_to_user(uint32_t command_id)
{
	...
	if (g_user_rsp.id != command_id) {
		pr_err("ERROR %s: Wrong response id 0x%08x iso 0x%08x\n",
...
static void tlc_process_cmd(void)
{
	...
	if (NULL == dci) {
		pr_err("ERROR %s: DCI has not been set up properly - exiting"\
			 "\n", __func__);
		return;
	} else {
	...
	if (CMD_TUI_SW_NONE == command_id) {
		pr_err("ERROR %s: Notified without command\n", __func__);
		return;
	} else {
		if (dci->nwd_rsp.id != CMD_TUI_SW_NONE)
			pr_warn("%s: Warning, previous response not ack\n",
				 __func__);
	}
	...
	default:
		pr_err("ERROR %s: Unknown command %d\n",
			 __func__, command_id);
		break;
	}
	...
	if (MC_DRV_OK != ret)
		pr_err("ERROR %s: Notify failed: %d\n", __func__, ret);
}
...
static void tlc_close_driver(void)
{
	...
	if (MC_DRV_OK != ret) {
		pr_err("ERROR %s: Closing driver session failed: %d\n",
			 __func__, ret);
	}
}
...
static void tlc_close(void)
{
	...
	if (MC_DRV_OK != ret) {
		pr_err("ERROR %s: Closing tbase device failed: %d\n",
			 __func__, ret);
	}
}
...
bool tlc_notify_event(uint32_t event_type)
{
	...
	if (NULL == dci) {
		pr_err("ERROR tlc_notify_event: DCI has not been set up "\
			 "properly - exiting\n");
		return false;
	}

	/* Prepare notification message in DCI */
	pr_info("tlc_notify_event: event_type = %d\n", event_type);
	...
	if (MC_DRV_OK != result) {
		pr_err("ERROR tlc_notify_event: mc_notify failed: %d\n",
			 result);
		ret = false;
	} else {
		ret = true;
...
int main_thread(void *uarg)
{
	...
	if (!tlc_open()) {
		pr_err("ERROR main_thread: open driver failed!\n");
		return 1;
	}
...
int tlc_wait_cmd(uint32_t *cmd_id)
{
	...
	if (wait_for_completion_interruptible(&dci_comp)) {
		pr_debug("%s: interrupted by system\n", __func__);
		return -ERESTARTSYS;
	}
