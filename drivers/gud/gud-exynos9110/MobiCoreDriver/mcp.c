		   ...
		   struct mc_uuid_t *uuid)
{
	...
	if (uuid) {
		...
		mc_dev_err("  %-15s= 0x%s session_id =0x%x cmd_id=%d", "cmd_info.uuid", cmd_info->uuid_str,in_session_id, cmd_id);
	} else {
		cmd_info->uuid_str[0] = '\0';
		mc_dev_err(" session_id =0x%x cmd_id=%d", in_session_id, cmd_id );
	}
		     ...
		     const struct mcp_buffer_map *tci_map)
{
	...
	mc_dev_err("open session %x ret %d state %d",
...
int mcp_close_session(struct mcp_session *session)
{
	...
	mutex_unlock(&l_ctx.sessions_lock);
	mc_dev_err("close session %x ret %d state %d",
		     session->sid, ret, session->state);
	return ret;
}
...
int mcp_notify(struct mcp_session *session, u32 payload)
{
	if (session->sid == SID_MCP)
		mc_dev_err("notify MCP payload=%d" ,payload);
	else
		mc_dev_err("notify session %x payload=%d", session->sid,payload);
