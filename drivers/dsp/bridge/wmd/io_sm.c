...
// TI patch start: non-line fetch abort
#define CM_FCLKEN_PER_LOCAL 0x48005000
#define CM_ICLKEN_PER_LOCAL 0x48005010
#define CM_IDLEST_PER_LOCAL 0x48005020
#define CM_AUTOIDLE_PER_LOCAL 0x48005030
...
// TI patch start: non-line fetch abort
		save_autoidle = omap_readl(CM_AUTOIDLE_PER_LOCAL);	//read autoidle setting

		PREVENT_NONLINE_FETCH_ABORT_START(CM_AUTOIDLE_PER_LOCAL, CM_ICLKEN_PER_LOCAL, CM_FCLKEN_PER_LOCAL, 0x1000);

		if(omap_readl(CM_IDLEST_PER_LOCAL) & 0x1000)	//0 --> can't be accessed
		{
			msleep(1);
			//printk("Clock not enbled..............\n");
			PREVENT_NONLINE_FETCH_ABORT_START(CM_AUTOIDLE_PER_LOCAL, CM_ICLKEN_PER_LOCAL, CM_FCLKEN_PER_LOCAL, 0x1000);
			msleep(1);
		}
// TI patch end: non-line fetch abort
...
// TI patch start: non-line fetch abort
		if(save_autoidle & 0x1000)	//autoidle is used
		{
			//printk("save_autoidle = %x---------------------------------..............\n", save_autoidle);
			PREVENT_NONLINE_FETCH_ABORT_END(CM_AUTOIDLE_PER_LOCAL, 0x1000);
		}
// TI patch end: non-line fetch abort
