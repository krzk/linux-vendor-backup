...
int node_create(struct node_object *hnode)
{
	...
	// Changes for DSP MMU fault recovery CSR - OMAPS00226408
	/* OMAPS00226408: MMU fault recovery Patch - start  */
	if (DSP_FAILED(status) && hnode->loaded) {
		hnode_mgr->nldr_fxns.pfn_unload(hnode->nldr_node_obj,NLDR_CREATE);
		hnode->loaded = false;
	}
	/* OMAPS00226408: MMU fault recovery Patch - end  */	
