...
void slsi_rx_connect_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	SLSI_NET_DBG1(dev, SLSI_MLME, "mlme_connect_ind(vif:%d, result:%d)\n",
		      fapi_get_vif(skb),
		      fapi_get_u16(skb, u.mlme_connect_ind.result_code));
	...
	if (fapi_get_u16(skb, u.mlme_connect_ind.result_code) != FAPI_RESULTCODE_SUCCESS) {
		status = WLAN_STATUS_UNSPECIFIED_FAILURE;
	} else {
		peer = slsi_get_peer_from_qs(sdev, dev, SLSI_STA_PEER_QUEUESET);

		if(!peer || !peer->assoc_ie) {
			if(!WARN(!peer, "peer context absent")) {
				...
				cfg80211_connect_result(dev, peer->address, NULL, 0, NULL, 0, WLAN_STATUS_UNSPECIFIED_FAILURE, GFP_KERNEL);
			}
			goto exit_with_vif;
		} else {
			if (peer->assoc_ie) {
	...
	WARN_ON(!ndev_vif->sta.mlme_scan_ind_skb);
