...
cont:
	...
	/* TODO: Remove the check when MSDU issue is solved */
	if ((fapi_get_sigid(skb) == MA_UNITDATA_IND) &&
	    (fapi_get_u16(skb, u.ma_unitdata_ind.data_unit_descriptor) == FAPI_DATAUNITDESCRIPTOR_AMSDU)) {
		if (bytes_to_alloc < 256) {
			SLSI_ERR_NODEV("incorrect FAPI_DATAUNITDESCRIPTOR_AMSDU size %zu < 256\n", bytes_to_alloc);
			print_hex_dump(KERN_ERR, "skb ", DUMP_PREFIX_NONE, 16, 1, skb->data, skb->len, 0);
			kfree_skb(skb);
			return NULL;
		}
	}
