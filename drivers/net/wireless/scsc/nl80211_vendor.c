...
void slsi_gscan_scan_res_dump(struct slsi_nl_scan_result_param *nl_scan_res)
{
...
struct slsi_gscan_result *slsi_prepare_scan_result(struct sk_buff *skb)
{
	...
	ie_len = fapi_get_datalen(skb) - (ie - (u8 *)mgmt);

	mem_reqd = (sizeof(struct slsi_gscan_result) - 1) + ie_len; /* Exclude 1 byte for ie_data[1] */
...
#ifdef CONFIG_SCSC_WLAN_DEBUG
	slsi_gscan_scan_res_dump(&scan_res->nl_scan_res);
#endif
...
void slsi_gscan_handle_scan_result(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb, u16 scan_id, bool scan_done)
{
	...
	scan_res = slsi_prepare_scan_result(skb);
	if (scan_res == NULL) {
		SLSI_NET_ERR(dev, "Failed to prepare scan result\n");
		goto out;
	}

	/* Check for hotlist AP */
