...
int slsi_start(struct slsi_dev *sdev)
{
	...
	SLSI_EC_GOTO(slsi_mib_download_file(sdev, sdev->mib_file_name, true), err, err_done);
#endif
...
#define SLSI_SCREEN_OFF_FILTERS_COUNT 3
#else
#define SLSI_SCREEN_OFF_FILTERS_COUNT 5
#endif
...
static int slsi_set_common_packet_filters(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	u8 mdns6_mask[6] = { 0x33, 0x33, 0x00, 0x00, 0x00, 0xFB };
#endif

	struct slsi_mlme_pattern_desc pattern_desc;
	struct slsi_mlme_pkt_filter_elem pkt_filter_elem[SLSI_SCREEN_OFF_FILTERS_COUNT];
	...
	SLSI_ETHER_COPY(pattern_desc.pattern, mdns_mask);

	slsi_create_packet_filter_element(SLSI_DNS_FILTER_ID,
					  FAPI_PACKETFILTERMODE_OPT_IN_SLEEP | FAPI_PACKETFILTERMODE_OPT_IN,
					  1, &pattern_desc, &pkt_filter_elem[num_filters], &pkt_filters_len);
	num_filters++;

	/*Opt out all broadcast*/
	pattern_desc.offset = 0;
	pattern_desc.mask_length = ETH_ALEN;
	SLSI_ETHER_COPY(pattern_desc.mask, addr_mask);
	SLSI_ETHER_COPY(pattern_desc.pattern, addr_mask);

	slsi_create_packet_filter_element(SLSI_ALL_BC_FILTER_ID,
...
#ifndef CONFIG_SCSC_WLAN_BLOCK_IPV6
	/*set the IPV6 specific packet filters*/

	/*opt in mDns*/
	pattern_desc.offset = 0;
	pattern_desc.mask_length = ETH_ALEN;
	SLSI_ETHER_COPY(pattern_desc.mask, addr_mask);
	SLSI_ETHER_COPY(pattern_desc.pattern, mdns6_mask);

	slsi_create_packet_filter_element(SLSI_DNS6_FILTER_ID,
					  FAPI_PACKETFILTERMODE_OPT_IN_SLEEP | FAPI_PACKETFILTERMODE_OPT_IN,
					  1, &pattern_desc, &pkt_filter_elem[num_filters], &pkt_filters_len);
	num_filters++;

	/*Opt out all IPv6 multicast */
	pattern_desc.offset = 0;
	pattern_desc.mask_length = 2;                                       /* prefix len*/
	memcpy(pattern_desc.mask, addr_mask, pattern_desc.mask_length);
	memcpy(pattern_desc.pattern, mdns6_mask, pattern_desc.mask_length); /*prefix for multicast is same as that for mDns*/

	slsi_create_packet_filter_element(SLSI_ALL_IPv6_MC_FILTER_ID,
					  FAPI_PACKETFILTERMODE_OPT_OUT_SLEEP | FAPI_PACKETFILTERMODE_OPT_OUT,
					  1, &pattern_desc, &pkt_filter_elem[num_filters], &pkt_filters_len);
	num_filters++;
#endif
...
static int  slsi_clear_packet_filters(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	slsi_create_packet_filter_element(SLSI_DNS_FILTER_ID, 0, 0, NULL, &pkt_filter_elem[num_filters], &pkt_filters_len);
	num_filters++;
	slsi_create_packet_filter_element(SLSI_ALL_BC_FILTER_ID, 0, 0, NULL, &pkt_filter_elem[num_filters], &pkt_filters_len);
	num_filters++;
	slsi_create_packet_filter_element(SLSI_ALL_IPv4_MC_FILTER_ID, 0, 0, NULL, &pkt_filter_elem[num_filters], &pkt_filters_len);
	num_filters++;
#ifndef CONFIG_SCSC_WLAN_BLOCK_IPV6
	slsi_create_packet_filter_element(SLSI_DNS6_FILTER_ID, 0, 0, NULL, &pkt_filter_elem[num_filters], &pkt_filters_len);
	num_filters++;
	slsi_create_packet_filter_element(SLSI_ALL_IPv6_MC_FILTER_ID, 0, 0, NULL, &pkt_filter_elem[num_filters], &pkt_filters_len);
	num_filters++;
#endif
