...
static inline bool is_channel_active(struct ieee80211_channel *channel)
{
	/* Ignore disabled channels, radar channels are ALWAYS passive */
	if ((channel->flags & IEEE80211_CHAN_DISABLED) || (channel->flags & IEEE80211_CHAN_RADAR))
		return false;

#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 14, 0)
	if (!(channel->flags & IEEE80211_CHAN_PASSIVE_SCAN))
		return true;
#else
