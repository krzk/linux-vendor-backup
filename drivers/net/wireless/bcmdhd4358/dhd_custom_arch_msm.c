...
int GPIO_WL_REG_ON = 991; // TRLTE_SKT_with_BCM4358
#ifdef CONFIG_BCMDHD_PCIE
...
#if 0
static struct class *brcm_class;
static atomic_t brcm_dev;
static struct device *wlan_dev;

static int dhd_wlan_class_create(void)
{
        printk(KERN_ERR "%s: Enter \n", __FUNCTION__);

        brcm_class = class_create(THIS_MODULE, "brcm");
        if (IS_ERR(brcm_class)) {
                pr_err("Failed to create class(sec) %ld\n", PTR_ERR(brcm_class));
                return PTR_ERR(brcm_class);
        }
        return 0;
}

static struct device *dhd_wlan_device_create(void *drvdata, const char *fmt)
{
        struct device *dev;
        ...
        if (IS_ERR(brcm_class)) {
                pr_err("Failed to create class(brcm) %ld\n", PTR_ERR(brcm_class));
                BUG();
        }

        if (!brcm_class) {
                pr_err("Not yet created class(brcm)!\n");
                BUG();
        }

        dev = device_create(brcm_class, NULL, atomic_inc_return(&brcm_dev),
                        drvdata, fmt);
        if (IS_ERR(dev))
...
#if 0	
	wlan_dev = dhd_wlan_device_create(NULL, "brcm-wlan");
	
	if(!wlan_dev)
	{
		printk("%s Failed to wlan device create \n", __FUNCTION__);
		return -ENODEV;
	}
#endif	
...
int brcm_wlan_power(int onoff)
{
	...
	if (onoff) 
	{
		...
		printk(KERN_ERR "%s: WL_REG_ON Off skip because not yot saveresore routine \n", __FUNCTION__);
#if 0
...
static struct resource dhd_wlan_resources[] = {
	[0] = {
...
static struct wifi_platform_data dhd_wlan_control = {
...
static struct platform_device dhd_device_wlan = {
	.name		= "bcmdhd_wlan",
	.id		= 1,
	.num_resources	= ARRAY_SIZE(dhd_wlan_resources),
	.resource	= dhd_wlan_resources,
	.dev		= {
		.platform_data = &dhd_wlan_control,
	},
};
EXPORT_SYMBOL(dhd_device_wlan);
...
int __init brcm_wlan_init(void)
{
	...
	return platform_device_register(&dhd_device_wlan);
}
