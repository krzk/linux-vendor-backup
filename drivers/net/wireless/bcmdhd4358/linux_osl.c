...
#if defined(CONFIG_64BIT)
extern struct dma_pool *dhd_dma_pool;
void*
osl_dma_alloc_consistent_for_ring_create(osl_t *osh, uint size, uint16 align_bits, uint *alloced, dmaaddr_t *pap)
{
	void *va;
	uint16 align = (1 << align_bits);
	ASSERT((osh && (osh->magic == OS_HANDLE_MAGIC)));
	...
	*alloced = size;
...
#else
	{
		...
		va =  dma_pool_alloc(dhd_dma_pool, GFP_ATOMIC, &pap_lin);
