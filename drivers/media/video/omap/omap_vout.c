...
int flg_720  = 0;
#define DROP_FRAMES_COUNT 1   //RSZ_720
			...
			struct v4l2_format *f)
{
	...
	{
		vout->use_isp_rsz_for_downscale = 1;   //VIK_SIM_720p
		flg_720 = 1;
	}
	else
	{
		vout->use_isp_rsz_for_downscale = 0;         //VIK_SIM_720p
		flg_720 = 0;
	}
#endif 
	ovid = &vout->vid_info;
			...
			struct v4l2_buffer *buffer)
{
	...
	if(stop_vsync_disable)
	{
		qbuf_aftr_vsync_disable = buffer->index;
		return ret;
	}
			...
			struct v4l2_buffer *b)
{
	...
	if(stop_vsync_disable || qbuf_aftr_vsync_disable != -1)
	{
		b->index = qbuf_aftr_vsync_disable;
		qbuf_aftr_vsync_disable = -1;
		return ret;
	}		
			...
			enum v4l2_buf_type i)
{
	...
	printk(KERN_ERR "SFF: Skip count reset\n");
	nDropCount = DROP_FRAMES_COUNT;
	return 0;
}
		...
		int vid_num)
{
	...
	if (static_vrfb_allocation) {
		printk(KERN_INFO VOUT_NAME ": static_vrfb_allocation !!!!!\n");
