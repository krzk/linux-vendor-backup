...
static int vd0_cnt = 50;
module_param(vd0_cnt, int, 0644);

static int vd0_cnt_720p = 100;
module_param(vd0_cnt_720p, int, 0644);
			   ...
			   int sensor_index)
{
	...
	if(pipe->ccdc_out_h >= JPEG_CAPTURE_HEIGHT) {
		// JPEG Capture 
		isp_reg_writel(dev,(((pipe->ccdc_out_h - 2) &
						ISPCCDC_VDINT_0_MASK) <<
					ISPCCDC_VDINT_0_SHIFT) |
				((pipe->ccdc_out_h/2 & ISPCCDC_VDINT_1_MASK) <<
				 ISPCCDC_VDINT_1_SHIFT),
				OMAP3_ISP_IOMEM_CCDC,
				ISPCCDC_VDINT);
	} else {
		// Preview 
		if (pipe->ccdc_out_h == 144 && pipe->ccdc_out_w == 176) vd0_interrupt_h = 40;
		else if (pipe->ccdc_out_h == 176 && pipe->ccdc_out_w == 144) vd0_interrupt_h = 40;
		else if (pipe->ccdc_out_h == 480 && pipe->ccdc_out_w == 640 && sensor_index == 2) vd0_interrupt_h = 90;
		else if (pipe->ccdc_out_h == 720 && pipe->ccdc_out_w == 1280) vd0_interrupt_h = 100; 
		else if (pipe->ccdc_out_h == 240) vd0_interrupt_h = 10;
		else if (pipe->ccdc_out_h == 480) vd0_interrupt_h = 50;
		else vd0_interrupt_h = 50;
		isp_reg_writel(dev,(((pipe->ccdc_out_h - vd0_interrupt_h) &
						ISPCCDC_VDINT_0_MASK) << 
					ISPCCDC_VDINT_0_SHIFT) |
