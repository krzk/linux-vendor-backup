...
static int print_ccdc_busy = 0;
module_param(print_ccdc_busy , int, 0644);

static int isp_shadow = 0;
module_param(isp_shadow , int, 0644);
...
int cnt_vd0 = 0;
atomic_t skip_buffer;

void isp_set_skip_buffer(struct device *dev, int skip_buffer_on) 
{
	if (skip_buffer_on) {
		printk("SKIP BUFF ON\n");
		atomic_set(&skip_buffer, ISP_SKIP_BUFF_ON);
	} else {
		printk("SKIP BUFF OFF\n");
		atomic_set(&skip_buffer, ISP_SKIP_BUFF_OFF);
	}
	return;
}
EXPORT_SYMBOL(isp_set_skip_buffer);

void isp_init_vd0_cnt(struct device *dev) 
{
	cnt_vd0 = 0;
	return;
}

EXPORT_SYMBOL(isp_init_vd0_cnt);

int isp_get_vd0_cnt(struct device *dev) 
{
	return cnt_vd0;
}

EXPORT_SYMBOL(isp_get_vd0_cnt);
...
static irqreturn_t isp_isr(int irq, void *_pdev)
{
	...
	/*
	if (irqstatus & CCDC_VD1 || (irqstatus & CCDC_VD0)) {
		if (CCDC_CAPTURE(isp)) {
			if (atomic_read(&skip_buffer)) {
				//if (irqstatus & CCDC_VD1) printk("[ISP] ignore VD1 \n");
				//if (irqstatus & CCDC_VD0) printk("[ISP] ignore VD0 \n");
				goto out_ignore_buff;
			}
		}
	}
...
void isp_set_vd0_done(struct device *dev, int vd0) 
{
	if(dev == NULL)
		dev = &omap3isp_pdev->dev;
	...
	{    
		printk("%s Clear Capture Done\n", __func__);
	}    

	return;
}

EXPORT_SYMBOL(isp_set_vd0_done);
...
static void isp_buf_process(struct device *dev, struct isp_bufs *bufs)
{
	...
	if (!last) {
		...
		if ((sensor_index == 2) || 
				(isp->pipeline.out_pix.width == 144) || 
				(isp->pipeline.out_pix.width == 176)) {
			if( !ispccdc_busy(&isp->isp_ccdc) ) {
				isp_set_buf(dev, ISP_BUF_NEXT_DONE(bufs));
			} else {
				while(ispccdc_busy(&isp->isp_ccdc)) {
//					if (print_ccdc_busy) {
//						if (ispccdc_busy(&isp->isp_ccdc)) printk("ccdc is busy\n");
//					}
				}
				isp_set_buf(dev, ISP_BUF_NEXT_DONE(bufs));
			}
...
//			}
		...
		if (CCDC_CAPTURE(isp)) {
			//printk("[ISP:%s] CCDC Enable\n", __func__);
			ispccdc_enable(&isp->isp_ccdc, 1);
		}
	} else {
		/* Tell ISP not to write any of our buffers. */
		if (CCDC_CAPTURE(isp)) {
			//printk("[ISP:%s] CCDC CAPTURE\n", __func__);
			isp_disable_interrupts(dev);
		}
#ifndef ZEUS_CAM	// not required to comment, but just for test purpose			
		if (RAW_CAPTURE(isp))
