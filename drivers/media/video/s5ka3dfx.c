  ...
  else
  {
    switch(sensor->preview_size)
    {
      ...
      default:
		...
		for (i = 0; i < 5; i++) {
			  if(s5ka3dfx_set_data(client, reg_check_dataline_start[i]))
				goto init_fail;
		}	
...
static int s5ka3dfx_check_dataline_stop()
{
	...
	for (i = 0; i < 2; i++) {
		err = s5ka3dfx_i2c_write(client, sizeof(reg_check_dataline_stop[i]), reg_check_dataline_stop[i]);
		if (err < 0)
		{
...
static int ioctl_try_fmt_cap(struct v4l2_int_device *s, struct v4l2_format *f)
{
  ...
  if(sensor->state == S5KA3DFX_STATE_CAPTURE)
  {
	...
	s5ka3dfx_set_flip(S5KA3DFX_FLIP_MIRROR);
...
extern void force_disable_tsp_autocal();
