...
static void jpeg_hx_device_enc_run(void *priv)
{
	...
	jpeg_hx_coef(dev->reg_base, ENCODING);
...
static void jpeg_hx_device_dec_run(void *priv)
{
	...
	jpeg_hx_coef(dev->reg_base, DECODING);
	jpeg_hx_set_timer(dev->reg_base, 0x10000000);
	jpeg_hx_start(dev->reg_base);
...
static int jpeg_hx_probe(struct platform_device *pdev)
{
	...
	/* clock */
	if(!pdev->id)
		jpeg->clk = clk_get(&pdev->dev, "jpeg-hx");
	else
		jpeg->clk = clk_get(&pdev->dev, "jpeg2-hx");
	...
	if (soc_is_exynos5410())
		jpeg->sclk_clk = clk_get(&pdev->dev, "dout_aclk_300_jpeg");
	else if (soc_is_exynos5420())
		jpeg->sclk_clk = clk_get(&pdev->dev, "aclk_300_jpeg_dout");
