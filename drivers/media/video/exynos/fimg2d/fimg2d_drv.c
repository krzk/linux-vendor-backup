...
static int fimg2d_do_bitblt(struct fimg2d_control *ctrl)
{
	...
	if (fimg2d_ip_version_is() >= IP_VER_G2D_5AR) {
		exynos_pm_interrupt_runtime_get_sync(ctrl->dev);
		fimg2d_debug("Done pm_interrupt_runtime_get_sync()\n");
	} else {
	...
	if (fimg2d_ip_version_is() >= IP_VER_G2D_5AR) {
		exynos_pm_interrupt_runtime_put_sync(ctrl->dev);
		fimg2d_debug("Done pm_interrupt_runtime_put_sync()\n");
	} else {
...
#elif defined CONFIG_ARM_EXYNOS5420_BUS_DEVFREQ
		/* mif lock : 667MHz */
		pm_qos_update_request(&exynos5_g2d_mif_qos, 667000);
		/* int lock : 333MHz */
		pm_qos_update_request(&exynos5_g2d_int_qos, 333000);
#endif
#ifdef CONFIG_ARM_EXYNOS_IKS_CPUFREQ
		/* cpu lock : KFC 800MHz */
...
static int fimg2d_release(struct inode *inode, struct file *file)
{
	...
	if (!count) {
		...
		pm_qos_update_request(&exynos5_g2d_mif_qos, 0);
		/* int unlock */
		pm_qos_update_request(&exynos5_g2d_int_qos, 0);
#endif
#ifdef CONFIG_ARM_EXYNOS_IKS_CPUFREQ
...
static int fimg2d_probe(struct platform_device *pdev)
{
	...
	pm_qos_add_request(&exynos5_g2d_mif_qos,
				PM_QOS_BUS_THROUGHPUT, 0);
	pm_qos_add_request(&exynos5_g2d_int_qos,
