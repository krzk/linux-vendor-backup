...
int vpu_dec_parse_vfg(struct nx_vpu_ctx *ctx)
{
	...
	buf->vb.v4l2_buf.field = ctx->codec.dec.interlace_flg;
	buf->vb.v4l2_buf.flags = ctx->codec.dec.frame_buf_delay;
...
int vpu_dec_decode_slice(struct nx_vpu_ctx *ctx)
{
	...
	if (dec_ctx->eos_tag == 0) {
		...
		timestamp.tv_sec = buf->vb.v4l2_buf.timestamp.tv_usec;
	...
	if (ctx->strm_queue_cnt > 0) {
		...
		buf->vb.v4l2_buf.field = ctx->codec.dec.interlace_flg;
		buf->vb.v4l2_buf.bytesused = ctx->strm_size;
		buf->vb.v4l2_buf.flags = ctx->codec.dec.frm_type;
		buf->vb.v4l2_buf.reserved = ctx->codec.dec.reliable_0_100;
