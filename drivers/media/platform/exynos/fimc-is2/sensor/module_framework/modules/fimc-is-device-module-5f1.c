...
#define VT_PIX_CLK_FREQ_MHZ (99.66666667)
#define LINE_LENGTH_PCK (2616)

#define SENSOR_SCENARIO_SECURE      6

bool is_final_cam_module_iris  = true;
bool is_iris_ver_read = true;
bool is_iris_mtf_test_check = true;
...
static int sensor_5f1_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	/* sensor reset */
	for (i = 0; i < ARRAY_SIZE(setfile_vision_5f1_reset); i++) {
	...
	/* sensor open clock */
	for (i = 0; i < ARRAY_SIZE(setfile_vision_5f1_open_clock); i++) {
	...
	/* read sensor version */
	if (!is_iris_ver_read) {
		ret = fimc_is_sensor_read16(client, 0x0000, &module_ID);
		pr_info("%s(moduleID:0x%x)\n", __func__, module_ID);

		ret = fimc_is_sensor_read16(client, 0x0002, &revision);
		pr_info("%s(revision:0x%x)\n", __func__, revision);

		ret = fimc_is_sensor_read8(client, 0x000D, &feature_ID1);
		ret = fimc_is_sensor_read8(client, 0x000E, &feature_ID2);
		pr_info("%s(feature_ID:0x%x%x)\n", __func__, feature_ID1, feature_ID2);

		/* read year */
		ret = fimc_is_sensor_read8(client, 0x0012, &is_iris_mtf_read_data[0]);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto exit;
		}
		pr_info(" %s(year:0x%x)\n", __func__, is_iris_mtf_read_data[0]);

		/* read month */
		ret = fimc_is_sensor_read8(client, 0x0013, &is_iris_mtf_read_data[1]);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto exit;
		}
		pr_info("%s(month:0x%x)\n", __func__, is_iris_mtf_read_data[1]);

		/* read company */
		ret = fimc_is_sensor_read8(client, 0x0019, &is_iris_mtf_read_data[2]);
		if (ret) {
			err("[%s][%d] i2c transfer failed", __func__, __LINE__);
			ret = -EINVAL;
			goto exit;
		...
		pr_info("%s(company:0x%x)\n", __func__, is_iris_mtf_read_data[2]);
	}
...
int sensor_5f1_stream_on(struct v4l2_subdev *subdev)
{
	...
	ret = fimc_is_sensor_write16(client, SENSOR_REG_STREAM_ON_1, 0x0100);
	if (ret < 0) {
...
int sensor_5f1_stream_off(struct v4l2_subdev *subdev)
{
	...
	ret = fimc_is_sensor_write16(client, SENSOR_REG_STREAM_ON_1, 0x0000);
	if (ret < 0) {
		err("fimc_is_sensor_write8 is fail(%d)", ret);
		goto p_err;
	}
...
int sensor_5f1_s_duration(struct v4l2_subdev *subdev, u64 duration)
{
	...
	if (duration == SENSOR_FPS_30) {
		for (i = 0; i < ARRAY_SIZE(setfile_vision_5f1_fps_30_26MHz); i++) {
...
int sensor_5f1_s_shutterspeed(struct v4l2_subdev *subdev, u64 shutterspeed)
{
	...
	calc_value = (((shutterspeed*((u16)VT_PIX_CLK_FREQ_MHZ)*4*1000))/LINE_LENGTH_PCK);
