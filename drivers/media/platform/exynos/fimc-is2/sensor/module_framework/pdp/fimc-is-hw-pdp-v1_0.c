...
void pdp_hw_g_pdp_reg(void __iomem *base_reg, struct pdp_read_reg_setting_t *reg_param)
{
	...
	for (i = 0; i < reg_param->buf_size; i++)
		*(reg_param->buf_array + i) = readl(reg_param->addr_array + i);
}
...
void pdp_hw_s_paf_mpd_bin(u32 __iomem *base_reg, u32 h, u32 v)
{
			...
			struct pdp_main_wininfo *winInfo)
{
	...
	fimc_is_hw_set_reg(base_reg, &pdp_regs[PDP_R_PAF_I_MWM_SX], winInfo->start_y);
	fimc_is_hw_set_reg(base_reg, &pdp_regs[PDP_R_PAF_I_MWM_EX], winInfo->end_x);
	fimc_is_hw_set_reg(base_reg, &pdp_regs[PDP_R_PAF_I_MWM_EX], winInfo->end_y);
}
