...
int fimc_is_vender_probe(struct fimc_is_vender *vender)
{
	...
	priv->iris_sensor_index = iris_sensor_index;
...
int fimc_is_vender_dt(struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "iris_sensor_index", &iris_sensor_index);
	if (ret)
		probe_err("iris_sensor_index read is fail(%d)", ret);
