...
static int fimc_is_hw_mcsc_load_setfile(struct fimc_is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	hw_mcsc->setfile[sensor_position] =
		vzalloc(setfile->table[0].size * setfile->using_count);
	...
	for (setfile_index = 0; setfile_index < setfile->using_count; setfile_index++) {
		...
		if (setfile_addr->setfile_version != MCSC_SETFILE_VERSION) {
			...
			vfree(hw_mcsc->setfile[sensor_position]);
			return -EINVAL;
		}
	}
	...
	ulong hw_map)
{
	...
	if (hw_mcsc->setfile[sensor_position])
		vfree(hw_mcsc->setfile[sensor_position]);
	hw_mcsc->setfile[sensor_position] = NULL;
	...
	int done_type)
{
	...
	if (test_bit(ENTRY_M5P, &done_frame->out_flag)) {
		ret = fimc_is_hardware_frame_done(hw_ip, frame,
	...
	u32 output_id, u32 instance)
{
	...
	fimc_is_scaler_set_wdma_10bit_type(hw_ip->regs, output_id, format, bitwidth);
		...
		u32 instance)
{
	...
	for (output_id = MCSC_OUTPUT0; output_id < cap->max_output; output_id++) {
		...
		if (test_bit(output_id, &hw_mcsc->out_en)) {
			...
			sdbg_hw(2, "crop size changed (%dx%d) -> (%dx%d) by DJAG\n", hw_ip,
				param->output[output_id].crop_width,
				param->output[output_id].crop_height,
				ALIGN(out_width - param->output[output_id].crop_offset_x * 2, 2),
				ALIGN(out_height - param->output[output_id].crop_offset_y * 2, 2));
