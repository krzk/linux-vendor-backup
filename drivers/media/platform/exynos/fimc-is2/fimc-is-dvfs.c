	...
	struct fimc_is_frame *frame)
{
	 ...
	 */
	if (dual_info->max_fps_master >= 30 && dual_info->max_fps_slave == 0)
		dual_info->mode = FIMC_IS_DUAL_MODE_BYPASS;
	else if (dual_info->max_fps_master >= 30 && dual_info->max_fps_slave >= 30)
		dual_info->mode = FIMC_IS_DUAL_MODE_SYNC;
	else if (dual_info->max_fps_master <= 5 && dual_info->max_fps_slave >= 30)
