	...
	struct param_mcs_output *output, u32 output_id, u32 instance)
{
...
#ifdef USE_MCSC_STRIP_OUT_CROP
	use_out_crop = output->use_out_crop;
	...
	if (use_out_crop) {
		...
		src_height = input->djag_out_height ? input->djag_out_height : input->height;
	} else
#endif
	{
	...
	if (use_out_crop) {
		if (is_scaler_get_djag_strip_enable(hw_ip->regs[REG_SETA], output_id)){
			is_scaler_get_djag_strip_config(hw_ip->regs[REG_SETA], output_id, &temp_stripe_pre_dst_x, &temp_stripe_start_pos_x);
		} else {
		...
		if (!post_en) {
			otcrop_pos_x = output->crop_offset_x;
			otcrop_pos_y = output->crop_offset_y;
			otcrop_width = output->crop_width;
			otcrop_height = output->crop_height;
			is_scaler_set_poly_out_crop_enable(hw_ip->regs[REG_SETA], output_id, 1);
			is_scaler_set_poly_out_crop_size(hw_ip->regs[REG_SETA], output_id, otcrop_pos_x, otcrop_pos_y, otcrop_width, otcrop_height);
	...
	struct param_mcs_output *output, u32 output_id, u32 instance)
{
	...
	otcrop_width = output->crop_width;
	otcrop_height = output->crop_height;
	is_scaler_set_post_out_crop_size(hw_ip->regs[REG_SETA], output_id, otcrop_pos_x, otcrop_pos_y, otcrop_width, otcrop_height);
	if (!poly_otcrop_en && use_out_crop) {
		if (is_scaler_get_poly_strip_enable(hw_ip->regs[REG_SETA], output_id)){
			is_scaler_get_poly_strip_config(hw_ip->regs[REG_SETA], output_id, &temp_stripe_pre_dst_x, &temp_stripe_start_pos_x);
		} else {
			temp_stripe_start_pos_x = output->stripe_in_start_pos_x;
		}
		temp_stripe_pre_dst_x = ALIGN_DOWN(GET_SCALED_SIZE(temp_stripe_start_pos_x, hratio) , MCSC_WIDTH_ALIGN);
