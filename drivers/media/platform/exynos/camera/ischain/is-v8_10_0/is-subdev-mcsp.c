		...
		u32 *use_out_crop)
{
	...
	u32 dma_offset = 0;
	int temp_stripe_x = 0, temp_stripe_w = 0, temp_roi_stripe_w = 0;
	u32 stripe_start_x = 0, stripe_end_x = 0, stripe_roi_start_x = 0, stripe_roi_end_x = 0, temp_offset = 0;
	u32 stripe_start_y = 0,  stripe_roi_start_y = 0, stripe_end_y = 0, stripe_roi_end_y = 0;
	u32 h_ratio = GET_ZOOM_RATIO(incrop->w, otcrop->w);
	u32 v_ratio = GET_ZOOM_RATIO(incrop->h, otcrop->h);
	...
	if (frame) {
		...
		if (!ldr_frame->stripe_info.region_id) {
			...
			if (*use_out_crop) {
				...
				stripe_w += STRIPE_MARGIN_WIDTH;
				if (!IS_ALIGNED(GET_SCALED_SIZE(stripe_w, h_ratio), MCSC_WIDTH_ALIGN))
					temp_offset = MCSC_WIDTH_ALIGN;
			}
		}

		if (*use_out_crop) {
			*stripe_start_pos_x  = stripe_start_x;
			/* Horizontal start,end point for outcrop */
			stripe_roi_start_x = ALIGN_DOWN(GET_SCALED_SIZE(stripe_roi_start_x, h_ratio), MCSC_WIDTH_ALIGN);
			stripe_roi_end_x = ALIGN_DOWN(GET_SCALED_SIZE(stripe_roi_end_x, h_ratio), MCSC_WIDTH_ALIGN);
			stripe_start_x = ALIGN_DOWN((GET_SCALED_SIZE(stripe_start_x, h_ratio) + 2), MCSC_WIDTH_ALIGN) + temp_offset;
			stripe_end_x = ALIGN_DOWN(GET_SCALED_SIZE(stripe_end_x, h_ratio), MCSC_WIDTH_ALIGN);

			/* Use 'incrop' as outcrop parameter */
			incrop->x = ALIGN_DOWN((stripe_roi_start_x - stripe_start_x + 2), MCSC_WIDTH_ALIGN);
			incrop->w = ALIGN_DOWN((stripe_roi_end_x - stripe_roi_start_x), MCSC_WIDTH_ALIGN);

			/* Vertical start,end point for outcrop */
			stripe_roi_start_y = incrop->y;
			stripe_roi_end_y = incrop->y + incrop->h;
			stripe_start_y = 0;
			stripe_end_y = mcs_param->input.height;

			stripe_roi_start_y = ALIGN_DOWN(GET_SCALED_SIZE(stripe_roi_start_y, v_ratio), MCSC_HEIGHT_ALIGN);
			stripe_roi_end_y = ALIGN_DOWN(GET_SCALED_SIZE(stripe_roi_end_y, v_ratio), MCSC_HEIGHT_ALIGN);
			stripe_start_y = ALIGN_DOWN((GET_SCALED_SIZE(stripe_start_y, v_ratio) + 1), MCSC_HEIGHT_ALIGN);
			stripe_end_y = ALIGN_DOWN(GET_SCALED_SIZE(stripe_end_y, v_ratio), MCSC_HEIGHT_ALIGN);

			incrop->y = ALIGN_DOWN((stripe_roi_start_y - stripe_start_y + 1), MCSC_HEIGHT_ALIGN);
			incrop->h = ALIGN_DOWN((stripe_roi_end_y - stripe_roi_start_y), MCSC_HEIGHT_ALIGN);
		} else {
			incrop->x = stripe_x;
			incrop->w = stripe_w;
		}
		...
		if (!ldr_frame->stripe_info.region_id) {
			...
			if (*use_out_crop) {
				stripe_w = stripe_end_x - stripe_start_x;
				temp_roi_stripe_w = stripe_roi_end_x - stripe_roi_start_x;
			} else {
				stripe_w = ALIGN(otcrop->w * frame->stripe_info.in.h_pix_ratio / STRIPE_RATIO_PRECISION, 4);
				temp_roi_stripe_w = stripe_w;
			}

			frame->stripe_info.out.h_pix_num = temp_roi_stripe_w;
			...
			if (x_flip)
				dma_offset = (otcrop->w - temp_roi_stripe_w) * fmt->bitsperpixel[0] / BITS_PER_BYTE;
		} else if (ldr_frame->stripe_info.region_id < ldr_frame->stripe_info.region_num - 1) {
			/* Middle region */
			if (*use_out_crop) {
				stripe_w = stripe_end_x - stripe_start_x;
			...
			/* Add horizontal DMA offset */
			if (x_flip)
				dma_offset = (otcrop->w - frame->stripe_info.out.h_pix_num - temp_roi_stripe_w) * fmt->bitsperpixel[0] / BITS_PER_BYTE;
			else
		...
		if (*use_out_crop)
			otcrop->h = ALIGN_DOWN((stripe_end_y - stripe_start_y), MCSC_HEIGHT_ALIGN);
		if (temp_stripe_w <= 0) {
	...
	u32 *indexes)
{
	...
	if (IS_ENABLED(CHAIN_USE_STRIPE_PROCESSING) && frame && frame->stripe_info.region_num) {
		stripe_ret = is_ischain_mxp_stripe_cfg(subdev, frame,
				...
				&stripe_start_pos_x, &use_out_crop);
		if (stripe_ret) {
	...
	if ((index - PARAM_MCS_OUTPUT0) != MCSC_OUTPUT_DS) {
		/* If stripe processing && bcrop, use out crop */
		if (use_out_crop) {
			is_ischain_mxp_compare_size(device, mcs_param, &otcrop_cfg, &incrop_cfg, true);
		} else {
	...
	mcs_output->stripe_in_start_pos_x = stripe_start_pos_x; /* for stripe */
	mcs_output->use_out_crop = use_out_crop;				/* for stripe */
