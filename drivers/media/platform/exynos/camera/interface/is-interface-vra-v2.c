...
int __nocfi is_lib_vra_invoke_fwalgs_event(struct is_lib_vra *lib_vra)
{
	...
	if (in_irq()) {
		...
		status = CALL_VRAOP(lib_vra, on_fw_algs_task_event,
					lib_vra->fr_work_heap, lib_vra->post_frame_info);
		if (status) {
...
void is_lib_vra_set_event_fw_algs(void *post_frame_info)
{
	struct is_lib_vra *lib_vra = g_lib_vra;
...
int __nocfi is_lib_vra_alloc_memory(struct is_lib_vra *lib_vra, ulong dma_addr)
{
	...
	/* ToDo: make define */
	alloc_info->max_image.width  = 320;
	alloc_info->max_image.height = 240;
	...
	lib_vra->dma_net_array.is_cached  = true;
	...
	size = sizeof(struct api_vpl_post_frame_info);
	lib_vra->post_frame_info = is_alloc_vra(size);
	memset(lib_vra->post_frame_info, 0, size);

	dbg_lib(3, "lib_vra_alloc_memory: api_vpl_post_frame_info (0x%p)\n", lib_vra->post_frame_info);
...
int __nocfi is_lib_vra_init_frame_desc(struct is_lib_vra *lib_vra, u32 instance)
{
	...
	if (lib_vra->fr_work_init.dram_input) {
		lib_vra->frame_desc[instance].sizes.width = 320;
		lib_vra->frame_desc[instance].sizes.height = 240;
	...
	struct camera2_stats_dm *dm)
{
	...
	for (face_num = 0; face_num < min_t(u32, lib_vra->all_face_num[instance], MAX_FACE_COUNT); face_num++) {
		...
		/* Score (0 ~ 255) */
		dm->faceScores[face_num] = base->score;
		/* ID */
...
int __nocfi is_lib_vra_test_input(struct is_lib_vra *lib_vra, u32 instance)
{
	...
	frame_desc->sizes.width  = 320;
	frame_desc->sizes.height = 240;
