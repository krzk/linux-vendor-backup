...
/* return value of vpl api functions */
enum api_vpl_type {
	VPL_OK			= 0,
	VPL_ERROR		= 1,
};
...
struct api_vpl_memory {
	vra_dram_adr_type		addr;		/* The memory pointer */
...
/* hold the width and the height of a frame. (in pixels) */
...
struct api_vpl_frame {
	vra_dram_adr_type				in_image_addr;		/* The frame's image buffer */
