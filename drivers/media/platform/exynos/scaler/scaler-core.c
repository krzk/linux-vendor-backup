...
static const u32 sc_version_table[][2] = {
	...
	{ 0x01200100, SCALER_VERSION(4, 2, 1) },
...
#endif
#ifdef CONFIG_DRM_DMA_SYNC
	BUG_ON(svb->mb.vb.vb2_buf.planes[0].dbuf == NULL);
#endif
	BUG_ON(!ctx->m2m_ctx);
...
#endif
#ifdef CONFIG_DRM_DMA_SYNC
	fence = tdm_fence(ctx->sc_dev->fence_dev, svb->mb.vb.vb2_buf.planes[0].dbuf);
	if (!fence)
		dev_warn(ctx->sc_dev->dev, "failed to get fence:buf\n");
	
	svb->mb.vb.vb2_buf.fence = fence;
#endif
