...
struct tdm_private {
	...
	struct workqueue_struct	*vbl_wq;
	struct tdm_vbl_work *vbl_work;
	struct work_struct virtual_vbl_work;
	unsigned long	virtual_vbl_us;
	struct completion	vbl_comp;
	atomic_t vbl_permission[TDM_CRTC_MAX];
#ifdef CONFIG_DRM_DMA_SYNC
