...
/* default phy config settings for exynos5410-odroidxu
 * Copied from branch 3.4.y
 */
#if 0   // HDMI PHY Configuration
    buffer[3]  |= 0xC0;   // PHY ADDR 0x10 : CH0 LEVEL MAX
    buffer[18] |= 0x03;   // PHY ADDR 0x4C : CH1 LEVEL MAX
    buffer[22] |= 0xFB;   // PHY ADDR 0x5C : CH2 LEVEL MAX & TX_CLK_LEVEL_MAX

    buffer[14] |= 0x80;   // PHY ADDR 0x3C : TX_AMP_LEVEL MAX
    buffer[15] |= 0x0F;   // PHY ADDR 0x40 : TX_AMP_LEVEL_MAX
#endif	

static struct hdmiphy_config hdmiphy_odroidxu_configs[] = {
	{
		.pixel_clock = 25200000,
		.conf = {
			0x52, 0x3F, 0x55, (0x40 | 0xC0), 0x01, 0x00, 0xC8, 0x82,
			0xC8, 0xBD, 0xD8, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x06 | 0x0F),
			0x80, 0x01, (0x84 | 0x03), 0x05, 0x02, 0x24, (0x66 | 0xFB), 0x54,
			0xF4, 0x24, 0x00, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 27000000,
		.conf = {
			0xD1, 0x22, 0x51, (0x40 | 0xC0), 0x08, 0xFC, 0x20, 0x98,
			0xE8, 0xCB, 0xD8, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x06 | 0x0F),
			0x80, 0x01, (0x84 | 0x03), 0x05, 0x22, 0x24, (0x86 | 0xFB), 0x54,
			0xE4, 0x24, 0x00, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 27027000,
		.conf = {
			0xD1, 0x2D, 0x72, (0x40 | 0xC0), 0x64, 0x12, 0x08, 0x43,
			0xE8, 0x0E, 0xD9, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x06 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x22, 0x24, (0x86 | 0xFB), 0x54,
			0xE3, 0x24, 0x00, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 36000000,
		.conf = {
			0x51, 0x2D, 0x55, (0x40 | 0xC0), 0x40, 0x00, 0xC8, 0x02,
			0xC8, 0x0E, 0xD9, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x08 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x02, 0x24, (0x66 | 0xFB), 0x54,
			0xAB, 0x24, 0x00, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 40000000,
		.conf = {
			0xD1, 0x21, 0x31, (0x40 | 0xC0), 0x3C, 0x28, 0xC8, 0x87,
			0xE8, 0xC8, 0xD8, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x08 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x02, 0x24, (0x66 | 0xFB), 0x54,
			0x9A, 0x24, 0x00, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 65000000,
		.conf = {
			0xD1, 0x36, 0x34, (0x40 | 0xC0), 0x0C, 0x04, 0xC8, 0x82,
			0xE8, 0x45, 0xD9, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x08 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x02, 0x24, (0x66 | 0xFB), 0x54,
			0xBD, 0x24, 0x01, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 71000000,
		.conf = {
			0xD1, 0x3B, 0x35, (0x40 | 0xC0), 0x0C, 0x04, 0xC8, 0x85,
			0xE8, 0x63, 0xD9, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x08 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x02, 0x24, (0x66 | 0xFB), 0x54,
			0x57, 0x24, 0x00, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 74175000,
		.conf = {
			0xD1, 0x1F, 0x10, (0x40 | 0xC0), 0x5B, 0xEF, 0x08, 0x81,
			0xE8, 0xB9, 0xD8, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x56 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x22, 0x24, (0x86 | 0xFB), 0x54,
			0xA6, 0x24, 0x01, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 74250000,
		.conf = {
			0xD1, 0x1F, 0x10, (0x40 | 0xC0), 0x40, 0xF8, 0x08, 0x81,
			0xE8, 0xBA, 0xD8, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x56 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x22, 0x24, (0x86 | 0xFB), 0x54,
			0xA5, 0x24, 0x01, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 83500000,
		.conf = {
			0xD1, 0x23, 0x11, (0x40 | 0xC0), 0x0C, 0xFB, 0xC8, 0x85,
			0xE8, 0xD1, 0xD8, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x08 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x02, 0x24, (0x66 | 0xFB), 0x54,
			0x4A, 0x24, 0x00, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 106500000,
		.conf = {
			0xD1, 0x2C, 0x12, (0x40 | 0xC0), 0x0C, 0x09, 0xC8, 0x84,
			0xE8, 0x0A, 0xD9, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x08 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x02, 0x24, (0x66 | 0xFB), 0x54,
			0x73, 0x24, 0x01, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 108000000,
		.conf = {
			0x51, 0x2D, 0x15, (0x40 | 0xC0), 0x01, 0x00, 0xC8, 0x82,
			0xC8, 0x0E, 0xD9, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x08 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x02, 0x24, (0x66 | 0xFB), 0x54,
			0xC7, 0x25, 0x03, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 148352000,
		.conf = {
			0xD1, 0x1F, 0x00, (0x40 | 0xC0), 0x5B, 0xEF, 0x08, 0x81,
			0xE8, 0xB9, 0xD8, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x66 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x22, 0x24, (0x86 | 0xFB), 0x54,
			0x4B, 0x25, 0x03, 0x00, 0x00, 0x01, 0x80,
		},
	},
	{
		.pixel_clock = 148500000,
		.conf = {
			0xD1, 0x1F, 0x00, (0x40 | 0xC0), 0x40, 0xF0, 0x08, 0x81,
			0xe8, 0xBA, 0xD8, 0x45, 0xA0, 0xAC, (0x80 | 0x80), (0x66 | 0x0F),
			0x80, 0x09, (0x84 | 0x03), 0x05, 0x22, 0x24, (0x86 | 0xFB), 0x54,
			0x4B, 0x25, 0x03, 0x00, 0x00, 0x01, 0x80,
		},
	},
...
static struct hdmiphy_drv_data exynos5410_odroidxu_hdmiphy_drv_data = {
	.confs = hdmiphy_odroidxu_configs,
	.count = ARRAY_SIZE(hdmiphy_odroidxu_configs)
};
...
static struct of_device_id hdmiphy_platform_device_match_types[] = {
	{
		...
		.data	= &exynos5410_odroidxu_hdmiphy_drv_data,
	}, {
		/* end node */
	}
};
