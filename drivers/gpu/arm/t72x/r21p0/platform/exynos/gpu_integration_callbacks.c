...
#define GPU_DQA_UEVENT_ENV	"FEATURE=GPUI"
#define GPU_DQA_UEVENT_TRIG	5
#endif
...
#include <mali_kbase_gpu_memory_debugfs.h>
#ifdef CONFIG_MALI_TIZEN_MEM_INFO
int gpu_memory_seq_show(struct seq_file *sfile, void *data)
{
	...
	list_for_each(entry, kbdev_list) {
		...
		list_for_each_entry(element, &kbdev->kctx_list, link) {
			...
			struct kbase_mem_pool *mem_pool = &kctx->mem_pool;

			spin_lock(&(mem_pool->pool_lock));
			free_size += mem_pool->cur_size;
		...
		seq_printf(sfile, " %16s  %18s  %20s\n",
			"dev name", "total used pages", "total shrink pages");
		seq_puts(sfile, "--------------------------------------------");
		seq_puts(sfile, "---------------\n");
		seq_printf(sfile, " %16s  %18u  %20zu\n",
			kbdev->devname, atomic_read(&(memdev->used_pages)),
			free_size);
		seq_puts(sfile, "============================================");
		seq_puts(sfile, "===============\n\n");
		seq_printf(sfile, "%8s %8s %26s     %20s  %16s  %12s\n",
		...
		list_for_each_entry(element, &kbdev->kctx_list, link) {
			struct kbase_context *kctx = element->kctx;
			struct kbase_mem_pool *mem_pool = &kctx->mem_pool;
			size_t each_free_size;

			/*
			 * Show the memory usage and cap for each kctx opened
			 * on this device.
			 */

			spin_lock(&(mem_pool->pool_lock));
			each_free_size = mem_pool->cur_size;
			spin_unlock(&(mem_pool->pool_lock));

			seq_printf(sfile,
				"%8d %8d  (%24s), %s-0x%p    %12u  %10zu\n",
				kctx->pid, kctx->tgid, kctx->name, "kctx", kctx,
...
void gpu_update_status(void *dev, char *str, u32 val)
{
	...
	char *env[2] = {GPU_DQA_UEVENT_ENV, NULL};
	int i, fault_count = 0;

	KBASE_DEBUG_ASSERT(kbdev != NULL);

	if (!strcmp(str, "completion_code")) {
		switch (val) {
		case 0x40:
			platform->exception_count[EXCEPTION_JOB_CONFIG_FAULT]++;
			break;
		case 0x41:
			platform->exception_count[EXCEPTION_JOB_POWER_FAULT]++;
			break;
		case 0x42:
			platform->exception_count[EXCEPTION_JOB_READ_FAULT]++;
			break;
		case 0x43:
			platform->exception_count[EXCEPTION_JOB_WRITE_FAULT]++;
			break;
		case 0x44:
			platform->exception_count[EXCEPTION_JOB_AFFINITY_FAULT]++;
			break;
		case 0x48:
			platform->exception_count[EXCEPTION_JOB_BUS_FAULT]++;
			break;
		case 0x58:
			platform->exception_count[EXCEPTION_DATA_INVALIDATE_FAULT]++;
			break;
		case 0x59:
			platform->exception_count[EXCEPTION_TILE_RANGE_FAULT]++;
			break;
		case 0x60:
			platform->exception_count[EXCEPTION_OUT_OF_MEMORY_FAULT]++;
			break;
		/* GPU FAULT */
		case 0x80:
			platform->exception_count[EXCEPTION_DELAYED_BUS_FAULT]++;
			break;
		case 0x88:
			platform->exception_count[EXCEPTION_SHAREABILITY_FAULT]++;
			break;
		/* MMU FAULT */
		case 0xC0 ... 0xC7:
	...
	} else if (!strcmp(str, "soft_stop"))
		platform->exception_count[EXCEPTION_SOFT_STOP]++;
	else if (!strcmp(str, "hard_stop"))
		platform->exception_count[EXCEPTION_HARD_STOP]++;
	else if (!strcmp(str, "reset_count"))
		platform->exception_count[EXCEPTION_RESET]++;

	for (i = EXCEPTION_JOB_CONFIG_FAULT; i < EXCEPTION_END; i++)
		fault_count += platform->exception_count[i];

	if (GPU_DQA_UEVENT_TRIG == fault_count)
