...
#ifdef CONFIG_MALI_TIZEN_DQA
static int gpu_get_status(struct exynos_context *platform, char *buf,
	size_t buf_size)
{
	int cnt = 0, i, mmu_fault_count = 0;
	...
	for (i = EXCEPTION_MMU_TRANSLATION_FAULT;
		i <= EXCEPTION_MMU_MEMORY_ATTRIBUTES_FAULT; i++)
		mmu_fault_count += platform->exception_count[i];

	cnt += snprintf(buf + cnt, buf_size - cnt, "reset count : %d\n",
		platform->exception_count[EXCEPTION_RESET]);
	cnt += snprintf(buf + cnt, buf_size - cnt, "data invalid count : %d\n",
		platform->exception_count[EXCEPTION_DATA_INVALIDATE_FAULT]);
	...
	for (i = 0; i < BMAX_RETRY_CNT; i++) {
		cnt += snprintf(buf + cnt, buf_size - cnt,
			...
			platform->balance_retry_count[i]);
	}
...
static ssize_t show_kernel_sysfs_gpu_info(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	...
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"SSTOP\":\"%d\",",
		platform->exception_count[EXCEPTION_SOFT_STOP]);
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"HSTOP\":\"%d\",",
		platform->exception_count[EXCEPTION_HARD_STOP]);
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"RESET\":\"%d\",",
		platform->exception_count[EXCEPTION_RESET]);
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"DIFLT\":\"%d\",",
		platform->exception_count[EXCEPTION_DATA_INVALIDATE_FAULT]);
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"TRFLT\":\"%d\",",
		platform->exception_count[EXCEPTION_MMU_TRANSLATION_FAULT]);
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"PMFLT\":\"%d\",",
		platform->exception_count[EXCEPTION_MMU_PERMISSION_FAULT]);
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"BFLT\":\"%d\",",
		platform->exception_count[EXCEPTION_MMU_TRANSTAB_BUS_FAULT]);
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"ACCFG\":\"%d\",",
		platform->exception_count[EXCEPTION_MMU_ACCESS_FLAG_FAULT]);
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"ASFLT\":\"%d\",",
		platform->exception_count[EXCEPTION_MMU_ADDRESS_SIZE_FAULT]);
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"ATFLT\":\"%d\",",
		platform->exception_count[EXCEPTION_MMU_MEMORY_ATTRIBUTES_FAULT]);
	ret += snprintf(buf + ret, BUF_SIZE - ret, "\"UNKN\":\"%d\"",
		platform->exception_count[EXCEPTION_UNKNOWN]);
...
struct gpufreq_stat *gpufreq_stat_alloc_stat_table(void)
{
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
 	struct gpufreq_stat *stat_table = NULL;

	if (!platform)
		return stat_table;

	stat_table = kzalloc(sizeof(*stat_table), GFP_KERNEL);
	if (!stat_table)
		return stat_table;

	stat_table->table_size = platform->table_size;

	return stat_table;
}

void gpufreq_stat_free_stat_table(
					struct gpufreq_stat *stat_table)
{
	kfree(stat_table);
}

int gpufreq_stat_get_stat(int type,
 					struct gpufreq_stat *gpufreq_stat)
{
	...
	gpufreq_stat->table_size = platform->table_size;

	gpu_dvfs_update_time_in_state(gpu_control_is_power_on(pkbdev) * platform->cur_clock);

	for (i = gpu_dvfs_get_level(platform->gpu_min_clock);
			i >= gpu_dvfs_get_level(platform->gpu_max_clock); i--) {
		gpufreq_stat->table[i].clock = platform->table[i].clock;
		gpufreq_stat->table[i].time = platform->table[i].emon_time;
...
int gpufreq_stat_get_stat_delta(
 					struct gpufreq_stat *gpufreq_stat_delta)
{
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
	unsigned int i;

	if (!gpufreq_stat_delta)
		return -EINVAL;
	...
	for (i = gpu_dvfs_get_level(platform->gpu_min_clock);
			i >= gpu_dvfs_get_level(platform->gpu_max_clock); i--) {
		gpufreq_stat_delta->table[i].clock = platform->table[i].clock;
		gpufreq_stat_delta->table[i].time = platform->table[i].delta_time;
