...
bool check_trace_code(enum kbase_trace_code code)
{
	...
	switch (temp) {
	case KBASE_TRACE_CODE(PM_PWROFF):
	case KBASE_TRACE_CODE(PM_PWRON):
	...
	case KBASE_TRACE_CODE(JM_SOFTSTOP_0):
	case KBASE_TRACE_CODE(JM_SOFTSTOP_1):
	case KBASE_TRACE_CODE(LSI_WAIT_SIGNALED):
	case KBASE_TRACE_CODE(LSI_TRIGGER_SIGNALED):
	case KBASE_TRACE_CODE(LSI_ATOM_SUBMIT):
		return true;
	default:
		return false;
	}
...
/* MALI_SEC_INTEGRATION */
	...
	if (NULL == katom) {
		...
		if(katom->dep[0].atom)
			trace_msg->atom_udata[0] = katom->dep[0].atom->age;
		else
			trace_msg->atom_udata[0] = 0;

		if(katom->dep[1].atom)
			trace_msg->atom_udata[1] = katom->dep[1].atom->age;
		else
