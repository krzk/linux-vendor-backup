...
kbase_sync_fence_out_trigger(struct kbase_jd_atom *katom, int result)
{
	...
	KBASE_TRACE_ADD(katom->kctx->kbdev, LSI_TRIGGER_SIGNALED, katom->kctx, katom, 0, katom->dma_fence.fence->seqno);
...
{
	kbase_fence_free_callbacks(katom);
	KBASE_TRACE_ADD(katom->kctx->kbdev, LSI_WAIT_SIGNALED, katom->kctx, katom, 0, katom->dma_fence.fence_in->seqno);
