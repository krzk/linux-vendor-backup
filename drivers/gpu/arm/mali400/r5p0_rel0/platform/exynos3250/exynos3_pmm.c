...
static void mali_exynos_dvfs_work(struct work_struct *work)
{
	...
	/*
	 * In a theoretical approach, there should be no cases like below,
	 * (that's because MALI core should guarantee its utilization value
	 * less than 256) but there is a case the value is 308 in real.
	 * So removes BUG_ON and corrects it as maximum one.
	 */
	if (step >= mali->nr_steps)
		step = mali->nr_steps - 1;
