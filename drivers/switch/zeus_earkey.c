...
int send_end_extra_detect(void)
{
	int sendend_state = 0;

	sendend_state = gpio_get_value(EAR_KEY_GPIO) ^ EAR_KEY_INVERT_ENABLE;
	//printk("[SKLee] sendend_state = %d\n", sendend_state);

	return sendend_state;
}
EXPORT_SYMBOL(send_end_extra_detect);
...
static void ear_switch_change(struct work_struct *ignored)
{
  ...
  //  		dev_err(ip_dev->dev.parent,"Input Device not allocated\n");
...
#if (defined(CONFIG_CHN_KERNEL_STE_LATONA))	
	if(gpio_get_value(EAR_KEY_GPIO));
		 ear_key_enable_irq();
#endif
	...
	printk("[HyoSun] Headset status : %d, irq token : %d \n", get_headset_status(), send_end_irq_token);
