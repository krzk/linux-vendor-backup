		...
		u8 val, bool attached)
{
	...
	ret = max14577_update_reg(info->maxim_core->regmap_muic,
				...
				MAXIM_CONTROL1_CLEAR_IDBEN_MICEN_MASK, MAXIM_CONTROL1_SW_UART);
	if (ret < 0) {
	...
	ret = max14577_update_reg(info->maxim_core->regmap_muic,
			...
			MAXIM_CONTROL2_LOWPWR_MASK | MAXIM_CONTROL2_CPEN_MASK, ctrl2);
	if (ret < 0) {
		dev_err(info->dev, "failed to update MUIC register\n");
		return ret;
	}
		...
		enum max14577_muic_cable_group group, bool *attached)
{
	...
	case MAX14577_CABLE_GROUP_ADC:
		 ...
		 */
		adc = info->status[MAX14577_MUIC_STATUS1] & MAXIM_STATUS1_ADC_MASK;
		...
		int cable_type, bool attached)
{
	char cable_name[32];
	int ret = 0;
	u8 path = MAXIM_CONTROL1_SW_UART;
...
static int max77836_parse_irq(struct max14577_muic_info *info, int irq_type)
{
	...
	case MAX77836_IRQ_INT1_ADC1K:
		info->irq_adc = true;
		return 1;
	case MAX77836_IRQ_INT2_VIDRM: // FIXME: VIDRM interrupt not tested
		/* Handle charger accessory */
		info->irq_chg = true;
		return 1;
	default:
...
static int max14577_muic_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < info->muic_irqs_num; i++) {
		...
		virq = regmap_irq_get_virq(maxim_core->irq_data_muic, muic_irq->irq);
		if (!virq)
			return -EINVAL;
		muic_irq->virq = virq;
