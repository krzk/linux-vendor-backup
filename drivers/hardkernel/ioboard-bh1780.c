...
#define BH1780_WORK_PERIOD  msecs_to_jiffies(100)
...
static int bh1780_power_up(struct bh1780_data *bh1780)
{
	i2c_smbus_write_byte_data(bh1780->client,
		(BH1780_COMMAND_REG + BH1780_CONTROL_REG), BH1780_POWER_UP);
...
static int bh1780_measure(struct bh1780_data *bh1780)
{
	...
	if (i2c_smbus_write_byte(bh1780->client,
...
//[*]------------------------------------------------------------------------[*]
static DEVICE_ATTR(lux,     0660, bh1780_data_show,	NULL);
static DEVICE_ATTR(enable,  0660, bh1780_enable_show,	bh1780_enable_set);
...
static int bh1780_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if ((err = bh1780_detect(client, NULL)) != 0)
		goto error;

	if ((err = i2c_smbus_write_byte(bh1780->client,
		(BH1780_COMMAND_REG + BH1780_PART_REV_REG))) < 0) {
		dev_err(&client->dev, "I2C write byte error: data=0x%02x\n",
	...
	if ((err = sysfs_create_group(&client->dev.kobj, &bh1780_attribute_group)) < 0)
		goto error;
