...
#define BMP180_WORK_PERIOD  msecs_to_jiffies(100)
...
static void bmp180_work_func(struct work_struct *work)
{
	...
	x3 = ((x1 + x2) + 2) >> 2;
	b4 = (bmp180->eeprom_vals.AC4 * (unsigned long)(x3 + 32768)) >> 15;
...
//[*]------------------------------------------------------------------------[*]
static DEVICE_ATTR(pressure,    0660, bmp180_pressure_show,	NULL);
static DEVICE_ATTR(temperature, 0660, bmp180_temperature_show,	NULL);
static DEVICE_ATTR(enable,      0660, bmp180_enable_show, bmp180_enable_set);
...
static int bmp180_detect(struct i2c_client *client)
{
	...
	if (bmp180_i2c_read(client, BMP180_ID_REG, &id, sizeof(id)))
		return  -1;
...
static int bmp180_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if ((err = sysfs_create_group(&client->dev.kobj, &bmp180_attribute_group)) < 0)
		goto error;
...
static int bmp180_remove(struct i2c_client *client)
{
	...
	if (bmp180->enabled)
		cancel_delayed_work_sync(&bmp180->work);
