...
void bcmbtsdio_process_f3_interrupt(struct sdio_func *func)
{
	...
	if ((!intrd) || err) {
       ...
       if (dhd_bt_wake) {
               printk(KERN_INFO "Resume caused by Bluetooth");
#ifdef CONFIG_SLEEP_MONITOR
               add_slp_mon_irq_list(0, "BT");
#endif
#ifdef CONFIG_ENERGY_MONITOR
               energy_monitor_record_wakeup_reason(0, "BT");
#endif
               dhd_bt_wake = false;
       }
...
static int bcmbtsdio_open(struct hci_dev *hdev)
{
	...
	{
		int int_enable = dhd_bus_cfg_read(g_dhd_handle, SDIO_FUNC_3,
...
static int bcmbtsdio_setup(struct hci_dev *hdev)
{
	...
	if (IS_ERR(skb)) {
		ret = PTR_ERR(skb);
		BT_ERR("%s: Reset Fail (%d)", __FUNCTION__, ret);
		return ret;
	}
	kfree_skb(skb);
	return 0;
