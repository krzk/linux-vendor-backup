/* linux/drivers/devfreq/exynos/exynos9110_bus_mif.c
 *
 * Copyright (c) 2015 Samsung Electronics Co., Ltd.
 *		http://www.samsung.com
 *
 * Samsung EXYNOS9110 SoC MIF devfreq driver
 *
...
static int exynos9110_mif_constraint_parse(struct exynos_devfreq_data *data,
...
static int exynos9110_devfreq_mif_cmu_dump(struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_mif_resume(struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_mif_suspend(struct exynos_devfreq_data *data)
{
#ifndef CONFIG_EXYNOS_DVFS_MANAGER
	if (pm_qos_request_active(&data->default_pm_qos_max))
...
static int exynos9110_devfreq_mif_reboot(struct exynos_devfreq_data *data)
{
	if (pm_qos_request_active(&data->default_pm_qos_max))
		pm_qos_update_request(&data->default_pm_qos_max,
				data->reboot_freq);
...
static int exynos9110_devfreq_mif_get_freq(struct device *dev, u32 *cur_freq,
...
static int exynos9110_devfreq_mif_set_freq(struct device *dev, u32 new_freq,
		struct clk *clk, struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_mif_init_freq_table(struct exynos_devfreq_data *data)
{
	...
	ret = exynos9110_mif_constraint_parse(data, min_freq, max_freq);
	if (ret) {
...
static int exynos9110_devfreq_mif_um_register(struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_mif_um_unregister(struct exynos_devfreq_data *data)
{
#ifdef CONFIG_EXYNOS_WD_DVFS
	int i;
	if (data->use_get_dev) {
...
static int exynos9110_devfreq_mif_get_status(struct exynos_devfreq_data *data)
{
#ifdef CONFIG_EXYNOS_WD_DVFS
	int i;
...
static int exynos9110_devfreq_mif_set_freq_prepare(struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_mif_set_freq_post(struct exynos_devfreq_data *data)
{
	if (data->new_freq > data->old_freq)
		bts_update_scen(BS_MIF_CHANGE, data->new_freq);
	return 0;
...
static int __init exynos9110_devfreq_mif_init_prepare(struct exynos_devfreq_data *data)
{
	data->ops.um_register = exynos9110_devfreq_mif_um_register;
	data->ops.um_unregister = exynos9110_devfreq_mif_um_unregister;
	data->ops.get_dev_status = exynos9110_devfreq_mif_get_status;
	data->ops.get_freq = exynos9110_devfreq_mif_get_freq;
	data->ops.set_freq = exynos9110_devfreq_mif_set_freq;
	data->ops.init_freq_table = exynos9110_devfreq_mif_init_freq_table;
	data->ops.suspend = exynos9110_devfreq_mif_suspend;
	data->ops.reboot = exynos9110_devfreq_mif_reboot;
	data->ops.resume = exynos9110_devfreq_mif_resume;
	data->ops.cmu_dump = exynos9110_devfreq_mif_cmu_dump;
	data->ops.set_freq_prepare = exynos9110_devfreq_mif_set_freq_prepare;
	data->ops.set_freq_post = exynos9110_devfreq_mif_set_freq_post;

	return 0;
}

static int __init exynos9110_devfreq_mif_initcall(void)
{
	if (register_exynos_devfreq_init_prepare(DEVFREQ_MIF,
				exynos9110_devfreq_mif_init_prepare))
		return -EINVAL;

	return 0;
}
fs_initcall(exynos9110_devfreq_mif_initcall);
