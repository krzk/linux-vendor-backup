/* linux/drivers/devfreq/exynos/exynos9110_bus_int.c
 *
 * Copyright (c) 2015 Samsung Electronics Co., Ltd.
 *              http://www.samsung.com
 *
 * Samsung EXYNOS9110 SoC INT devfreq driver
 *
...
static int exynos9110_int_constraint_parse(struct exynos_devfreq_data *data,
...
static int exynos9110_devfreq_int_cmu_dump(struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_int_reboot(struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_int_get_freq(struct device *dev, u32 *cur_freq,
...
static int exynos9110_devfreq_int_set_freq(struct device *dev, u32 new_freq,
		struct clk *clk, struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_int_resume(struct exynos_devfreq_data *data)
{
	u32 cur_freq;

	/* for sync from resume frequency */
	if (exynos9110_devfreq_int_get_freq(data->dev, &cur_freq, data->clk, data)) {
...
static int exynos9110_devfreq_int_init_freq_table(struct exynos_devfreq_data *data)
{
	...
	ret = exynos9110_int_constraint_parse(data, min_freq, max_freq);
	if (ret) {
...
static int exynos9110_devfreq_int_get_status(struct exynos_devfreq_data *data)
{
...
static int __init exynos9110_devfreq_int_init_prepare(struct exynos_devfreq_data *data)
{
	data->ops.get_dev_status = exynos9110_devfreq_int_get_status;
	data->ops.get_freq = exynos9110_devfreq_int_get_freq;
	data->ops.set_freq = exynos9110_devfreq_int_set_freq;
	data->ops.init_freq_table = exynos9110_devfreq_int_init_freq_table;
	data->ops.resume = exynos9110_devfreq_int_resume;
	data->ops.reboot = exynos9110_devfreq_int_reboot;
	data->ops.cmu_dump = exynos9110_devfreq_int_cmu_dump;

	return 0;
}

static int __init exynos9110_devfreq_int_initcall(void)
{
	if (register_exynos_devfreq_init_prepare(DEVFREQ_INT,
				exynos9110_devfreq_int_init_prepare))
		return -EINVAL;

	return 0;
}

fs_initcall(exynos9110_devfreq_int_initcall);
