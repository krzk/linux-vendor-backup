/* linux/drivers/devfreq/exynos/exynos9110_bus_disp.c
 *
 * Copyright (c) 2015 Samsung Electronics Co., Ltd.
 *              http://www.samsung.com
 *
 * Samsung EXYNOS9110 SoC DISP devfreq driver
 *
...
static int exynos9110_devfreq_disp_cmu_dump(struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_disp_reboot(struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_disp_get_freq(struct device *dev, u32 *cur_freq,
...
static int exynos9110_devfreq_disp_set_freq(struct device *dev, u32 new_freq,
		struct clk *clk, struct exynos_devfreq_data *data)
{
...
static int exynos9110_devfreq_disp_init_freq_table(struct exynos_devfreq_data *data)
{
...
static int __init exynos9110_devfreq_disp_init_prepare(struct exynos_devfreq_data *data)
{
	data->ops.get_freq = exynos9110_devfreq_disp_get_freq;
	data->ops.set_freq = exynos9110_devfreq_disp_set_freq;
	data->ops.init_freq_table = exynos9110_devfreq_disp_init_freq_table;
	data->ops.reboot = exynos9110_devfreq_disp_reboot;
	data->ops.cmu_dump = exynos9110_devfreq_disp_cmu_dump;

	return 0;
}

static int __init exynos9110_devfreq_disp_initcall(void)
{
	if (register_exynos_devfreq_init_prepare(DEVFREQ_DISP,
				exynos9110_devfreq_disp_init_prepare))
		return -EINVAL;

	return 0;
}
fs_initcall(exynos9110_devfreq_disp_initcall);
