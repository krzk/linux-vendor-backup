...
static struct pm_qos_request exynos4415_miflock_qos;
static void __iomem *vidcon_base;
				      ...
				      unsigned long *_freq, u32 flags)
{
	...
	/* If INT is not at L0, set HIVCLK bit of VIDCON0 */
	if (int_bus_opp_list[0].clk == freq)
		__raw_writel(__raw_readl(vidcon_base) & ~0x8000, vidcon_base);
	else
		__raw_writel(__raw_readl(vidcon_base) | 0x8000, vidcon_base);

	/* MIF locking - if INT is L0 or L1, lock MIF to L0*/
	if ((freq == int_bus_opp_list[0].clk) || (freq == int_bus_opp_list[1].clk)) {
		if (pm_qos_request_active(&exynos4415_miflock_qos))
			pm_qos_update_request(&exynos4415_miflock_qos, 543000);
	} else {
		if (pm_qos_request_active(&exynos4415_miflock_qos))
			pm_qos_update_request(&exynos4415_miflock_qos, 206000);
	}
...
static __devinit int exynos4415_busfreq_int_probe(struct platform_device *pdev)
{
	...
	/* Map the VIDCON0 register */
	vidcon_base = ioremap(0x11c00000, 32);
	if (!vidcon_base)
		dev_err(dev, "Error while Mapping VIDCON0...\n");
	...
	pm_qos_add_request(&exynos4415_miflock_qos, PM_QOS_BUS_THROUGHPUT, 543000);
...
static int exynos4415_busfreq_int_suspend(struct device *dev)
{
	...
	exynos_set_abb(ID_INT, ABB_BYPASS);
	return 0;
}
