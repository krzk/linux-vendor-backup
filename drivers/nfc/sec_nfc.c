...
static irqreturn_t sec_nfc_irq_thread_fn(int irq, void *dev_id)
{
	...
	if (gpio_get_value(pdata->irq) == 0) {
		...
		pr_info("[NFC]  Warning,irq-gpio state is low!\n");
		return IRQ_HANDLED;
	}
#endif
	...
	if (info->i2c_info.read_irq == SEC_NFC_SKIP) {
		dev_dbg(info->dev,
			"%s: Now power swiching. Skip this IRQ\n", __func__);
				...
				size_t count, loff_t *ppos)
{
	...
	ret = i2c_master_recv(info->i2c_info.i2c_dev,
		info->i2c_info.buf, count);
...
void sec_nfc_clk_ctl_enable(struct sec_nfc_info *info)
{
	...
	ret = request_threaded_irq(irq, NULL, sec_nfc_clk_irq_thread,
			...
			| IRQF_ONESHOT, SEC_NFC_DRIVER_NAME, info);
	if (ret < 0)
					...
					enum sec_nfc_mode mode)
{
	struct sec_nfc_platform_data *pdata = info->pdata;
	int value = 0;
	pr_info(" sec_nfc_set_mode is : %d\n", mode);

	pr_info(" sec_nfc_set_mode info mode is : %d\n", info->mode);

	/* intfo lock is aleady gotten before calling this function */
	if (info->mode == mode) {
		dev_dbg(info->dev, "Power mode is already %d", mode);
		pr_info("nfc Power mode is already %d", mode);
		return;
	}
	info->mode = mode;
	...
	if (mode != SEC_NFC_MODE_OFF) {
		...
		pr_info("nfc gpio_set_value VEN ON\n");
#ifdef CONFIG_SLEEP_MONITOR
		nfc_is_flag = 1;
	...
	value = gpio_get_value(pdata->ven);
	pr_info("nfc ven value : %d\n", value);\

	value = gpio_get_value(pdata->firm);
	pr_info("nfc firm value : %d\n", value);
...
static int sec_nfc_clk_parse_dt(struct sec_nfc_info *info)
{
	...
	ret = fimc_is_set_parent_dt(info->pdev,
		"mout_sclk_isp_sensor1", "oscclk");
	if (ret) {
		pr_err("%s, fimc_is_set_parent_dt:%d\n", __func__, ret);
		return -EPERM;
	}
	ret = fimc_is_set_rate_dt(info->pdev,
		"dout_sclk_isp_sensor1_a", 24 * 1000000);
	if (ret) {
		pr_err("%s, fimc_is_set_rate_dt A:%d\n", __func__, ret);
		return -EPERM;
	}
	ret = fimc_is_set_rate_dt(info->pdev,
		"dout_sclk_isp_sensor1_b", 24 * 1000000);
	if (ret) {
...
static int sec_nfc_parse_dt(struct device *dev,
	struct sec_nfc_platform_data *pdata)
{
	pr_info("nfc Don't use the Device Tree!!\n");
	return 0;
}
#endif
