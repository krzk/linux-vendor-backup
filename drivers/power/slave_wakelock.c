...
int add_slave_wakelock(
	const char *name, unsigned int pid, u32 sid, u8 lock_flag)
{
		...
		struct slave_wakelock *slwl, unsigned int idx)
{
	...
	seq_printf(m, "%-8d%-16s%-8u  %-5u  "
				"%-12lu  %-12lld  "
				"%-12lld    %-12lld  "
				"%-12lld  %-12lld    "
				"%-12lld  ",
						idx, slwl->name, slwl->pid, slwl->sid,
						active_count, ktime_to_ms(active_since),
...
static int slave_wakelock_show(struct seq_file *m, void *unused)
{
	...
	seq_puts(m, "idx     name            pid       sid    "
				"active_count  active_since  "
...
int slave_wake_lock(const char *buf)
{
	...
	/* Add slave wakelock to list */
	add_slave_wakelock(name, pid, sid, lock_flag);
