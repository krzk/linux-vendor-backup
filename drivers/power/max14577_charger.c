...
static enum maxim_muic_charger_type maxim_get_charger_type(
	...
	case MAX14577_CHARGER_TYPE_RESERVED:
		...
		return val;
	default:
		WARN_ONCE(1, "max14577: Unsupported charger type register value 0x%02x", val);
		return val;
	}
}
...
static int max14577_get_online(struct max14577_charger *chg)
{
	...
	reg_data = ((reg_data & MAXIM_STATUS2_CHGTYP_MASK) >> MAXIM_STATUS2_CHGTYP_SHIFT);
...
static void max14577_charger_reg_init(struct max14577_charger *chg)
{
	...
	max14577_update_reg(rmap, MAXIM_MUIC_REG_CDETCTRL1,
			MAXIM_CDETCTRL1_CHGDETEN_MASK | MAXIM_CDETCTRL1_CHGTYPMAN_MASK,
			reg_data);
