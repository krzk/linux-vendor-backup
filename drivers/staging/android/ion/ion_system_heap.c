...
unsigned long reclaim_ion_for_oom(struct ion_heap *heap)
{
	...
	struct ion_system_heap *sys_heap = container_of(heap,
							struct ion_system_heap,
							heap);
	gfp_t high_mask = __GFP_HIGHMEM;
	int i, nr_pool, nr_reclaimed = 0;

	nr_pool = ion_system_heap_shrink(heap, high_mask, 0);

	if (nr_pool > 0) {
		pr_info("-------------ION PAGE POOL DETAIL---------------\n");
		pr_info("\n[CACHABLE PAGE POOL]\n");
		for (i = 0; i < num_orders; i++) {
			pool = sys_heap->pools[i];
			pr_info("[order %d : %10d]", orders[i],
				pool->low_count + pool->high_count);
		}
		pr_info("\n[NON CACHABLE PAGE POOL]\n");
		for (i = num_orders; i < num_orders * 2; i++) {
			pool = sys_heap->pools[i];
			pr_info("[order %d : %10d]", orders[i % num_orders],
				pool->low_count + pool->high_count);
		}
		pr_info("\n----------------------------------------\n");
		nr_reclaimed = ion_system_heap_shrink(heap, high_mask, nr_pool);
		pr_info("%s : freed (%d)", __func__, nr_pool);
	}
