	...
	if (ret >= 0 && response->seq == seq) {
		if (response->reason == CMD_COMMS_ACK) {
			...
			if ((response->reason == CMD_COMMS_READ) || (response->reason == CMD_COMMS_WRITE)) {
				...
				for (i = 0; i < ret; i += 25)
					pr_debug(
					    "nanohub-NACK: %d(%x): %d: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
					    ret, (u32)response->reason, i, b[i], b[i + 1], b[i + 2], b[i + 3],
				...
				if (response->reason == CMD_COMMS_NACK)
					...
					ret = ERROR_BUSY;
				DEBUG_PRINT(KERN_ERR, "%s error c1: %d\n", __func__);
			}
		}
		...
		if (ret >= 0) {
			...
			for (i = 0; i < ret; i += 25)
				pr_debug(
				    "nanohub-NOSEQ: %d: %d: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
		...
		ret = ERROR_NACK;
		DEBUG_PRINT(KERN_ERR, "%s error c2: %d\n", __func__);
	}
