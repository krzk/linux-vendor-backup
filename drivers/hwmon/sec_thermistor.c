...
static int sec_therm_get_temp_cb(void *priv, unsigned int *raw, int chk_lv, int caller_type)
{
	...
	*raw = 0;
	...
	list_for_each_entry(info, info_lists, list) {
		...
		curr_temp = (unsigned int)temp;
		if (curr_temp > 0xFF) {
			pr_warn("%s: temp exceed MAX %s/%d\n",
				__func__, info->name, (temp/10));
			*raw = *raw | (0xFF << (total_cnt * 8));
		} else {
			*raw = *raw | (curr_temp << (total_cnt * 8));
		}

		if (info->type_id == TYPE_SEC_THREM_AP) {
			if (curr_temp > 60)
				pretty = 0x0F;
			else if (curr_temp < 45)
				pretty = 0;
			else
				pretty = curr_temp - 45;
		}
