...
#define TEMPERATURE_RESOLUTION		1000	/* change milli-degree to degree */
...
struct sec_therm_info {
	...
	char name_hr[PLATFORM_NAME_SIZE];
	unsigned type_id;
...
static const struct of_device_id sec_therm_match[] = {
	...
	{ .compatible = "samsung,sec-blk-thermistor",
		.data = &sec_thermistor_id[TYPE_SEC_THREM_BLK] },
	{ },
};
...
sec_therm_parse_dt(struct platform_device *pdev)
{
	...
	pdata->adc_arr_size = (unsigned int)(len1 / sizeof(u32));
...
/* CAUTION : This converting algorithm very coupled into LSI CP */
static int convert_adc_to_cp_temper(unsigned int adc)
{
	int temperature;
	ulong gpadc;
	long scaled_temp = 0;
	const long scaling_factor = 1000000;
	...
	if (scaled_temp >= 0)
		temperature = (scaled_temp + (scaling_factor/2)) /
				(scaling_factor / TEMPERATURE_RESOLUTION);
	else
		temperature = (scaled_temp - (scaling_factor/2)) /
				(scaling_factor / TEMPERATURE_RESOLUTION);
...
static int convert_adc_to_temper(struct sec_therm_info *info, unsigned int adc)
{
	...
	if (!info->pdata->adc_table || !info->pdata->adc_arr_size) {
		/* using fake temp */
		return DEFAULT_TEMPERATURE * TEMPERATURE_RESOLUTION;
	}
	...
	list_for_each_entry(info, &therm_info_lists, list) {
		if (info->type_id == therm_id) {
			...
			if (info->type_id == TYPE_SEC_THREM_CP_SPEC)
				t = convert_adc_to_cp_temper(adc) / TEMPERATURE_RESOLUTION;
			else
				t = (convert_adc_to_temper(info, adc) / TEMPERATURE_RESOLUTION);
	...
	struct device_attribute *devattr, char *buf)
{
	...
	if (adc < 0)
		...
		if (info->type_id == TYPE_SEC_THREM_CP_SPEC)
			temp = convert_adc_to_cp_temper(adc) / TEMPERATURE_RESOLUTION;
		else
			temp = (convert_adc_to_temper(info, adc)) / TEMPERATURE_RESOLUTION;
	}
	...
	return snprintf(buf, PAGE_SIZE, "temp:%d adc:%d\n", temp, adc);
}

static ssize_t sec_therm_show_hr(struct device *dev,
	struct device_attribute *devattr, char *buf)
{
	struct sec_therm_info *info;
	struct sec_therm_device_attribute *attr =
	...
	switch (debug_mode[info->type_id]) {
	case TEMPERATURE_CHANGE_DIRECT_VALUE:
		temp = debug_temp[info->type_id] * TEMPERATURE_RESOLUTION;
		break;
	case TEMPERATURE_CHANGE_OFFSET:
		temp += debug_temp[info->type_id] * TEMPERATURE_RESOLUTION;
		break;
...
#ifdef CONFIG_SENSORS_SEC_THERM_HISTORY
	if (info->pdata->history_index >= 0)
		sec_therm_history_update(info->pdata->history_index, temp / TEMPERATURE_RESOLUTION);
#endif

	return snprintf(buf, PAGE_SIZE, "temp:%d adc:%d [res:%d]\n", temp, adc, TEMPERATURE_RESOLUTION);
}
...
int sec_therm_get_ap_temperature(void)
{
	...
	if (adc < 0)
		...
		temp = convert_adc_to_temper(g_ap_therm_info, adc) / TEMPERATURE_RESOLUTION;
...
static int sec_therm_probe(struct platform_device *pdev)
{
	...
	if (temperature_dev) {
		...
		struct sec_therm_device_attribute *attrs_hr = devm_kzalloc(&pdev->dev,
				sizeof(*attrs), GFP_KERNEL);
		...
		attrs_hr->info = info;
		snprintf(info->name_hr, sizeof(info->name_hr), "%s_hr", info->name);
		attrs_hr->dev_attr.attr.name = info->name_hr;
		attrs_hr->dev_attr.show = sec_therm_show_hr;

		attrs_hr->dev_attr.store = NULL;
		attrs_hr->dev_attr.attr.mode = S_IRUGO;

		sysfs_attr_init(&attrs_hr->dev_attr.attr);

		ret = device_create_file(temperature_dev, &attrs_hr->dev_attr);
		if (ret)
			pr_err("%s: Failed to create attr: %d\n", __func__, ret);
