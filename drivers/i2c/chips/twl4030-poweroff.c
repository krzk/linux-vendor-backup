...
static void board_poweroff(void)
{
	...
	while(i<100){
		value = gpio_get_value(PHONE_ACTIVE_GPIO);
		if(value == 0){
			printk("[OneDram] Phone Active pin is low \n");
			break;
		}
		mdelay(100);
		i++;
		if(i==100){
			printk("[OneDram] Phone Active pin is still high\n");
		}
	}
	...
	if ( get_real_usbic_state() || gpio_get_value( OMAP_GPIO_JIG_ON18 ) )
	{
