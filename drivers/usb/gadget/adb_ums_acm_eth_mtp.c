...
extern int usb_change_config(struct usb_composite_dev *cdev,struct usb_configuration *config);
extern int get_usbmenupath_value(void);
extern void UsbMenuSel_Createsysfs(void);
extern void cleanup_UsbMenuSel(void);
...
static struct usb_string strings_dev[] = {
	...
	[STRING_SERIAL_IDX].s = "OMAP36XX_Android",
	{  }			/* end of list */
};
...
#if defined(CONFIG_SAMSUNG_KIES_UMS_SUPPORT) && defined(CONFIG_ARCH_OMAP34XX) \
 && defined(CONFIG_SAMSUNG_MODEL_NAME)
	{
		u8 model_name[16];
...
static int ums_only_bind_config(struct usb_configuration *c)
{
    ...
    /*change product id to temp value to load Window default UMS driver
	  This prevent loading unwanted driver by cached dirty driver*/
	dev->cdev->desc.idProduct = __constant_cpu_to_le16(0x1234);
...
static int acm_ums_adb_bind_config(struct usb_configuration *c)
{
	...
	printk("[USB] %s PID is %d\n",__func__,dev->cdev->desc.idProduct);
...
#define	ANDROID_NO_DEBUG_ACM_UMS_CONFIG_STRING "ACM+UMS (No Debugging mode)"
...
static int __init android_bind(struct usb_composite_dev *cdev)
{
	...
	/*set bcddevice number with 0x400 for auto kies install */
...
{
	int ret;
	int usbmenupath_value;


	usbmenupath_value = get_usbmenupath_value();

	if (enable != dev->adb_enabled) {
		prev_enable_status = dev->adb_enabled;
		printk("[USB] %s - enable(0x%04x), prev_status(0x%04x)\n", __func__, enable, prev_enable_status);

		if (dev->cdev && dev->cdev->gadget) {
			printk("[USB] %s disconnect\n",__func__);
		...
		if (enable == USBSTATUS_ADB_REMOVE) {
			...
			enable = usbmenupath_value & ~(USBSTATUS_ADB|0x03);
		}
...
*/
		...
		{	
		    /*set acm only mode when ASKON is selected for default*/
			ret = usb_change_config(dev->cdev, &acm_only_config);
			if (ret) {
				printk("[%s] Fail to acm_mtp_config()\n", __func__);
			}
...
EXPORT_SYMBOL(usb_switch_select);

//used by switch_sio for enable_adb at boot time to get param value
//and call enable_adb for set 
int set_usbmenu_mode(void)
{
    
	//enable_adb(_android_dev, usbmenuvalue);
    int usbmenupath_value=0;
	int ret=0;
	int enable=0;
	struct android_dev *dev;
	
	usbmenupath_value = get_usbmenupath_value();
	printk("[USB] %s USB menu value is %d\n",__func__, usbmenupath_value);
	
	dev = _android_dev;

	/*clear UART USB bit*/
	enable = usbmenupath_value & ~(0x03);
		...
		{
		    /*set acm only mode in ASKON mode for default*/
			ret = usb_change_config(dev->cdev, &acm_only_config);
			if (ret) {
				printk("[%s] Fail to acm_mtp_config()\n", __func__);
			}
...
*/
		if (dev->cdev && dev->cdev->gadget ) {
			printk("[USB] %s reenumerate at boot time\n",__func__);
			usb_gadget_disconnect(dev->cdev->gadget);
			mdelay(5);
			usb_gadget_connect(dev->cdev->gadget);
			}
...
EXPORT_SYMBOL(set_usbmenu_mode);
...
static int __init init(void)
{
    ...
    /*create sysfs files for USB MENU SEL to determine USB composite set*/
	UsbMenuSel_Createsysfs();
