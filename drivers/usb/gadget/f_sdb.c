...
requeue_req:
	...
	if (ret < 0) {
		printk(KERN_INFO "sdb read_wq wakeup by signal(%d)\n", ret);
		dev->error = 1;
		r = ret;

		mutex_lock(&sdb_lock);
		if (!dev->sdb_func || !dev->rx_req)
			printk(KERN_ERR "sdb already unbinded\n");
		else {
			printk(KERN_INFO "sdb rx_req was forcely dequeue\n");
			ret = usb_ep_dequeue(dev->sdb_func->ep_out,
					dev->rx_req);
			if (ret < 0)
				printk(KERN_ERR
					"sdb rx_req dequeue failed(%d)\n", ret);
			else
