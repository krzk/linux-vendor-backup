...
#define DEBUG_MTPB(fmt, args...) pr_info("[MTP_SET]"fmt, ##args)
#else
#define DEBUG_MTPB(fmt, args...) do {} while (0)
#endif

#ifdef DEBUG_MTP_READ
#define DEBUG_MTPR(fmt, args...) pr_info("[MTP_RD]"fmt, ##args)
#else
#define DEBUG_MTPR(fmt, args...) do {} while (0)
#endif

#ifdef DEBUG_MTP_WRITE
#define DEBUG_MTPW(fmt, args...) pr_info("[MTP_WR]"fmt, ##args)
#else
...
/* legacy gadget checking-definition by name */
...
struct mtpg_dev {
	...
	/* number of rx, tx & intr req to allocate */
	u8 rxq_cnt;
	u8 txq_cnt;
	u8 intq_cnt;
};
...
static long mtpg_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	case SET_SETUP_DATA:
		...
		if (size < 0) {
			status = -EIO;
			DEBUG_MTPR("[%s]\t%d:size is negative\n",
						__func__, __LINE__);
			break;
		}
...
#ifdef CONFIG_COMPAT
static long mtpg_compat_ioctl(struct file *file,
				 ...
				 struct usb_endpoint_descriptor *intr_desc)
{
	...
	for (i = 0; i < DIV_ROUND_UP(dev->txq_cnt, 4); i++) {
...
static int mtp_setup(struct usb_composite_dev *cdev, bool mini_mode)
{
	 ...
	 */
	if (mini_mode) {
		/* minimum resouce using mode for IOT/Wearable dev */
		mtpg->rxq_cnt = 4;
		mtpg->txq_cnt = 8;
		mtpg->intq_cnt = 2;
		...
		if (gadget_is_s3c_hsotg(gadget)) {
			/* s3c udc's max transferable size at once is 512KB-1 */
			mtpg->udc_max.txPktSize = mtpg->udc_max.rxPktSize =
