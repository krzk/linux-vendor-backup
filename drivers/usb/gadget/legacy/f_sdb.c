...
fail:
	...
	pr_err("sdb_bind() could not allocate requests\n");
	return -ENOMEM;
}
...
requeue_req:
	...
	if (dev && !dev->error) {
		...
		if (dev->rx_req && dev->rx_req->actual == 0)
			goto requeue_req;
		...
		unsigned intf, unsigned alt)
{
	...
	if (sdb_func->inf_id != intf) {
		pr_err("sdb_function_set_alt error wrong intf:%d alt:%d\n",
						intf, alt);
		return -EINVAL;
	}
	...
	if (ret) {
		pr_err("error, usb_ep_enable for sdb ep_in\n");
		return ret;
	}
	...
	if (ret) {
		usb_ep_disable(sdb_func->ep_in);
		sdb_func->ep_in->driver_data = NULL;
		pr_err("error, usb_ep_enable for sdb ep_out\n");
		return ret;
	}
...
err1:
	kfree(dev);
	_sdb_dev = NULL;
	pr_err("sdb gadget driver failed to initialize\n");
	return ret;
}
...
static int sdb_bind_config(struct usb_configuration *c)
{
	...
	if (!_sdb_dev) {
		pr_err("Error There is no _sdb_dev!!\n");
		return -ENODEV;
	}
	...
	if (ret)
		pr_err("Error in usb_add_function failed for sdb\n");
