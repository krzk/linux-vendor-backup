...
extern void max14577_EnableDisable_AccDet(int);

#if defined (CONFIG_MAX14577_MICROUSB_CURRENT_CHANGE)
extern void change_current_status( int status, bool is_sleep );
#endif
...
int stop_temperature_freezing_recover = CHARGE_RECOVER_TEMPERATURE_MIN;
...
static struct kobj_attribute batt_sysfs_testmode[] = {
	...
	*/
    ...
    __ATTR( batt_type, 0666, show_batt_type, NULL ), 
};
...
int _get_t2adc_data_( int ch )
{
	...
	twl_i2c_write_u8(0x0A, 0x0, 0x23);
#endif
...
static int battery_monitor_core( bool is_sleep )
{
	...
	if(sec_bootmode == 5)
	{
		stop_temperature_overheat = CHARGE_STOP_TEMPERATURE_MAX_OFFMODE;
		stop_temperature_freezing_recover = CHARGE_RECOVER_TEMPERATURE_MIN_OFFMODE;
	}
	else
	{
		stop_temperature_overheat = CHARGE_STOP_TEMPERATURE_MAX;
		stop_temperature_freezing_recover = CHARGE_RECOVER_TEMPERATURE_MIN;
	}
    ...
    {
#if defined (CONFIG_MAX14577_MICROUSB_CURRENT_CHANGE)
		if ((sec_bci.battery.battery_temp >= CURRENT_CHANGE_TEMPERATURE))
		{
			change_current_status(CHARGING_CURRENT_400, is_sleep);
		}
		else
		{
			change_current_status(CHARGING_CURRENT_600, is_sleep);
		}
#endif
        ...
        if ( sec_bci.battery.support_monitor_temp )
        {
                ...
                || sec_bci.battery.battery_health == POWER_SUPPLY_HEALTH_COLD )
            {
                if ( sec_bci.battery.battery_temp <= recover_temperature_overheat //CHARGE_RECOVER_TEMPERATURE_MAX 
                    && sec_bci.battery.battery_temp >= stop_temperature_freezing_recover )
                {
