...
#if defined CONFIG_MACH_OMAP_SAMSUNG
		if(!usbsel){
#ifdef CONFIG_CHN_USB_FSA9480			
			gpio_set_value(OMAP_GPIO_USBSW_I2C_SCL, 0);
#else
...
#ifdef CONFIG_CHN_USB_FSA9480	
	if(gpio_request(OMAP_GPIO_USBSW_I2C_SCL, "Phone_VBUS_enable")<0) {
#else
...
#ifdef CONFIG_CHN_USB_FSA9480
	printk("RANJ ** MODEM USB SELECT PIN = %d configured as output \n", (int)OMAP_GPIO_USBSW_I2C_SCL);
	gpio_direction_output(OMAP_GPIO_USBSW_I2C_SCL, 0);
#else
