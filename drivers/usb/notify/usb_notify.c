...
#define USB_PATH 0x20000
static void usb_path_setting(void)
{
	void __iomem *base;
	u32 reg;

	pr_info("[%s]\n", __func__);
	base = ioremap(0x11860000, 0x1000);
	reg = readl(base + UART_IO_SHARE_CTRL);
	if (reg == USB_PATH) {
		pr_info("it is already set as USBPATH\n");
		return;
	}
	pr_info("previous UART_IO_SHARE : 0x%x\n", reg);
	writel(USB_PATH, base + UART_IO_SHARE_CTRL);
}
