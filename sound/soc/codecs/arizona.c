...
int arizona_set_aif_tristate(struct snd_soc_codec *codec, bool onoff)
{
	struct arizona *arizona = dev_get_drvdata(codec->dev->parent);

	dev_info(arizona->dev, "%s %d\n", __func__, onoff);
	if (onoff) {
		regmap_update_bits(arizona->regmap, ARIZONA_AIF1_TX_PIN_CTRL,
			ARIZONA_AIF1TX_DAT_TRI_MASK, ARIZONA_AIF1TX_DAT_TRI);
		regmap_update_bits(arizona->regmap, ARIZONA_AIF1_RATE_CTRL,
			ARIZONA_AIF1_TRI_MASK, ARIZONA_AIF1_TRI);

		regmap_update_bits(arizona->regmap, ARIZONA_AIF2_TX_PIN_CTRL,
			ARIZONA_AIF2TX_DAT_TRI_MASK, ARIZONA_AIF2TX_DAT_TRI);
		regmap_update_bits(arizona->regmap, ARIZONA_AIF2_RATE_CTRL,
			ARIZONA_AIF2_TRI_MASK, ARIZONA_AIF2_TRI);

		regmap_update_bits(arizona->regmap, ARIZONA_AIF3_TX_PIN_CTRL,
			ARIZONA_AIF3TX_DAT_TRI_MASK, ARIZONA_AIF3TX_DAT_TRI);
		regmap_update_bits(arizona->regmap, ARIZONA_AIF3_RATE_CTRL,
			ARIZONA_AIF3_TRI_MASK, ARIZONA_AIF3_TRI);
	} else {
		regmap_update_bits(arizona->regmap, ARIZONA_AIF1_TX_PIN_CTRL,
			ARIZONA_AIF1TX_DAT_TRI_MASK, 0);
		regmap_update_bits(arizona->regmap, ARIZONA_AIF1_RATE_CTRL,
			ARIZONA_AIF1_TRI_MASK, 0);

		regmap_update_bits(arizona->regmap, ARIZONA_AIF2_TX_PIN_CTRL,
			ARIZONA_AIF2TX_DAT_TRI_MASK, 0);
		regmap_update_bits(arizona->regmap, ARIZONA_AIF2_RATE_CTRL,
			ARIZONA_AIF2_TRI_MASK, 0);

		regmap_update_bits(arizona->regmap, ARIZONA_AIF3_TX_PIN_CTRL,
			ARIZONA_AIF3TX_DAT_TRI_MASK, 0);
		regmap_update_bits(arizona->regmap, ARIZONA_AIF3_RATE_CTRL,
			ARIZONA_AIF3_TRI_MASK, 0);
	}
	return 0;
}
EXPORT_SYMBOL_GPL(arizona_set_aif_tristate);
