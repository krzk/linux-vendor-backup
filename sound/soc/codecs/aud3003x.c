		...
		if (aud3003x->codec_ver == REV_0_1) {
			if (!aud3003x->amic_cal_done)
				queue_delayed_work(aud3003x->amicl_cap_cal_wq,
						&aud3003x->amicl_cap_cal_work, msecs_to_jiffies(300));
			else
				queue_delayed_work(aud3003x->amicl_cap_cal_wq,
		...
		if (aud3003x->codec_ver == REV_0_1) {
			if (!aud3003x->amic_cal_done)
				queue_delayed_work(aud3003x->amicc_cap_cal_wq,
						&aud3003x->amicc_cap_cal_work, msecs_to_jiffies(300));
			else
				queue_delayed_work(aud3003x->amicc_cap_cal_wq,
		...
		if (aud3003x->codec_ver == REV_0_1) {
			if (!aud3003x->amic_cal_done)
				queue_delayed_work(aud3003x->amicr_cap_cal_wq,
						&aud3003x->amicr_cap_cal_work, msecs_to_jiffies(300));
			else
				queue_delayed_work(aud3003x->amicr_cap_cal_wq,
...
static void aud3003x_bgr_pulldn_work(struct work_struct *work)
{
	struct aud3003x_priv *aud3003x =
		container_of(work, struct aud3003x_priv, bgr_pulldn_work.work);
	struct snd_soc_component *codec = aud3003x->codec;

	dev_dbg(codec->dev, "%s called, bgr pull down %s for remove analog noise.\n",
			__func__, aud3003x->bgr_pulldn_flag ? "TURN ON" : "TURN OFF");

	mutex_lock(&aud3003x->bgr_pulldn_lock);
	if (aud3003x->bgr_pulldn_flag) {
		/* Remove analog noise when use rx/tx combo path */
		i2c_client_change(aud3003x, AUD3003D);
		snd_soc_component_update_bits(codec, AUD3003X_B0_ODSEL1,
				T_EN_BGR_PULLDN_MASK, T_EN_BGR_PULLDN_MASK);
		i2c_client_change(aud3003x, CODEC_CLOSE);
		...
		/* BGR pull down turn off sequence */
		i2c_client_change(aud3003x, AUD3003A);
		snd_soc_component_update_bits(codec, AUD3003X_102_REF_CTRL1,
				EN_BGR_PULL_DN_MASK, EN_BGR_PULL_DN_MASK);
		i2c_client_change(aud3003x, CODEC_CLOSE);
		...
		i2c_client_change(aud3003x, AUD3003D);
		snd_soc_component_update_bits(codec, AUD3003X_B0_ODSEL1,
				T_EN_BGR_PULLDN_MASK, 0);
		i2c_client_change(aud3003x, CODEC_CLOSE);
	}
	mutex_unlock(&aud3003x->bgr_pulldn_lock);
}
		...
		struct snd_soc_dai *dai)
{
	...
	if (aud3003x->capture_on || aud3003x->playback_on) {
		aud3003x->bgr_pulldn_flag = true;
		queue_delayed_work(aud3003x->bgr_pulldn_wq, &aud3003x->bgr_pulldn_work,
				msecs_to_jiffies(0));
	}
	...
	if (!aud3003x->capture_on && !aud3003x->playback_on) {
		...
		if (aud3003x->bgr_pulldn_flag) {
			aud3003x->bgr_pulldn_flag = false;
			queue_delayed_work(aud3003x->bgr_pulldn_wq, &aud3003x->bgr_pulldn_work,
					msecs_to_jiffies(0));
		}
	}
...
static void aud3003x_register_initialize(void *context)
{
	...
	/* PDB_BGR Testmode on */
	snd_soc_component_update_bits(codec, AUD3003X_B0_ODSEL1, T_PDB_BGR_MASK, T_PDB_BGR_MASK);
	...
	/* PDB_BGR on */
	snd_soc_component_update_bits(codec, AUD3003X_102_REF_CTRL1, PDB_BGR_MASK, PDB_BGR_MASK);
...
static int aud3003x_codec_probe(struct snd_soc_component *codec)
{
	...
	INIT_DELAYED_WORK(&aud3003x->bgr_pulldn_work, aud3003x_bgr_pulldn_work);
	aud3003x->bgr_pulldn_wq = create_singlethread_workqueue("bgr_pulldn_wq");
	if (aud3003x->bgr_pulldn_wq == NULL) {
		dev_err(codec->dev, "Failed to create bgr_pulldn_wq\n");
		return -ENOMEM;
	}
	...
	mutex_init(&aud3003x->bgr_pulldn_lock);
...
static void aud3003x_codec_remove(struct snd_soc_component *codec)
{
	...
	destroy_workqueue(aud3003x->bgr_pulldn_wq);
