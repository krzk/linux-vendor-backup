...
#define BIN_FILE_NAME		"s-voice.bin"
...
#ifdef WM_ADSP_USE_VMALLOC
	scratch = vmalloc(ctl->len);
#else
...
static int wm_adsp_load(struct wm_adsp *dsp)
{
	...
	while (pos < firmware->size &&
		...
		if (reg) {
#ifdef WM_ADSP_USE_VMALLOC
			buf = vmalloc(le32_to_cpu(region->len));
#else
...
#ifdef WM_ADSP_USE_VMALLOC
			memcpy(buf, region->data, le32_to_cpu(region->len));
#endif
					  ...
					  bool create)
{
	...
	if (region->type ==  WMFW_ADSP2_YM && region->alg == 0x24) {
		adsp_dbg(dsp, "skip to create ctl of YM region\n");
		return 0;
	}
...
static int wm_adsp_load_coeff(struct wm_adsp *dsp)
{
	...
	strncpy(file, BIN_FILE_NAME, PAGE_SIZE);
	file[PAGE_SIZE - 1] = '\0';
	ret = request_firmware(&firmware, file, dsp->dev);
	/*Try load s-voice.bin first.
	  If it fails then load default bin */
	if(ret != 0) {
		adsp_info(dsp, "%s load default bin file!!!\n", __func__);
		if (dsp->firmwares[dsp->fw].binfile)
	...
	while (pos < firmware->size &&
		...
		if (reg) {
			blk_len_sum += le32_to_cpu(blk->len);
			if (blk_len_sum > firmware->size) {
				adsp_err(dsp, "abnormal %s firmware size\n", file);
				WARN_ON(1);
				ret = -EFAULT;
				goto out_fw;
			}
#ifdef WM_ADSP_USE_VMALLOC
			buf = vmalloc(le32_to_cpu(blk->len));
#else
...
#ifdef WM_ADSP_USE_VMALLOC
			adsp_info(dsp, "buf 0x%p, data 0x%p, len %d\n",
					buf, blk->data, le32_to_cpu(blk->len));
			memcpy(buf, blk->data, le32_to_cpu(blk->len));
#endif
			ret = regmap_raw_write(regmap, reg, buf,
					       le32_to_cpu(blk->len));
			if (ret != 0) {
