...
//				twl4030_modify(codec, 0x1C, (0x0 << 3), ARX_APGA_GAIN_MASK); //ARXR2_APGA_CTL [3:7]
	...
	else if (mode == FM_RADIO)
	{
		switch(device)
		{
			...
			case SPK: 
				//play
				twl4030_write(codec, 0x12, (0x1 << 6) | 0x2b); // ARXR2PGA (fine[0:5], Corse[6:7])
				twl4030_write(codec, 0x13, (0x1 << 6) | 0x2b); // ARXL2PGA (fine[0:5], Corse[6:7])
			...
			case HP4P: 
				//play
				twl4030_write(codec, 0x12, (0x0 << 6) | 0x35); // ARXR2PGA (fine[0:5], Corse[6:7])
				twl4030_write(codec, 0x13, (0x0 << 6) | 0x35); // ARXL2PGA (fine[0:5], Corse[6:7])
