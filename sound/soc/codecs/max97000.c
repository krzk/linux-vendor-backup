...
static int max97000_set_reg_from_file(char* filename)
{
	struct file *fp = NULL;
	mm_segment_t oldfs;
	int nFileSize = 0, i;
...
void max97000_apply_case(int spkORear, int musicORvoice)
{
	if(spkORear == INA_SPK){
		if(musicORvoice == VOICE_CALL){
			max97000_set_reg_from_file("/system/etc/audio/codec/VoiceCallSpkAmp.ini");
		}
		else if(musicORvoice == PLAY_BACK){
			max97000_set_reg_from_file("/system/etc/audio/codec/MusicSpkAmp.ini");
		}
		else
			printk( "[SKLee : 97000(INA_SPK)] Abnormal Case!!\n");
	}
	else if(spkORear == INB_HP){
		if(musicORvoice == VOICE_CALL){
			max97000_set_reg_from_file("/system/etc/audio/codec/VoiceCallEarAmp.ini");
		}
		else if(musicORvoice == PLAY_BACK){
			max97000_set_reg_from_file("/system/etc/audio/codec/MusicEarAmp.ini");
		}
		else
			printk( "[SKLee : 97000(INB_HP)] Abnormal Case!!\n");
	}
//	else
...
void increaseAmpGain(void)
{
	max97000_regs[0] = 0x4;
	max97000_regs[3] = 0x18;
	max97000_regs[4] = 0x18;
	max97000_write_regs();
}
EXPORT_SYMBOL(increaseAmpGain);
                ...
                const struct i2c_device_id *id)
{
...
#if defined(MAX97000_USE_GPIO_I2C)
	max97000_dummy_i2c_client = (OMAP_GPIO_I2C_CLIENT *)client;
	...
	if (gpio_request(OMAP_GPIO_AMP_SHDN , "AMP_SHDN") == 0) 
	{
		...
		printk("[MAX97000] set gpio AMP_HDN\n");
	}else
		printk("[MAX97000] fail to gpio request AMP_SHDN\n");
