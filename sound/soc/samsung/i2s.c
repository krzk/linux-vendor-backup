...
static void audss_reg_save(struct i2s_dai *i2s)
{
...
#endif
	printk("CLKDIV : 0x%08x, CLKGATE : 0x%08x, CLKSRC : 0x%08x\n",
	...
	printk("Registers of Audio Subsystem are saved\n");
...
static void audss_reg_restore(struct i2s_dai *i2s)
{
	...
	printk("Registers of Audio Subsystem are restore\n");
	return;
}
...
void audss_enable(void *i2s_info)
{
	...
	if (!i2s->clk_init) {
		...
		ret = clk_set_parent(i2s->mau_epll_clk, i2s->mout_epll);
		if (ret)
			dev_err(&i2s->pdev->dev, "failed to set parent %s of clock %s.\n",
					i2s->mau_epll_clk->name, i2s->fout_epll->name);

		ret = clk_set_parent(i2s->mout_audss, i2s->mau_epll_clk);
		if (ret)
			dev_err(&i2s->pdev->dev, "failed to set parent %s of clock %s.\n",
					i2s->mau_epll_clk->name, i2s->mout_audss->name);
...
//		clk_set_rate(i2s->dout_bus, TARGET_BUSCLK_RATE);
		...
		printk("==========> before - CLKDIV : 0x%08x, CLKGATE : 0x%08x, CLKSRC : 0x%08x\n",
				readl(i2s->audss_base + EXYNOS_CLKDIV_AUDSS_OFFSET),
				readl(i2s->audss_base + EXYNOS_CLKGATE_AUDSS_OFFSET),
				readl(i2s->audss_base + EXYNOS_CLKSRC_AUDSS_OFFSET));
				
		clk_set_rate(i2s->dout_srp, 200000000);
		clk_set_rate(i2s->dout_bus, 100000000);
		clk_set_rate(i2s->dout_i2s, 200000000);

		printk("==========> after - CLKDIV : 0x%08x, CLKGATE : 0x%08x, CLKSRC : 0x%08x\n",
				readl(i2s->audss_base + EXYNOS_CLKDIV_AUDSS_OFFSET),
				readl(i2s->audss_base + EXYNOS_CLKGATE_AUDSS_OFFSET),
				readl(i2s->audss_base + EXYNOS_CLKSRC_AUDSS_OFFSET));
		...
		dev_info(&i2s->pdev->dev, "I2S rate = %ld\n", clk_get_rate(i2s->dout_i2s));
...
static int clk_set_heirachy(struct i2s_dai *i2s)
{
	...
	i2s->mau_epll_clk = clk_get(&i2s->pdev->dev, "mau_epll_clk");
	if (IS_ERR(i2s->mau_epll_clk)) {
		dev_err(&i2s->pdev->dev, "failed to get mau_epll_clk clock\n");
		ret = PTR_ERR(i2s->mau_epll_clk);
		goto err2;
	}
...
probe_exit:
	...
	if (other) {
		...
		other->mau_epll_clk = i2s->mau_epll_clk;
