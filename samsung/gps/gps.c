...
static ssize_t gps_en_value_show(struct device *dev, 
                                 ...
                                 char *buf)
{
	int value = gpio_get_value(OMAP_GPIO_AGPS_ONOFF);
                                  ...
                                  size_t count)
{
	...
	gpio_set_value(OMAP_GPIO_AGPS_ONOFF, enable);
...
{
	omap_writew(0x4, 0x4800216A);
	gpio_set_value(OMAP_GPIO_GPS_EN, 1);
	udelay(100);
	
	// initialize GPIO
	if(gpio_is_valid(OMAP_GPIO_AGPS_ONOFF))
	{
		if(gpio_request(OMAP_GPIO_AGPS_ONOFF, NULL))
		{
			printk(KERN_ERR "Failed to request OMAP_GPIO_GPS_EN\n");
		}
		gpio_direction_output(OMAP_GPIO_AGPS_ONOFF, 0);
	}
...
static int gps_exit_proc()
{
	...
	// free gpio	
	gpio_free(OMAP_GPIO_AGPS_ONOFF);
