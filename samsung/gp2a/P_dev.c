...
#endif
{
    ...
    input_set_capability(P_dev.inputdevice, EV_ABS, ABS_X);
...
void P_dev_work_func(struct work_struct *work)
{
	...
	else
	{
		...
		input_report_abs(P_dev.inputdevice, ABS_X,  (0x01 & P_dev.regs[PROX])? DETECTED : NOT_DETECTED);
...
static int set_mode(u16 mode, u16 detection_cyc, u16 sleep_func_state)
{
	...
	else if( mode == P_MODE_B )
	{
		...
		else
		{
			...
			input_report_abs(P_dev.inputdevice, ABS_X,  NOT_DETECTED);	// ryun 20091109 dummy value.
			input_sync(P_dev.inputdevice);
...
static DEVICE_ATTR(delay, S_IRUGO|S_IWUSR|S_IWGRP, P_delay_show, P_delay_store);
...
static DEVICE_ATTR(data, S_IRUGO, P_data_show, NULL);
static DEVICE_ATTR(status, S_IRUGO, P_status_show, NULL);
