...
int L_dev_init(void)
{
    ...
    input_set_capability(L_dev.inputdevice, EV_ABS, ABS_X);
    input_set_capability(L_dev.inputdevice, EV_ABS, ABS_BRAKE);     /* status */
    input_set_capability(L_dev.inputdevice, EV_ABS, ABS_MISC);         /* wake */
    input_set_capability(L_dev.inputdevice, EV_ABS, ABS_THROTTLE);     /* enabled/delay */
    L_dev.inputdevice->name = "light"; 
...
static void L_dev_work_func (struct work_struct *unused)
{
    ...
    if( !(L_dev.saved_polling_state) )
    {
        ...
        
        {
            ...
            if(L_dev.testmode == 1)
            {
                input_report_abs(L_dev.inputdevice, ABS_X, lux);
                input_sync(L_dev.inputdevice);
            }
            else if(L_dev.testmode == 2)
            {
                input_report_abs(L_dev.inputdevice, ABS_X, lux+200);
                input_sync(L_dev.inputdevice);
            }
            else
            {
...
static DEVICE_ATTR(delay, S_IRUGO|S_IWUSR|S_IWGRP, L_delay_show, L_delay_store);
...
static ssize_t L_wake_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    input_report_abs(L_dev.inputdevice, ABS_MISC, cnt++);
