...
static int yas_acc_suspend(OMAP_GPIO_I2C_CLIENT *, pm_message_t);
static int yas_acc_resume(OMAP_GPIO_I2C_CLIENT *);
#endif

static void yas_acc_work_func(struct work_struct *);
static int yas_acc_init_proc();
static int yas_acc_exit_proc();
static int yas_acc_suspend(OMAP_GPIO_I2C_CLIENT *, pm_message_t mesg);
static int yas_acc_resume(OMAP_GPIO_I2C_CLIENT *);

#if defined(CONFIG_HAS_EARLYSUSPEND)
static int yas_acc_early_suspend(struct early_suspend *handler);
static int yas_acc_early_resume(struct early_suspend *handler);
#endif
...
static int yas_acc_i2c_write(uint8_t slave, uint8_t adr, const uint8_t *buf, int len)
{
    ...
    OMAP_GPIO_I2C_CLIENT *client = data->client;
...
static ssize_t yas_acc_raw_data_show(struct device *dev,
                                         struct device_attribute *attr,
                                         char *buf)
{
    //struct input_dev *input = to_input_dev(dev);
...
static DEVICE_ATTR(calibration, S_IRUGO|S_IWUSR|S_IWGRP,
                   yas_acc_calibration_show, yas_acc_calibration_store);
static DEVICE_ATTR(raw_data, S_IRUGO,
                   yas_acc_raw_data_show, NULL);	
#if DEBUG
...
static int yas_acc_init_proc()
{
    ...
    data->client = omap_gpio_i2c_init(OMAP_GPIO_ACC_SDA, OMAP_GPIO_ACC_SCL, 0x08, 200);
#else    
    ...
    data->early_suspend.suspend = (void *)yas_acc_early_suspend;
    data->early_suspend.resume  = (void *)yas_acc_early_resume;
