...
/*
 * Save the bootloader's log on kernel log_buf
 */

static unsigned saved_log_start;

static int __init bootloader_log_copy(char *p)
{
	...
	if (!start) {
		pr_debug("boot log: failed to get meminfo\n");
		return -EFAULT;
	}

	blog = (char *)phys_to_virt(start);

	raw_spin_lock_irqsave(&logbuf_lock, flags);

	saved_log_start = log_start;
	log_start -= size;

	/* copy on last of ring buffer */
	for (i = 0; i < size;  i++)
		LOG_BUF(log_start + i) = blog[i];

	raw_spin_unlock_irqrestore(&logbuf_lock, flags);

	return 1;
}
__setup("bootloader.log=", bootloader_log_copy);
...
void register_console(struct console *newcon)
{
	...
	if (newcon->flags & CON_PRINTBUFFER) {
		 ...
		 */
		raw_spin_lock_irqsave(&logbuf_lock, flags);
		/* to skip showing of the bootloader's log on console */
		con_start = saved_log_start;
		raw_spin_unlock_irqrestore(&logbuf_lock, flags);
		/*
