...
#ifndef CONFIG_SWAP_ENERGY_HOOKS
/* ============================================================================
 * =                              ENERGY_XXX                                  =
 * ============================================================================
 */
...
#ifndef CONFIG_SWAP_CONTEXT_HOOKS
...
static struct swap_ctx_hook switch_to_hook = {
	.hook_switch_to = handler_switch
};
#endif /* CONFIG_SWAP_CONTEXT_HOOKS */

static struct swap_energy_hooks energy_hooks = {
...
static void entry_hook_sys_rw(struct syshook_probe *p, struct pt_regs *regs)
{
	struct sys_read_data *srd =
...
static void return_hook_sys_read(struct syshook_probe *p, struct pt_regs *regs)
{
	struct sys_read_data *srd;
	struct energy_data *ed;
	int ret = regs_return_value(regs);
...
static void return_hook_sys_write(struct syshook_probe *p, struct pt_regs *regs)
{
	struct sys_read_data *srd;
	struct energy_data *ed;
	int ret = regs_return_value(regs);
...
static struct syshook_probe sys_read_hook = {
	.sys_entry = entry_hook_sys_rw,
	.sys_exit = return_hook_sys_read
};

static struct syshook_probe sys_write_hook = {
	.sys_entry = entry_hook_sys_rw,
	.sys_exit = return_hook_sys_write
};
...
int do_set_energy(void)
{
	...
	/* FIXME: add native and compat support */
	swap_syshook_reg_compat(&sys_read_hook, get_nr_compat_read());
	swap_syshook_reg_compat(&sys_write_hook, get_nr_compat_write());

	swap_energy_hooks_set(&energy_hooks);
...
{
	/* TODO: uinit lcd*/
	swap_energy_hooks_unset();
	swap_context_hooks_unreg(&switch_to_hook);

	/* FIXME: add native and compat support */
	swap_syshook_unreg_compat(&sys_write_hook);
	swap_syshook_unreg_compat(&sys_read_hook);
