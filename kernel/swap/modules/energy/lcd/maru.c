...
static struct kretprobe set_backlight_krp = {
	...
	.data_size = sizeof(struct bl_data)
};
...
static int convert_bd2power(struct backlight_device *bd)
{
	/* FYI: see marubl_send_intensity() from drivers/maru/maru_bl.c */
	...
	return off ? FB_BLANK_POWERDOWN : FB_BLANK_UNBLANK;
}

static int entry_handler_set_backlight(struct kretprobe_instance *ri,
				       struct pt_regs *regs)
{
	struct bl_data *data = (struct bl_data *)ri->data;
	struct backlight_device *bd;

	bd = (struct backlight_device *)swap_get_karg(regs, 0);

	data->brightness = bd->props.brightness;
	data->power = convert_bd2power(bd);
				     ...
				     struct pt_regs *regs)
{
	...
	if (!ret && maru_ops.notifier) {
		maru_ops.notifier(&maru_ops, LAT_BRIGHTNESS,
				  INT2VOIDP(data->brightness));

		maru_ops.notifier(&maru_ops, LAT_POWER,
				  INT2VOIDP(data->power));
	}
