...
static struct swap_td_hooks td_hooks = {
	.owner = THIS_MODULE,
	.put_task = do_put_task,
};

static int td_hooks_init(void)
{
	return swap_td_hooks_reg(&td_hooks);
}

static void td_hooks_uninit(void)
{
	swap_td_hooks_unreg(&td_hooks);
}
void swap_ktd_put_task(struct task_struct *task) {}

#else /* CONFIG_SWAP_TD_HOOKS */

static int td_hooks_init(void) { return 0; }
static void td_hooks_uninit(void) {}
void swap_ktd_put_task(struct task_struct *task)
{
	do_put_task(task);
