...
static struct td_raw restore_tdraw;
...
static struct restore_data *current_restore_td(void)
{
	...
	return (struct restore_data *)swap_td_raw(&restore_tdraw, current);
}
...
static void arch_prepare_ss_slot(struct kp_core *p)
{
	...
	flush_icache((unsigned long)p->ainsn.insn, MAX_INSN_SIZE);
}
...
int swap_arch_init_kprobes(void)
{
	...
	ret = swap_td_raw_reg(&restore_tdraw, sizeof(struct restore_data));
	if (ret)
		return ret;

	ret = dbg_iface_init();
	if (ret)
		swap_td_raw_unreg(&restore_tdraw);
