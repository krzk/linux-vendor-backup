...
static int find_best_idle(struct tp_env *env)
{
	...
	if (best_cpu >= 0) {
		trace_ems_find_best_idle(env->p, task_util,
			...
			*(unsigned int *)cpumask_bits(&active_candidates), best_cpu);
		return best_cpu;
	}
