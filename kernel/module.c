...
#include <../drivers/gud/gud-exynos3250/MobiCoreKernelApi/public/mobicore_driver_api.h>
#include <../drivers/gud/gud-exynos3250/MobiCoreKernelApi/public/mobicore_driver_cmd.h>
...
static int lkmauth(Elf_Ehdr * hdr, int len)
{
	...
	if (!lkm_sec_info_loaded) {
		/* load lkm_sec_info */
		buf_len = read_file_buf("/etc/tima/lkm_sec_info", &buf);
		if (buf_len == 0) {
			pr_err
...
#ifdef TIMA_LKM_AUTH_ENABLED
	if (lkmauth(hdr, len) != RET_LKMAUTH_SUCCESS) {
		pr_err
