...
#include <../drivers/gud/gud-exynos9110/MobiCoreDriver/public/mobicore_driver_api.h>
...
int read_file_buf(char *filename, void **buf)
{
	...
	if (!IS_ERR(f)) {
		...
		*buf = vmalloc(file_size);
		...
		// Read the file
		iRet = vfs_read(f, (char *)*buf, file_size, &f->f_pos);
...
static int lkmauth(Elf_Ehdr * hdr, int len)
{
	...
	if (!lkm_sec_info_loaded) {
		...
		pr_err("TIMA: lkmauth -- virtual address of lkm_sec_info buffer in tl is : %x\n", (uint32_t)map_info.secure_virt_addr);
