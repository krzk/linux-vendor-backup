...
void omap_pm_set_min_bus_tput(struct device *dev, u8 agent_id, unsigned long r)
{
	...
	if (r == 0) {
		...
		//resource_release("vdd2_opp", dev);  // commented by TI as we are locking VDD2 OPP
	} else {
		pr_debug("OMAP PM: add min bus tput constraint: "
			 "dev %s for agent_id %d: rate %ld KiB\n",
			 dev_name(dev), agent_id, r);
		//resource_request("vdd2_opp", dev, r);  // commented by TI as we are locking VDD2 OPP
	}
}
...
u8 omap_pm_get_max_vdd1_opp()
{
	if (cpu_is_omap3630()) {
		...
		return VDD1_OPP6;  // By default it is VDD1_OPP5
	}
}
