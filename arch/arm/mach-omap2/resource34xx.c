...
//TI_PATCH_START 20101103 - 200MHz bus clock: only one opp level for core domain.
#define DISABLE_VDD2_OPP_SCALING 1
//TI_PATCH_END
/**
...
#if  1
		resp->curr_level = 1 ;        // TI PATCH ADDED to make first time SR possible for 1 G 
		...
		int current_level)
{
	...
	if (!sr_class1p5)
		...
		current_v = target_v ;                                                       // TI PATCH ADDED START for force update of voltage after SR 1.5    
		target_v = (opp[target_level].sr_adjust_vsel) ?			
		          opp[target_level].sr_vsr_step_vsel ?				
		          opp[target_level].sr_vsr_step_vsel :				
		          opp[target_level].sr_dynamic_vnom : target_v;					
		omap_scale_voltage(t_opp, c_opp,target_v, current_v);    // recalibrated voltage updated		
		}		                                                                     //TI PATCH ADDED END for force update of voltage after SR 1.5
...
int resource_set_opp_level(int res, u32 target_level, int flags)
{
	...
	else if (res == VDD2_OPP)
	{
        ...
        {
            /* 
             * TI_PATCH_START 20101103 - 200MHz bus clock
             * only one opp level for core domain.
             */
#ifdef DISABLE_VDD2_OPP_SCALING
...
#if 0 //TI Patch: Latona TD --> to imporve touch boost delay: It's not required for Latona TD because vdd2 OPP level is fixed.
		/*
