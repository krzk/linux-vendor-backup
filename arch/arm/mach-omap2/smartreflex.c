...
int sr_recalibrate(int srid, u32 t_opp, u32 c_opp)
{
	...
	if (srid == SR1) {
		...
		if(vsel_step >  mpu_opps[target_opp_no].vsel)                // TI PATCH ADDED START to limit calibrated value to nominal voltage			
		{			
			mpu_opps[target_opp_no].sr_vsr_step_vsel =	mpu_opps[target_opp_no].vsel;      
		}	
		else                                                        // TI TI PATCH ADDED START to limit calibrated value to nominal voltage			
