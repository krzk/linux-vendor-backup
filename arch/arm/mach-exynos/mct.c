...
struct saved_mct_log_tag {
	unsigned long value;
};
#define SAVED_MCT_LOG_NUM 20
static unsigned int  saved_mct_log_cnt;
static struct saved_mct_log_tag saved_mct_log[SAVED_MCT_LOG_NUM];

static notrace unsigned long mct_read_current_timer(void)
{
        int cnt = 0;
        unsigned long ret = exynos4_read_sched_clock();

	if (++saved_mct_log_cnt >= SAVED_MCT_LOG_NUM)
		saved_mct_log_cnt = 0;

	cnt = saved_mct_log_cnt;

	saved_mct_log[cnt].value = ret;
