...
/* domains for g2d */
EXYNOS_INTERRUPT_GPD(exynos5420_pd_g2d, EXYNOS5420_G2D_CONFIGURATION, "pd-g2d");
/* domains for mscl */
...
static void __iomem *exynos5420_pwr_reg_g2d[] = {
...
static struct clk *clk_266_g2d;
static struct clk *clk_266_g2d_sw;
static struct clk *clk_acp;
...
static void exynos5420_bts_initialize(void)
{
	...
	exynos_pm_add_callback(&exynos5420_pd_g2d,EXYNOS_PROCESS_AFTER, EXYNOS_PROCESS_ON,
				false, exynos5_pm_domain_bts_on);
	exynos_pm_add_callback(&exynos5420_pd_g2d,EXYNOS_PROCESS_BEFORE, EXYNOS_PROCESS_OFF,
				false, exynos5_pm_domain_bts_off);
...
static int exynos5420_pm_domain_g2d_post_power_on(struct exynos_pm_domain *domain)
{
	clk_set_parent(clk_266_g2d, clk_266_g2d_sw);
	clk_set_rate(clk_acp, clk_get_rate(clk_266_g2d) / 2);

	return 0;
}

static int exynos5420_pm_domain_g2d_pre_power_off(struct exynos_pm_domain *domain)
{
	unsigned int temp;

	temp = __raw_readl(EXYNOS5_CLKGATE_IP_G2D);
	temp |= 0x7FEF;
	__raw_writel(temp, EXYNOS5_CLKGATE_IP_G2D);
...
static int exynos5420_pm_domain_init(void)
{
	...
	hdmi = clk_get(&s5p_device_hdmi.dev, "hdmi");
	clk_266_g2d_sw = clk_get(NULL, "aclk_266_g2d_sw");
	clk_266_g2d = clk_get(NULL, "aclk_266_g2d");
	clk_acp = clk_get(NULL, "pclk_acp");
	...
	exynos_pm_powerdomain_init(&exynos5420_pd_g2d);
	...
	exynos_pm_add_reg(&exynos5420_pd_g2d, EXYNOS_PROCESS_BEFORE, EXYNOS_PROCESS_ONOFF,
		EXYNOS5420_G2D_OPTION, 2);
	for (i = 0; i < ARRAY_SIZE(exynos5420_pwr_reg_g2d); ++i)
		exynos_pm_add_reg(&exynos5420_pd_g2d, EXYNOS_PROCESS_BEFORE, EXYNOS_PROCESS_ONOFF,
			exynos5420_pwr_reg_g2d[i], 0);
	exynos_pm_add_callback(&exynos5420_pd_g2d, EXYNOS_PROCESS_AFTER, EXYNOS_PROCESS_ON,
		true, exynos5420_pm_domain_g2d_post_power_on);
	exynos_pm_add_callback(&exynos5420_pd_g2d, EXYNOS_PROCESS_BEFORE, EXYNOS_PROCESS_OFF,
		false, exynos5420_pm_domain_g2d_pre_power_off);
	...
	exynos_pm_add_clk(&exynos54xx_pd_gscl, &SYSMMU_PLATDEV(camif2).dev, SYSMMU_CLOCK_NAME);
	...
	exynos_pm_add_platdev(&exynos5420_pd_g2d, &SYSMMU_PLATDEV(2d));
	exynos_pm_add_platdev(&exynos5420_pd_g2d, &SYSMMU_PLATDEV(2d_wr));
#ifdef CONFIG_S5P_DEV_ACE
	exynos_pm_add_platdev(&exynos5420_pd_g2d, &s5p_device_sss);
	exynos_pm_add_platdev(&exynos5420_pd_g2d, &s5p_device_slimsss);
#endif
#ifdef CONFIG_S5P_DEV_FIMG2D
	exynos_pm_add_platdev(&exynos5420_pd_g2d, &s5p_device_fimg2d);
#endif
