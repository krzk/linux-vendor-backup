...
enum bts_id {
	...
	BTS_FIMC_LITE3 = (1 << BTS_IDX_FIMC_LITE3),
...
#define BTS_FIMC (BTS_FIMC_LITE0 | BTS_FIMC_LITE1 | BTS_FIMC_LITE3)
...
static struct bts_info exynos5_bts[] = {
	...
	[BTS_IDX_FIMC_LITE3] = {
		.id = BTS_FIMC_LITE3,
		.name = "fimc_lite3",
		.pa_base = EXYNOS5_PA_BTS_FIMCLITE3,
		.pd_name = "pd-fimclite",
		.clk_name = "gscl0_flite2",
		.table.table_list = axiqos_0xcccc_table,
		.table.table_num = ARRAY_SIZE(axiqos_0xcccc_table),
		.on = true,
	},
...
static void set_bts_ip_table(struct bts_info *bts)
{
	...
	else if (bts->id & BTS_FIMC_LITE3)
		clk_enable(exynos5_bts_clk[BTS_CLOCK_FIMCLITE3].clk);
	...
	if (bts->id & BTS_G3D)
		...
		clk_disable(exynos5_bts_clk[BTS_CLOCK_FIMCLITE3].clk);
