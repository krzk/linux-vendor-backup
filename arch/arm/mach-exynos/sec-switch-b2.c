...
static void muic_init_cb(void)
{
	...
	/* for jig connected mode */
	ret = switch_dev_register(&switch_jig);
	if (ret < 0)
		pr_err("%s:%s Failed to register jig switch(%d)\n",
			MUIC_DEV_NAME, __func__, ret);
	else
		pr_info("%s:%s: switch_jig switch_dev registered\n",
			MUIC_DEV_NAME, __func__);
#endif
}
...
static int muic_charger_cb(enum muic_attached_dev cable_type)
{
	...
	struct power_supply *psy_wpc = power_supply_get_by_name("sec-wpc");
#endif
	...
	case ATTACHED_DEV_USB_MUIC:
		...
		if (!psy_wpc || !psy_wpc->get_property)
			pr_err("%s: fail to get sec-wpc psy\n", __func__);
		else
			psy_wpc->get_property(psy_wpc, POWER_SUPPLY_PROP_ONLINE, &value);
...
#ifndef CONFIG_CHARGER_MAX14577
	case ATTACHED_DEV_JIG_USB_OFF_MUIC:
	case ATTACHED_DEV_JIG_USB_ON_MUIC:
#endif
		current_cable_type = POWER_SUPPLY_TYPE_USB;
...
#ifdef CONFIG_SWITCH
	switch_set_state(&switch_jig, jig_attached);
#endif
}
