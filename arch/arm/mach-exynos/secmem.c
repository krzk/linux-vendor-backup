...
static long secmem_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SECMEM_IOC_GET_CRYPTO_LOCK:
	{
		...
		exynos_pm_interrupt_runtime_get_sync(&s5p_device_sss.dev);
#endif
#ifdef CONFIG_S5P_DEV_FIMG2D
		exynos_pm_interrupt_runtime_get_sync(&s5p_device_fimg2d.dev);
#endif
#endif
		break;
	}
...
#ifdef CONFIG_SOC_EXYNOS5420
#ifdef CONFIG_CRYPTO_S5P_DEV_ACE
		exynos_pm_interrupt_runtime_put_sync(&s5p_device_sss.dev);
#endif
#ifdef CONFIG_S5P_DEV_FIMG2D
		exynos_pm_interrupt_runtime_put_sync(&s5p_device_fimg2d.dev);
#endif
#endif
		break;
	}
