...
TRACE_EVENT(ems_find_best_idle,
	...
	TP_PROTO(struct task_struct *p, int task_util, unsigned int idle_candidates,
			unsigned int active_candidates, int best_cpu),

	TP_ARGS(p, task_util, idle_candidates, active_candidates, best_cpu),
	...
	TP_printk("comm=%s pid=%d tsk_util=%d idle=%#x active=%#x best_cpu=%d",
		  __entry->comm, __entry->pid, __entry->task_util,
		__entry->idle_candidates, __entry->active_candidates, __entry->best_cpu)
);
